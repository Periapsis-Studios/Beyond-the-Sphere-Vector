...
proc genGodotApi() =
  ...
  const targetDir = "src-bindings"/"godotapi"
  createDir(targetDir)
...
task "build", "Builds the client for the current platform":
  ...
  createDir("_dlls")
  withDir "src-bindings":
    direShell(["nimble", "c", ".."/"src-bindings"/"stub.nim", "-o:.."/"_dlls"/libFile])

task "clean", "Remove files produced by build":
  removeDir(".nimcache")
  removeDir("src-bindings"/".nimcache")
  removeDir("src-bindings"/"godotapi")
  removeDir("_dlls")
  removeFile("nakefile")

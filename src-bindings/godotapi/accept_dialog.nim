...
method builtinTextEntered*(self: AcceptDialog; arg0: string) {.gcsafe, locks: 0,
    base.}
...
proc removeButton*(self: AcceptDialog; button: Control) {.gcsafe, locks: 0.}
...
proc getOk(self: AcceptDialog): Button =
  if isNil(acceptDialogGetOkMethodBind):
    acceptDialogGetOkMethodBind = getMethod(cstring"AcceptDialog",
        cstring"get_ok")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
var acceptDialogRemoveButtonMethodBind {.threadvar.}: ptr GodotMethodBind
proc removeButton(self: AcceptDialog; button: Control) =
  if isNil(acceptDialogRemoveButtonMethodBind):
    acceptDialogRemoveButtonMethodBind = getMethod(cstring"AcceptDialog",
        cstring"remove_button")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not button.isNil:
    button.godotObject
  else:
    nil
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  acceptDialogRemoveButtonMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

...
proc frames(self: AnimatedSprite): SpriteFrames =
  if isNil(animatedSpriteGetSpriteFramesMethodBind):
    animatedSpriteGetSpriteFramesMethodBind = getMethod(
        cstring"AnimatedSprite", cstring"get_sprite_frames")
  var ptrCallRet: pointer
...
proc `frames=`(self: AnimatedSprite; val: SpriteFrames) =
  if isNil(animatedSpriteSetSpriteFramesMethodBind):
    animatedSpriteSetSpriteFramesMethodBind = getMethod(
        cstring"AnimatedSprite", cstring"set_sprite_frames")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc speedScale(self: AnimatedSprite): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animatedSpriteGetSpeedScaleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

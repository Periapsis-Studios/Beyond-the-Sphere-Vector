...
proc animation(self: AnimatedSprite3D): string =
  if isNil(animatedSprite3DGetAnimationMethodBind):
    animatedSprite3DGetAnimationMethodBind = getMethod(
        cstring"AnimatedSprite3D", cstring"get_animation")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  animatedSprite3DGetAnimationMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc `animation=`(self: AnimatedSprite3D; val: string) =
  if isNil(animatedSprite3DSetAnimationMethodBind):
    animatedSprite3DSetAnimationMethodBind = getMethod(
        cstring"AnimatedSprite3D", cstring"set_animation")
  var
...
proc `frame=`(self: AnimatedSprite3D; val: int64) =
  ...
  var ptrCallRet: pointer
  animatedSprite3DSetFrameMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

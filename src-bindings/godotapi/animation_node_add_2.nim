...
proc sync(self: AnimationNodeAdd2): bool =
  ...
  animationNodeAdd2IsUsingSyncMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `sync=`(self: AnimationNodeAdd2; val: bool) =
  if isNil(animationNodeAdd2SetUseSyncMethodBind):
    animationNodeAdd2SetUseSyncMethodBind = getMethod(
        cstring"AnimationNodeAdd2", cstring"set_use_sync")
  var

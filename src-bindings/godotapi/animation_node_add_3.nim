...
proc sync(self: AnimationNodeAdd3): bool =
  ...
  animationNodeAdd3IsUsingSyncMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `sync=`(self: AnimationNodeAdd3; val: bool) =
  if isNil(animationNodeAdd3SetUseSyncMethodBind):
    animationNodeAdd3SetUseSyncMethodBind = getMethod(
        cstring"AnimationNodeAdd3", cstring"set_use_sync")
  var

...
proc getCurrentLength*(self: AnimationNodeStateMachinePlayback): float64 {.
    gcsafe, locks: 0.}
proc getCurrentNode*(self: AnimationNodeStateMachinePlayback): string {.gcsafe,
    locks: 0.}
proc getCurrentPlayPosition*(self: AnimationNodeStateMachinePlayback): float64 {.
    gcsafe, locks: 0.}
proc getTravelPath*(self: AnimationNodeStateMachinePlayback): PoolStringArray {.
    gcsafe, locks: 0.}
proc isPlaying*(self: AnimationNodeStateMachinePlayback): bool {.gcsafe,
    locks: 0.}
proc start*(self: AnimationNodeStateMachinePlayback; node: string) {.gcsafe,
    locks: 0.}
...
proc travel*(self: AnimationNodeStateMachinePlayback; toNode: string) {.gcsafe,
    locks: 0.}
var animationNodeStateMachinePlaybackGetCurrentLengthMethodBind {.threadvar.}: ptr GodotMethodBind
proc getCurrentLength(self: AnimationNodeStateMachinePlayback): float64 =
  if isNil(animationNodeStateMachinePlaybackGetCurrentLengthMethodBind):
    animationNodeStateMachinePlaybackGetCurrentLengthMethodBind = getMethod(
        cstring"AnimationNodeStateMachinePlayback", cstring"get_current_length")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animationNodeStateMachinePlaybackGetCurrentLengthMethodBind.ptrCall(
...
var animationNodeStateMachinePlaybackGetCurrentPlayPositionMethodBind
    {.threadvar.}: ptr GodotMethodBind
proc getCurrentPlayPosition(self: AnimationNodeStateMachinePlayback): float64 =
  if isNil(animationNodeStateMachinePlaybackGetCurrentPlayPositionMethodBind):
    animationNodeStateMachinePlaybackGetCurrentPlayPositionMethodBind = getMethod(
        cstring"AnimationNodeStateMachinePlayback",
        cstring"get_current_play_position")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animationNodeStateMachinePlaybackGetCurrentPlayPositionMethodBind.ptrCall(
      self.godotObject, nil, ptrCallRet)

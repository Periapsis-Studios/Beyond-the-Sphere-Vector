...
proc `assignedAnimation=`*(self: AnimationPlayer; val: string) {.gcsafe,
    locks: 0.}
...
proc `currentAnimationLength=`*(self: AnimationPlayer; val: float64) {.gcsafe,
    locks: 0.}
proc currentAnimationPosition*(self: AnimationPlayer): float64 {.gcsafe,
    locks: 0.}
...
proc playbackDefaultBlendTime*(self: AnimationPlayer): float64 {.gcsafe,
    locks: 0.}
...
proc `playbackProcessMode=`*(self: AnimationPlayer; val: int64) {.gcsafe,
    locks: 0.}
...
proc resetOnSave*(self: AnimationPlayer): bool {.gcsafe, locks: 0.}
proc `resetOnSave=`*(self: AnimationPlayer; val: bool) {.gcsafe, locks: 0.}
...
proc findAnimation*(self: AnimationPlayer; animation: Animation): string {.
    gcsafe, locks: 0.}
proc getAnimation*(self: AnimationPlayer; name: string): Animation {.gcsafe,
    locks: 0.}
proc getAnimationList*(self: AnimationPlayer): PoolStringArray {.gcsafe,
    locks: 0.}
...
proc play*(self: AnimationPlayer; name: string = "";
           customBlend: float64 = -1.0; customSpeed: float64 = 1.0;
           fromEnd: bool = false) {.gcsafe, locks: 0.}
...
proc `playbackActive=`(self: AnimationPlayer; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  animationPlayerSetActiveMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc playbackProcessMode(self: AnimationPlayer): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animationPlayerGetAnimationProcessModeMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc playbackSpeed(self: AnimationPlayer): float64 =
  if isNil(animationPlayerGetSpeedScaleMethodBind):
    animationPlayerGetSpeedScaleMethodBind = getMethod(
        cstring"AnimationPlayer", cstring"get_speed_scale")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animationPlayerGetSpeedScaleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `playbackSpeed=`(self: AnimationPlayer; val: float64) =
  if isNil(animationPlayerSetSpeedScaleMethodBind):
    animationPlayerSetSpeedScaleMethodBind = getMethod(
        cstring"AnimationPlayer", cstring"set_speed_scale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
var animationPlayerIsResetOnSaveEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc resetOnSave(self: AnimationPlayer): bool =
  if isNil(animationPlayerIsResetOnSaveEnabledMethodBind):
    animationPlayerIsResetOnSaveEnabledMethodBind = getMethod(
        cstring"AnimationPlayer", cstring"is_reset_on_save_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animationPlayerIsResetOnSaveEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var animationPlayerSetResetOnSaveEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc `resetOnSave=`(self: AnimationPlayer; val: bool) =
  if isNil(animationPlayerSetResetOnSaveEnabledMethodBind):
    animationPlayerSetResetOnSaveEnabledMethodBind = getMethod(
        cstring"AnimationPlayer", cstring"set_reset_on_save_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  animationPlayerSetResetOnSaveEnabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc findAnimation(self: AnimationPlayer; animation: Animation): string =
  if isNil(animationPlayerFindAnimationMethodBind):
    animationPlayerFindAnimationMethodBind = getMethod(
        cstring"AnimationPlayer", cstring"find_animation")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc playBackwards(self: AnimationPlayer; name: string = "";
  ...
  if isNil(animationPlayerPlayBackwardsMethodBind):
    animationPlayerPlayBackwardsMethodBind = getMethod(
        cstring"AnimationPlayer", cstring"play_backwards")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(name)

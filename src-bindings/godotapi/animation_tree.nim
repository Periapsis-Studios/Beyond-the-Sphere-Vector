...
proc animPlayer(self: AnimationTree): NodePath =
  if isNil(animationTreeGetAnimationPlayerMethodBind):
    animationTreeGetAnimationPlayerMethodBind = getMethod(
        cstring"AnimationTree", cstring"get_animation_player")
  var ptrCallRet: pointer
...
proc `animPlayer=`(self: AnimationTree; val: NodePath) =
  if isNil(animationTreeSetAnimationPlayerMethodBind):
    animationTreeSetAnimationPlayerMethodBind = getMethod(
        cstring"AnimationTree", cstring"set_animation_player")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc processMode(self: AnimationTree): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  animationTreeGetProcessModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc rootMotionTrack(self: AnimationTree): NodePath =
  if isNil(animationTreeGetRootMotionTrackMethodBind):
    animationTreeGetRootMotionTrackMethodBind = getMethod(
        cstring"AnimationTree", cstring"get_root_motion_track")
  var ptrCallRet: pointer
  var ptrCallVal: GodotNodePath
  ptrCallRet = addr(ptrCallVal)
...
proc `rootMotionTrack=`(self: AnimationTree; val: NodePath) =
  if isNil(animationTreeSetRootMotionTrackMethodBind):
    animationTreeSetRootMotionTrackMethodBind = getMethod(
        cstring"AnimationTree", cstring"set_root_motion_track")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = val.godotNodePath
...
proc `treeRoot=`(self: AnimationTree; val: AnimationNode) =
  ...
  var ptrCallRet: pointer
  animationTreeSetTreeRootMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

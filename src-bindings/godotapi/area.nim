...
method areaInout*(self: Area; arg0: int64; arg1: RID; arg2: int64; arg3: int64;
...
method bodyInout*(self: Area; arg0: int64; arg1: RID; arg2: int64; arg3: int64;
                  arg4: int64) {.gcsafe, locks: 0, base.}
...
proc `gravity=`(self: Area; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  areaSetGravityMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc gravityDistanceScale(self: Area): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  areaGetGravityDistanceScaleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc monitorable(self: Area): bool =
  if isNil(areaIsMonitorableMethodBind):
    areaIsMonitorableMethodBind = getMethod(cstring"Area",
...
proc monitoring(self: Area): bool =
  if isNil(areaIsMonitoringMethodBind):
    areaIsMonitoringMethodBind = getMethod(cstring"Area",
...
proc `monitoring=`(self: Area; val: bool) =
  if isNil(areaSetMonitoringMethodBind):
    areaSetMonitoringMethodBind = getMethod(cstring"Area",
...
proc reverbBusName(self: Area): string =
  if isNil(areaGetReverbBusMethodBind):
    areaGetReverbBusMethodBind = getMethod(cstring"Area",
        cstring"get_reverb_bus")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
...
proc `reverbBusName=`(self: Area; val: string) =
  if isNil(areaSetReverbBusMethodBind):
    areaSetReverbBusMethodBind = getMethod(cstring"Area",
        cstring"set_reverb_bus")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(val)
...
proc `spaceOverride=`(self: Area; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  areaSetSpaceOverrideModeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc overlapsArea(self: Area; area: Node): bool =
  if isNil(areaOverlapsAreaMethodBind):
    areaOverlapsAreaMethodBind = getMethod(cstring"Area",
...
proc overlapsBody(self: Area; body: Node): bool =
  if isNil(areaOverlapsBodyMethodBind):
    areaOverlapsBodyMethodBind = getMethod(cstring"Area",

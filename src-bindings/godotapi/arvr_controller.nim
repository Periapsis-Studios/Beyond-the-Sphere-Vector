...
proc getJoystickAxis*(self: ARVRController; axis: int64): float64 {.gcsafe,
    locks: 0.}
proc getJoystickId*(self: ARVRController): int64 {.gcsafe, locks: 0.}
proc getMesh*(self: ARVRController): Mesh {.gcsafe, locks: 0.}
proc isButtonPressed*(self: ARVRController; button: int64): int64 {.gcsafe,
    locks: 0.}
...
proc controllerId(self: ARVRController): int64 =
  if isNil(aRVRControllerGetControllerIdMethodBind):
    aRVRControllerGetControllerIdMethodBind = getMethod(
        cstring"ARVRController", cstring"get_controller_id")
...
proc `controllerId=`(self: ARVRController; val: int64) =
  if isNil(aRVRControllerSetControllerIdMethodBind):
    aRVRControllerSetControllerIdMethodBind = getMethod(
        cstring"ARVRController", cstring"set_controller_id")
  var
...
proc getJoystickAxis(self: ARVRController; axis: int64): float64 =
  if isNil(aRVRControllerGetJoystickAxisMethodBind):
    aRVRControllerGetJoystickAxisMethodBind = getMethod(
        cstring"ARVRController", cstring"get_joystick_axis")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getJoystickId(self: ARVRController): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRControllerGetJoystickIdMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc isButtonPressed(self: ARVRController; button: int64): int64 =
  if isNil(aRVRControllerIsButtonPressedMethodBind):
    aRVRControllerIsButtonPressedMethodBind = getMethod(
        cstring"ARVRController", cstring"is_button_pressed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
proc `arIsAnchorDetectionEnabled=`*(self: ARVRInterface; val: bool) {.gcsafe,
    locks: 0.}
proc interfaceIsInitialized*(self: ARVRInterface): bool {.gcsafe, locks: 0.}
proc `interfaceIsInitialized=`*(self: ARVRInterface; val: bool) {.gcsafe,
    locks: 0.}
...
proc getCameraFeedId(self: ARVRInterface): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRInterfaceGetCameraFeedIdMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getCapabilities(self: ARVRInterface): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRInterfaceGetCapabilitiesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getTrackingStatus(self: ARVRInterface): int64 =
  if isNil(aRVRInterfaceGetTrackingStatusMethodBind):
    aRVRInterfaceGetTrackingStatusMethodBind = getMethod(
        cstring"ARVRInterface", cstring"get_tracking_status")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

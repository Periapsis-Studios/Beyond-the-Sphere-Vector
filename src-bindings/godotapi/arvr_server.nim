...
proc addInterface*(interfacee: ARVRInterface) {.gcsafe, locks: 0.}
proc addTracker*(tracker: ARVRPositionalTracker) {.gcsafe, locks: 0.}
proc centerOnHmd*(rotationMode: int64; keepHeight: bool) {.gcsafe, locks: 0.}
proc clearPrimaryInterfaceIf*(interfacee: ARVRInterface) {.gcsafe, locks: 0.}
...
proc removeInterface*(interfacee: ARVRInterface) {.gcsafe, locks: 0.}
proc removeTracker*(tracker: ARVRPositionalTracker) {.gcsafe, locks: 0.}
proc setPrimaryInterface*(interfacee: ARVRInterface) {.gcsafe, locks: 0.}
proc setWorldScale*(arg0: float64) {.gcsafe, locks: 0.}
var aRVRServerAddInterfaceMethodBind {.threadvar.}: ptr GodotMethodBind
proc addInterface(interfacee: ARVRInterface) =
  if isNil(singletonARVRServer):
    singletonARVRServer = getSingleton[ARVRServer]()
  let self = singletonARVRServer
  if isNil(aRVRServerAddInterfaceMethodBind):
    aRVRServerAddInterfaceMethodBind = getMethod(cstring"ARVRServer",
        cstring"add_interface")
  var
  ...
  aRVRServerAddInterfaceMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)

var aRVRServerAddTrackerMethodBind {.threadvar.}: ptr GodotMethodBind
proc addTracker(tracker: ARVRPositionalTracker) =
  if isNil(singletonARVRServer):
    singletonARVRServer = getSingleton[ARVRServer]()
  let self = singletonARVRServer
  if isNil(aRVRServerAddTrackerMethodBind):
    aRVRServerAddTrackerMethodBind = getMethod(cstring"ARVRServer",
        cstring"add_tracker")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not tracker.isNil:
    tracker.godotObject
  else:
    nil
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  aRVRServerAddTrackerMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
var aRVRServerClearPrimaryInterfaceIfMethodBind {.threadvar.}: ptr GodotMethodBind
proc clearPrimaryInterfaceIf(interfacee: ARVRInterface) =
  if isNil(singletonARVRServer):
    singletonARVRServer = getSingleton[ARVRServer]()
  let self = singletonARVRServer
  if isNil(aRVRServerClearPrimaryInterfaceIfMethodBind):
    aRVRServerClearPrimaryInterfaceIfMethodBind = getMethod(
        cstring"ARVRServer", cstring"clear_primary_interface_if")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not interfacee.isNil:
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  aRVRServerClearPrimaryInterfaceIfMethodBind.ptrCall(self.godotObject,
...
proc getInterfaceCount(): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRServerGetInterfaceCountMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getLastCommitUsec(): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRServerGetLastCommitUsecMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getLastProcessUsec(): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRServerGetLastProcessUsecMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getReferenceFrame(): Transform =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  aRVRServerGetReferenceFrameMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var aRVRServerRemoveInterfaceMethodBind {.threadvar.}: ptr GodotMethodBind
proc removeInterface(interfacee: ARVRInterface) =
  if isNil(singletonARVRServer):
    singletonARVRServer = getSingleton[ARVRServer]()
  let self = singletonARVRServer
  if isNil(aRVRServerRemoveInterfaceMethodBind):
    aRVRServerRemoveInterfaceMethodBind = getMethod(cstring"ARVRServer",
        cstring"remove_interface")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not interfacee.isNil:
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  aRVRServerRemoveInterfaceMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var aRVRServerRemoveTrackerMethodBind {.threadvar.}: ptr GodotMethodBind
proc removeTracker(tracker: ARVRPositionalTracker) =
  if isNil(singletonARVRServer):
    singletonARVRServer = getSingleton[ARVRServer]()
  let self = singletonARVRServer
  if isNil(aRVRServerRemoveTrackerMethodBind):
    aRVRServerRemoveTrackerMethodBind = getMethod(cstring"ARVRServer",
        cstring"remove_tracker")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not tracker.isNil:
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  aRVRServerRemoveTrackerMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)

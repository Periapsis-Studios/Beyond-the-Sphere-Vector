...
proc `feedbackdelayMs=`*(self: AudioEffectDelay; val: float64) {.gcsafe,
    locks: 0.}
proc feedbacklevelDb*(self: AudioEffectDelay): float64 {.gcsafe, locks: 0.}
proc `feedbacklevelDb=`*(self: AudioEffectDelay; val: float64) {.gcsafe,
    locks: 0.}
proc feedbacklowpass*(self: AudioEffectDelay): float64 {.gcsafe, locks: 0.}
proc `feedbacklowpass=`*(self: AudioEffectDelay; val: float64) {.gcsafe,
    locks: 0.}
...
proc tap1active(self: AudioEffectDelay): bool =
  if isNil(audioEffectDelayIsTap1ActiveMethodBind):
    audioEffectDelayIsTap1ActiveMethodBind = getMethod(
        cstring"AudioEffectDelay", cstring"is_tap1_active")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioEffectDelayIsTap1ActiveMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc tap2active(self: AudioEffectDelay): bool =
  if isNil(audioEffectDelayIsTap2ActiveMethodBind):
    audioEffectDelayIsTap2ActiveMethodBind = getMethod(
        cstring"AudioEffectDelay", cstring"is_tap2_active")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioEffectDelayIsTap2ActiveMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

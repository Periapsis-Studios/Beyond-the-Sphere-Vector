...
proc cutoffHz(self: AudioEffectFilter): float64 =
  if isNil(audioEffectFilterGetCutoffMethodBind):
    audioEffectFilterGetCutoffMethodBind = getMethod(
        cstring"AudioEffectFilter", cstring"get_cutoff")
...
proc `cutoffHz=`(self: AudioEffectFilter; val: float64) =
  if isNil(audioEffectFilterSetCutoffMethodBind):
    audioEffectFilterSetCutoffMethodBind = getMethod(
        cstring"AudioEffectFilter", cstring"set_cutoff")
  var
...
proc `gain=`(self: AudioEffectFilter; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  audioEffectFilterSetGainMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

...
proc feedback(self: AudioEffectPhaser): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioEffectPhaserGetFeedbackMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc rateHz(self: AudioEffectPhaser): float64 =
  if isNil(audioEffectPhaserGetRateHzMethodBind):
    audioEffectPhaserGetRateHzMethodBind = getMethod(
        cstring"AudioEffectPhaser", cstring"get_rate_hz")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `rateHz=`(self: AudioEffectPhaser; val: float64) =
  if isNil(audioEffectPhaserSetRateHzMethodBind):
    audioEffectPhaserSetRateHzMethodBind = getMethod(
        cstring"AudioEffectPhaser", cstring"set_rate_hz")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

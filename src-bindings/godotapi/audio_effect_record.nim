...
proc getRecording*(self: AudioEffectRecord): AudioStreamSample {.gcsafe,
    locks: 0.}
proc isRecordingActive*(self: AudioEffectRecord): bool {.gcsafe, locks: 0.}
proc setRecordingActive*(self: AudioEffectRecord; record: bool) {.gcsafe,
    locks: 0.}
...
proc format(self: AudioEffectRecord): int64 =
  if isNil(audioEffectRecordGetFormatMethodBind):
    audioEffectRecordGetFormatMethodBind = getMethod(
        cstring"AudioEffectRecord", cstring"get_format")
...
proc `format=`(self: AudioEffectRecord; val: int64) =
  if isNil(audioEffectRecordSetFormatMethodBind):
    audioEffectRecordSetFormatMethodBind = getMethod(
        cstring"AudioEffectRecord", cstring"set_format")
  var

...
proc `predelayFeedback=`*(self: AudioEffectReverb; val: float64) {.gcsafe,
    locks: 0.}
...
proc damping(self: AudioEffectReverb): float64 =
  if isNil(audioEffectReverbGetDampingMethodBind):
    audioEffectReverbGetDampingMethodBind = getMethod(
        cstring"AudioEffectReverb", cstring"get_damping")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioEffectReverbGetDampingMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `damping=`(self: AudioEffectReverb; val: float64) =
  if isNil(audioEffectReverbSetDampingMethodBind):
    audioEffectReverbSetDampingMethodBind = getMethod(
        cstring"AudioEffectReverb", cstring"set_damping")
  var
...
proc roomSize(self: AudioEffectReverb): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioEffectReverbGetRoomSizeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc spread(self: AudioEffectReverb): float64 =
  if isNil(audioEffectReverbGetSpreadMethodBind):
    audioEffectReverbGetSpreadMethodBind = getMethod(
        cstring"AudioEffectReverb", cstring"get_spread")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `spread=`(self: AudioEffectReverb; val: float64) =
  if isNil(audioEffectReverbSetSpreadMethodBind):
    audioEffectReverbSetSpreadMethodBind = getMethod(
        cstring"AudioEffectReverb", cstring"set_spread")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

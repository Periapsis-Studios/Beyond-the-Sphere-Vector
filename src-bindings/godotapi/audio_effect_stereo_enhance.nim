...
proc `panPullout=`*(self: AudioEffectStereoEnhance; val: float64) {.gcsafe,
    locks: 0.}
proc surround*(self: AudioEffectStereoEnhance): float64 {.gcsafe, locks: 0.}
proc `surround=`*(self: AudioEffectStereoEnhance; val: float64) {.gcsafe,
    locks: 0.}
...
proc timePulloutMs(self: AudioEffectStereoEnhance): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioEffectStereoEnhanceGetTimePulloutMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

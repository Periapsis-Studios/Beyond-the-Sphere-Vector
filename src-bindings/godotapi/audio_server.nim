...
proc addBusEffect*(busIdx: int64; effect: AudioEffect;
...
proc getBusEffect*(busIdx: int64; effectIdx: int64): AudioEffect {.gcsafe,
    locks: 0.}
proc getBusEffectCount*(busIdx: int64): int64 {.gcsafe, locks: 0.}
proc getBusEffectInstance*(busIdx: int64; effectIdx: int64;
                           channel: int64 = 0'i64): AudioEffectInstance {.
    gcsafe, locks: 0.}
proc getBusIndex*(busName: string): int64 {.gcsafe, locks: 0.}
proc getBusName*(busIdx: int64): string {.gcsafe, locks: 0.}
proc getBusPeakVolumeLeftDb*(busIdx: int64; channel: int64): float64 {.gcsafe,
    locks: 0.}
...
proc isBusEffectEnabled*(busIdx: int64; effectIdx: int64): bool {.gcsafe,
    locks: 0.}
...
proc addBus(atPosition: int64 = -1'i64) =
  ...
  if isNil(audioServerAddBusMethodBind):
    audioServerAddBusMethodBind = getMethod(cstring"AudioServer",
        cstring"add_bus")
  var
...
proc captureGetDevice(): string =
  ...
  audioServerCaptureGetDeviceMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc generateBusLayout(): AudioBusLayout =
  ...
  ptrCallRet = addr(ptrCallVal)
  audioServerGenerateBusLayoutMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getBusPeakVolumeLeftDb(busIdx: int64; channel: int64): float64 =
  ...
  if isNil(audioServerGetBusPeakVolumeLeftDbMethodBind):
    audioServerGetBusPeakVolumeLeftDbMethodBind = getMethod(
        cstring"AudioServer", cstring"get_bus_peak_volume_left_db")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(busIdx)
...
proc getOutputLatency(): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioServerGetOutputLatencyMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getTimeToNextMix(): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioServerGetTimeToNextMixMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc isBusBypassingEffects(busIdx: int64): bool =
  ...
  if isNil(audioServerIsBusBypassingEffectsMethodBind):
    audioServerIsBusBypassingEffectsMethodBind = getMethod(
        cstring"AudioServer", cstring"is_bus_bypassing_effects")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(busIdx)
...
proc unlock() =
  ...
  if isNil(audioServerUnlockMethodBind):
    audioServerUnlockMethodBind = getMethod(cstring"AudioServer",
        cstring"unlock")
  var ptrCallRet: pointer

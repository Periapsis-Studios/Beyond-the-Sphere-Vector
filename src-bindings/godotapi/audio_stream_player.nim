...
proc getStreamPlayback*(self: AudioStreamPlayer): AudioStreamPlayback {.gcsafe,
    locks: 0.}
proc play*(self: AudioStreamPlayer; fromPosition: float64 = 0.0) {.gcsafe,
    locks: 0.}
...
proc playing(self: AudioStreamPlayer): bool =
  if isNil(audioStreamPlayerIsPlayingMethodBind):
    audioStreamPlayerIsPlayingMethodBind = getMethod(
        cstring"AudioStreamPlayer", cstring"is_playing")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc stream(self: AudioStreamPlayer): AudioStream =
  if isNil(audioStreamPlayerGetStreamMethodBind):
    audioStreamPlayerGetStreamMethodBind = getMethod(
        cstring"AudioStreamPlayer", cstring"get_stream")
  var ptrCallRet: pointer
...
proc `stream=`(self: AudioStreamPlayer; val: AudioStream) =
  if isNil(audioStreamPlayerSetStreamMethodBind):
    audioStreamPlayerSetStreamMethodBind = getMethod(
        cstring"AudioStreamPlayer", cstring"set_stream")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc volumeDb(self: AudioStreamPlayer): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayerGetVolumeDbMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

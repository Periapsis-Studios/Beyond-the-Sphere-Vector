...
proc getStreamPlayback*(self: AudioStreamPlayer2D): AudioStreamPlayback {.
    gcsafe, locks: 0.}
proc play*(self: AudioStreamPlayer2D; fromPosition: float64 = 0.0) {.gcsafe,
    locks: 0.}
...
proc bus(self: AudioStreamPlayer2D): string =
  if isNil(audioStreamPlayer2DGetBusMethodBind):
    audioStreamPlayer2DGetBusMethodBind = getMethod(
        cstring"AudioStreamPlayer2D", cstring"get_bus")
  var ptrCallRet: pointer
...
proc `bus=`(self: AudioStreamPlayer2D; val: string) =
  if isNil(audioStreamPlayer2DSetBusMethodBind):
    audioStreamPlayer2DSetBusMethodBind = getMethod(
        cstring"AudioStreamPlayer2D", cstring"set_bus")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc playing(self: AudioStreamPlayer2D): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer2DIsPlayingMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc stream(self: AudioStreamPlayer2D): AudioStream =
  ...
  ptrCallRet = addr(ptrCallVal)
  audioStreamPlayer2DGetStreamMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getPlaybackPosition(self: AudioStreamPlayer2D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer2DGetPlaybackPositionMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

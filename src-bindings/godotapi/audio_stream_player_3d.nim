...
proc `attenuationModel=`*(self: AudioStreamPlayer3D; val: int64) {.gcsafe,
    locks: 0.}
...
proc `dopplerTracking=`*(self: AudioStreamPlayer3D; val: int64) {.gcsafe,
    locks: 0.}
proc emissionAngleDegrees*(self: AudioStreamPlayer3D): float64 {.gcsafe,
    locks: 0.}
...
proc `emissionAngleEnabled=`*(self: AudioStreamPlayer3D; val: bool) {.gcsafe,
    locks: 0.}
proc emissionAngleFilterAttenuationDb*(self: AudioStreamPlayer3D): float64 {.
    gcsafe, locks: 0.}
proc `emissionAngleFilterAttenuationDb=`*(self: AudioStreamPlayer3D;
    val: float64) {.gcsafe, locks: 0.}
...
proc `outOfRangeMode=`*(self: AudioStreamPlayer3D; val: int64) {.gcsafe,
    locks: 0.}
...
proc getStreamPlayback*(self: AudioStreamPlayer3D): AudioStreamPlayback {.
    gcsafe, locks: 0.}
proc play*(self: AudioStreamPlayer3D; fromPosition: float64 = 0.0) {.gcsafe,
    locks: 0.}
...
proc attenuationModel(self: AudioStreamPlayer3D): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer3DGetAttenuationModelMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc bus(self: AudioStreamPlayer3D): string =
  if isNil(audioStreamPlayer3DGetBusMethodBind):
    audioStreamPlayer3DGetBusMethodBind = getMethod(
        cstring"AudioStreamPlayer3D", cstring"get_bus")
  var ptrCallRet: pointer
...
proc `bus=`(self: AudioStreamPlayer3D; val: string) =
  if isNil(audioStreamPlayer3DSetBusMethodBind):
    audioStreamPlayer3DSetBusMethodBind = getMethod(
        cstring"AudioStreamPlayer3D", cstring"set_bus")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var audioStreamPlayer3DGetEmissionAngleFilterAttenuationDbMethodBind
...
var audioStreamPlayer3DSetEmissionAngleFilterAttenuationDbMethodBind
    {.threadvar.}: ptr GodotMethodBind
...
proc maxDb(self: AudioStreamPlayer3D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer3DGetMaxDbMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc playing(self: AudioStreamPlayer3D): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer3DIsPlayingMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc stream(self: AudioStreamPlayer3D): AudioStream =
  ...
  ptrCallRet = addr(ptrCallVal)
  audioStreamPlayer3DGetStreamMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc unitDb(self: AudioStreamPlayer3D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer3DGetUnitDbMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getPlaybackPosition(self: AudioStreamPlayer3D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  audioStreamPlayer3DGetPlaybackPositionMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

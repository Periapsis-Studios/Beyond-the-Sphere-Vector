...
proc interior*(self: BakedLightmapData): bool {.gcsafe, locks: 0.}
proc `interior=`*(self: BakedLightmapData; val: bool) {.gcsafe, locks: 0.}
...
proc addUser*(self: BakedLightmapData; path: NodePath; lightmap: Resource;
              lightmapSlice: int64; lightmapUvRect: Rect2; instance: int64) {.
    gcsafe, locks: 0.}
proc clearData*(self: BakedLightmapData) {.gcsafe, locks: 0.}
proc clearUsers*(self: BakedLightmapData) {.gcsafe, locks: 0.}
proc getUserCount*(self: BakedLightmapData): int64 {.gcsafe, locks: 0.}
proc getUserLightmap*(self: BakedLightmapData; userIdx: int64): Resource {.
    gcsafe, locks: 0.}
proc getUserPath*(self: BakedLightmapData; userIdx: int64): NodePath {.gcsafe,
    locks: 0.}
...
proc bounds(self: BakedLightmapData): AABB =
  if isNil(bakedLightmapDataGetBoundsMethodBind):
    bakedLightmapDataGetBoundsMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"get_bounds")
...
proc `bounds=`(self: BakedLightmapData; val: AABB) =
  if isNil(bakedLightmapDataSetBoundsMethodBind):
    bakedLightmapDataSetBoundsMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"set_bounds")
  var
...
proc cellSpaceTransform(self: BakedLightmapData): Transform =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  bakedLightmapDataGetCellSpaceTransformMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc energy(self: BakedLightmapData): float64 =
  if isNil(bakedLightmapDataGetEnergyMethodBind):
    bakedLightmapDataGetEnergyMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"get_energy")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `energy=`(self: BakedLightmapData; val: float64) =
  if isNil(bakedLightmapDataSetEnergyMethodBind):
    bakedLightmapDataSetEnergyMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"set_energy")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
var bakedLightmapDataIsInteriorMethodBind {.threadvar.}: ptr GodotMethodBind
proc interior(self: BakedLightmapData): bool =
  if isNil(bakedLightmapDataIsInteriorMethodBind):
    bakedLightmapDataIsInteriorMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"is_interior")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  bakedLightmapDataIsInteriorMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var bakedLightmapDataSetInteriorMethodBind {.threadvar.}: ptr GodotMethodBind
proc `interior=`(self: BakedLightmapData; val: bool) =
  if isNil(bakedLightmapDataSetInteriorMethodBind):
    bakedLightmapDataSetInteriorMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"set_interior")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  bakedLightmapDataSetInteriorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc octree(self: BakedLightmapData): PoolByteArray =
  if isNil(bakedLightmapDataGetOctreeMethodBind):
    bakedLightmapDataGetOctreeMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"get_octree")
  var ptrCallRet: pointer
...
proc `octree=`(self: BakedLightmapData; val: PoolByteArray) =
  if isNil(bakedLightmapDataSetOctreeMethodBind):
    bakedLightmapDataSetOctreeMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"set_octree")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc userData(self: BakedLightmapData): Array =
  ...
  ptrCallRet = addr(ptrCallVal)
  bakedLightmapDataUnderscoregetUserDataMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc addUser(self: BakedLightmapData; path: NodePath; lightmap: Resource;
  ...
  argsToPassToGodot[][2] = unsafeAddr(lightmapSlice)
  argsToPassToGodot[][3] = unsafeAddr(lightmapUvRect)
  argsToPassToGodot[][4] = unsafeAddr(instance)
  var ptrCallRet: pointer
  bakedLightmapDataAddUserMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var bakedLightmapDataClearDataMethodBind {.threadvar.}: ptr GodotMethodBind
proc clearData(self: BakedLightmapData) =
  if isNil(bakedLightmapDataClearDataMethodBind):
    bakedLightmapDataClearDataMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"clear_data")
  var ptrCallRet: pointer
  bakedLightmapDataClearDataMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc clearUsers(self: BakedLightmapData) =
  if isNil(bakedLightmapDataClearUsersMethodBind):
    bakedLightmapDataClearUsersMethodBind = getMethod(
        cstring"BakedLightmapData", cstring"clear_users")
  var ptrCallRet: pointer
  bakedLightmapDataClearUsersMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

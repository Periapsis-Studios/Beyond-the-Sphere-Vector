...
proc setPressedNoSignal*(self: BaseButton; pressed: bool) {.gcsafe, locks: 0.}
...
proc `group=`(self: BaseButton; val: ButtonGroup) =
  ...
  var ptrCallRet: pointer
  baseButtonSetButtonGroupMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
method pressed(self: BaseButton) =
  if isNil(baseButtonUnderscorepressedMethodBind):
    baseButtonUnderscorepressedMethodBind = getMethod(cstring"BaseButton",
        cstring"_pressed")
  var ptrCallRet: pointer
  baseButtonUnderscorepressedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method unhandledInput(self: BaseButton; event: InputEvent) =
  if isNil(baseButtonUnderscoreunhandledInputMethodBind):
    baseButtonUnderscoreunhandledInputMethodBind = getMethod(
        cstring"BaseButton", cstring"_unhandled_input")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not event.isNil:
...
var baseButtonSetPressedNoSignalMethodBind {.threadvar.}: ptr GodotMethodBind
proc setPressedNoSignal(self: BaseButton; pressed: bool) =
  if isNil(baseButtonSetPressedNoSignalMethodBind):
    baseButtonSetPressedNoSignalMethodBind = getMethod(cstring"BaseButton",
        cstring"set_pressed_no_signal")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(pressed)
  var ptrCallRet: pointer
  baseButtonSetPressedNoSignalMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

...
proc isPositionBehind*(self: Camera; worldPoint: Vector3): bool {.gcsafe,
    locks: 0.}
...
proc projectPosition*(self: Camera; screenPoint: Vector2; zDepth: float64): Vector3 {.
    gcsafe, locks: 0.}
proc projectRayNormal*(self: Camera; screenPoint: Vector2): Vector3 {.gcsafe,
    locks: 0.}
proc projectRayOrigin*(self: Camera; screenPoint: Vector2): Vector3 {.gcsafe,
    locks: 0.}
proc setCullMaskBit*(self: Camera; layer: int64; enable: bool) {.gcsafe,
    locks: 0.}
...
proc setPerspective*(self: Camera; fov: float64; zNear: float64; zFar: float64) {.
    gcsafe, locks: 0.}
proc unprojectPosition*(self: Camera; worldPoint: Vector3): Vector2 {.gcsafe,
    locks: 0.}
...
proc `current=`(self: Camera; val: bool) =
  if isNil(cameraSetCurrentMethodBind):
    cameraSetCurrentMethodBind = getMethod(cstring"Camera",
        cstring"set_current")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `dopplerTracking=`(self: Camera; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cameraSetDopplerTrackingMethodBind.ptrCall(self.godotObject,
...
proc `far=`(self: Camera; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cameraSetZfarMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc hOffset(self: Camera): float64 =
  if isNil(cameraGetHOffsetMethodBind):
    cameraGetHOffsetMethodBind = getMethod(cstring"Camera",
...
proc `hOffset=`(self: Camera; val: float64) =
  if isNil(cameraSetHOffsetMethodBind):
    cameraSetHOffsetMethodBind = getMethod(cstring"Camera",
...
proc `near=`(self: Camera; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cameraSetZnearMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc `size=`(self: Camera; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cameraSetSizeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc vOffset(self: Camera): float64 =
  if isNil(cameraGetVOffsetMethodBind):
    cameraGetVOffsetMethodBind = getMethod(cstring"Camera",
...
proc `vOffset=`(self: Camera; val: float64) =
  if isNil(cameraSetVOffsetMethodBind):
    cameraSetVOffsetMethodBind = getMethod(cstring"Camera",
...
proc getFrustum(self: Camera): Array =
  if isNil(cameraGetFrustumMethodBind):
    cameraGetFrustumMethodBind = getMethod(cstring"Camera",
        cstring"get_frustum")
  var ptrCallRet: pointer
...
proc makeCurrent(self: Camera) =
  if isNil(cameraMakeCurrentMethodBind):
    cameraMakeCurrentMethodBind = getMethod(cstring"Camera",
...
proc setFrustum(self: Camera; size: float64; offset: Vector2; zNear: float64;
  ...
  if isNil(cameraSetFrustumMethodBind):
    cameraSetFrustumMethodBind = getMethod(cstring"Camera",
        cstring"set_frustum")
  var

...
method editSetPosition*(self: CanvasItem; position: Vector2) {.gcsafe, locks: 0,
    base.}
method editSetRect*(self: CanvasItem; rect: Rect2) {.gcsafe, locks: 0, base.}
method editSetRotation*(self: CanvasItem; degrees: float64) {.gcsafe, locks: 0,
    base.}
method editSetScale*(self: CanvasItem; scale: Vector2) {.gcsafe, locks: 0, base.}
method editSetState*(self: CanvasItem; state: Dictionary) {.gcsafe, locks: 0,
    base.}
...
proc drawArc*(self: CanvasItem; center: Vector2; radius: float64;
              startAngle: float64; endAngle: float64; pointCount: int64;
              color: Color; width: float64 = 1.0; antialiased: bool = false) {.
    gcsafe, locks: 0.}
proc drawChar*(self: CanvasItem; font: Font; position: Vector2; char: string;
    ...
    gcsafe, locks: 0.}
proc drawCircle*(self: CanvasItem; position: Vector2; radius: float64;
                 color: Color) {.gcsafe, locks: 0.}
proc drawColoredPolygon*(self: CanvasItem; points: PoolVector2Array;
                         color: Color;
...
proc drawPolygon*(self: CanvasItem; points: PoolVector2Array;
...
proc drawSetTransformMatrix*(self: CanvasItem; xform: Transform2D) {.gcsafe,
    locks: 0.}
proc drawString*(self: CanvasItem; font: Font; position: Vector2; text: string;
                 modulate: Color = initColor(1.0, 1.0, 1.0, 1.0);
                 clipW: int64 = -1'i64) {.gcsafe, locks: 0.}
proc drawStyleBox*(self: CanvasItem; styleBox: StyleBox; rect: Rect2) {.gcsafe,
    locks: 0.}
proc drawTexture*(self: CanvasItem; texture: Texture; position: Vector2;
                  modulate: Color = initColor(1.0, 1.0, 1.0, 1.0);
                  normalMap: Texture = nil) {.gcsafe, locks: 0.}
proc drawTextureRect*(self: CanvasItem; texture: Texture; rect: Rect2;
                      tile: bool;
                      modulate: Color = initColor(1.0, 1.0, 1.0, 1.0);
...
proc getGlobalTransformWithCanvas*(self: CanvasItem): Transform2D {.gcsafe,
    locks: 0.}
...
proc isLocalTransformNotificationEnabled*(self: CanvasItem): bool {.gcsafe,
    locks: 0.}
...
proc showOnTop(self: CanvasItem): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  canvasItemUnderscoreisOnTopMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method updateCallback(self: CanvasItem) =
  if isNil(canvasItemUnderscoreupdateCallbackMethodBind):
    canvasItemUnderscoreupdateCallbackMethodBind = getMethod(
        cstring"CanvasItem", cstring"_update_callback")
  var ptrCallRet: pointer
...
proc drawArc(self: CanvasItem; center: Vector2; radius: float64;
  ...
  if isNil(canvasItemDrawArcMethodBind):
    canvasItemDrawArcMethodBind = getMethod(cstring"CanvasItem",
        cstring"draw_arc")
  var
...
proc getCanvasTransform(self: CanvasItem): Transform2D =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  canvasItemGetCanvasTransformMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getGlobalTransform(self: CanvasItem): Transform2D =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  canvasItemGetGlobalTransformMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getGlobalTransformWithCanvas(self: CanvasItem): Transform2D =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  canvasItemGetGlobalTransformWithCanvasMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc makeCanvasPositionLocal(self: CanvasItem; screenPoint: Vector2): Vector2 =
  if isNil(canvasItemMakeCanvasPositionLocalMethodBind):
    canvasItemMakeCanvasPositionLocalMethodBind = getMethod(
        cstring"CanvasItem", cstring"make_canvas_position_local")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc makeInputLocal(self: CanvasItem; event: InputEvent): InputEvent =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  canvasItemMakeInputLocalMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc setNotifyLocalTransform(self: CanvasItem; enable: bool) =
  if isNil(canvasItemSetNotifyLocalTransformMethodBind):
    canvasItemSetNotifyLocalTransformMethodBind = getMethod(
        cstring"CanvasItem", cstring"set_notify_local_transform")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(enable)

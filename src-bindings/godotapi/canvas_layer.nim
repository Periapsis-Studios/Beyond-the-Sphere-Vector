...
proc customViewport(self: CanvasLayer): Node =
  ...
  canvasLayerGetCustomViewportMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc followViewportScale(self: CanvasLayer): float64 =
  if isNil(canvasLayerGetFollowViewportScaleMethodBind):
    canvasLayerGetFollowViewportScaleMethodBind = getMethod(
        cstring"CanvasLayer", cstring"get_follow_viewport_scale")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `followViewportScale=`(self: CanvasLayer; val: float64) =
  if isNil(canvasLayerSetFollowViewportScaleMethodBind):
    canvasLayerSetFollowViewportScaleMethodBind = getMethod(
        cstring"CanvasLayer", cstring"set_follow_viewport_scale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

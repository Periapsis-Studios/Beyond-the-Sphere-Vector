...
proc useTopLeft(self: CenterContainer): bool =
  if isNil(centerContainerIsUsingTopLeftMethodBind):
    centerContainerIsUsingTopLeftMethodBind = getMethod(
        cstring"CenterContainer", cstring"is_using_top_left")
...
proc `useTopLeft=`(self: CenterContainer; val: bool) =
  if isNil(centerContainerSetUseTopLeftMethodBind):
    centerContainerSetUseTopLeftMethodBind = getMethod(
        cstring"CenterContainer", cstring"set_use_top_left")
  var

...
proc character(self: CharFXTransform): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  charFXTransformGetCharacterMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc elapsedTime(self: CharFXTransform): float64 =
  if isNil(charFXTransformGetElapsedTimeMethodBind):
    charFXTransformGetElapsedTimeMethodBind = getMethod(
        cstring"CharFXTransform", cstring"get_elapsed_time")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `elapsedTime=`(self: CharFXTransform; val: float64) =
  if isNil(charFXTransformSetElapsedTimeMethodBind):
    charFXTransformSetElapsedTimeMethodBind = getMethod(
        cstring"CharFXTransform", cstring"set_elapsed_time")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc env(self: CharFXTransform): Dictionary =
  if isNil(charFXTransformGetEnvironmentMethodBind):
    charFXTransformGetEnvironmentMethodBind = getMethod(
        cstring"CharFXTransform", cstring"get_environment")
  var ptrCallRet: pointer
...
proc `env=`(self: CharFXTransform; val: Dictionary) =
  if isNil(charFXTransformSetEnvironmentMethodBind):
    charFXTransformSetEnvironmentMethodBind = getMethod(
        cstring"CharFXTransform", cstring"set_environment")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `offset=`(self: CharFXTransform; val: Vector2) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  charFXTransformSetOffsetMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `visible=`(self: CharFXTransform; val: bool) =
  if isNil(charFXTransformSetVisibilityMethodBind):
    charFXTransformSetVisibilityMethodBind = getMethod(
        cstring"CharFXTransform", cstring"set_visibility")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

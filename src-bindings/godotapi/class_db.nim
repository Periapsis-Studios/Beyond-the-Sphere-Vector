...
proc classGetEnumConstants*(class: string; enumm: string;
                            noInheritance: bool = false): PoolStringArray {.
    gcsafe, locks: 0.}
proc classGetEnumList*(class: string; noInheritance: bool = false): PoolStringArray {.
    gcsafe, locks: 0.}
proc classGetIntegerConstant*(class: string; name: string): int64 {.gcsafe,
    locks: 0.}
proc classGetIntegerConstantEnum*(class: string; name: string;
                                  noInheritance: bool = false): string {.gcsafe,
    locks: 0.}
...
proc classGetMethodList*(class: string; noInheritance: bool = false): Array {.
    gcsafe, locks: 0.}
proc classGetProperty*(objectt: Object; property: string): Variant {.gcsafe,
    locks: 0.}
proc classGetPropertyList*(class: string; noInheritance: bool = false): Array {.
    gcsafe, locks: 0.}
proc classGetSignal*(class: string; signal: string): Dictionary {.gcsafe,
    locks: 0.}
proc classGetSignalList*(class: string; noInheritance: bool = false): Array {.
    gcsafe, locks: 0.}
proc classHasEnum*(class: string; name: string; noInheritance: bool = false): bool {.
    gcsafe, locks: 0.}
proc classHasIntegerConstant*(class: string; name: string): bool {.gcsafe,
    locks: 0.}
...
var classDBClassGetEnumConstantsMethodBind {.threadvar.}: ptr GodotMethodBind
proc classGetEnumConstants(class: string; enumm: string;
                           noInheritance: bool = false): PoolStringArray =
  if isNil(singletonClassDB):
    singletonClassDB = getSingleton[ClassDB]()
  let self = singletonClassDB
  if isNil(classDBClassGetEnumConstantsMethodBind):
    classDBClassGetEnumConstantsMethodBind = getMethod(cstring"_ClassDB",
        cstring"class_get_enum_constants")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(class)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var argToPassToGodot1 = toGodotString(enumm)
  ...
  ptrCallRet = addr(ptrCallVal)
  classDBClassGetEnumConstantsMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
  result = newPoolStringArray(ptrCallVal)

var classDBClassGetEnumListMethodBind {.threadvar.}: ptr GodotMethodBind
proc classGetEnumList(class: string; noInheritance: bool = false): PoolStringArray =
  if isNil(singletonClassDB):
    singletonClassDB = getSingleton[ClassDB]()
  let self = singletonClassDB
  if isNil(classDBClassGetEnumListMethodBind):
    classDBClassGetEnumListMethodBind = getMethod(cstring"_ClassDB",
        cstring"class_get_enum_list")
  var
  ...
  initGodotPoolStringArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  classDBClassGetEnumListMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
  deinit(argToPassToGodot0)
  result = newPoolStringArray(ptrCallVal)
...
var classDBClassGetIntegerConstantEnumMethodBind {.threadvar.}: ptr GodotMethodBind
proc classGetIntegerConstantEnum(class: string; name: string;
                                 noInheritance: bool = false): string =
  if isNil(singletonClassDB):
    singletonClassDB = getSingleton[ClassDB]()
  let self = singletonClassDB
  if isNil(classDBClassGetIntegerConstantEnumMethodBind):
    classDBClassGetIntegerConstantEnumMethodBind = getMethod(cstring"_ClassDB",
        cstring"class_get_integer_constant_enum")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(class)
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  classDBClassGetIntegerConstantEnumMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
  result = $ptrCallVal
...
var classDBClassHasEnumMethodBind {.threadvar.}: ptr GodotMethodBind
proc classHasEnum(class: string; name: string; noInheritance: bool = false): bool =
  if isNil(singletonClassDB):
    singletonClassDB = getSingleton[ClassDB]()
  let self = singletonClassDB
  if isNil(classDBClassHasEnumMethodBind):
    classDBClassHasEnumMethodBind = getMethod(cstring"_ClassDB",
        cstring"class_has_enum")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(class)
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  classDBClassHasEnumMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                        ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)

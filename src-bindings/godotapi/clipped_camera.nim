...
proc getCollisionMaskBit*(self: ClippedCamera; bit: int64): bool {.gcsafe,
    locks: 0.}
...
proc processMode(self: ClippedCamera): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  clippedCameraGetProcessModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc clearExceptions(self: ClippedCamera) =
  ...
  var ptrCallRet: pointer
  clippedCameraClearExceptionsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc removeExceptionRid(self: ClippedCamera; rid: RID) =
  if isNil(clippedCameraRemoveExceptionRidMethodBind):
    clippedCameraRemoveExceptionRidMethodBind = getMethod(
        cstring"ClippedCamera", cstring"remove_exception_rid")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(rid)

...
proc margin*(self: CollisionPolygon): float64 {.gcsafe, locks: 0.}
proc `margin=`*(self: CollisionPolygon; val: float64) {.gcsafe, locks: 0.}
proc polygon*(self: CollisionPolygon): PoolVector2Array {.gcsafe, locks: 0.}
proc `polygon=`*(self: CollisionPolygon; val: PoolVector2Array) {.gcsafe,
    locks: 0.}
method isEditable3dPolygon*(self: CollisionPolygon): bool {.gcsafe, locks: 0,
    base.}
...
proc `depth=`(self: CollisionPolygon; val: float64) =
  ...
  var ptrCallRet: pointer
  collisionPolygonSetDepthMethodBind.ptrCall(self.godotObject,
...
proc `disabled=`(self: CollisionPolygon; val: bool) =
  if isNil(collisionPolygonSetDisabledMethodBind):
    collisionPolygonSetDisabledMethodBind = getMethod(
        cstring"CollisionPolygon", cstring"set_disabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
var collisionPolygonGetMarginMethodBind {.threadvar.}: ptr GodotMethodBind
proc margin(self: CollisionPolygon): float64 =
  if isNil(collisionPolygonGetMarginMethodBind):
    collisionPolygonGetMarginMethodBind = getMethod(cstring"CollisionPolygon",
        cstring"get_margin")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  collisionPolygonGetMarginMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var collisionPolygonSetMarginMethodBind {.threadvar.}: ptr GodotMethodBind
proc `margin=`(self: CollisionPolygon; val: float64) =
  if isNil(collisionPolygonSetMarginMethodBind):
    collisionPolygonSetMarginMethodBind = getMethod(cstring"CollisionPolygon",
        cstring"set_margin")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  collisionPolygonSetMarginMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

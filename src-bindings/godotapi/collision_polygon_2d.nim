...
proc oneWayCollisionMargin*(self: CollisionPolygon2D): float64 {.gcsafe,
    locks: 0.}
...
proc `polygon=`*(self: CollisionPolygon2D; val: PoolVector2Array) {.gcsafe,
    locks: 0.}
...
proc disabled(self: CollisionPolygon2D): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  collisionPolygon2DIsDisabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc polygon(self: CollisionPolygon2D): PoolVector2Array =
  ...
  collisionPolygon2DGetPolygonMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

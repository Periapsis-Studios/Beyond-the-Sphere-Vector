...
proc resourceChanged*(self: CollisionShape; resource: Resource) {.gcsafe,
    locks: 0.}
...
proc resourceChanged(self: CollisionShape; resource: Resource) =
  if isNil(collisionShapeResourceChangedMethodBind):
    collisionShapeResourceChangedMethodBind = getMethod(
        cstring"CollisionShape", cstring"resource_changed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

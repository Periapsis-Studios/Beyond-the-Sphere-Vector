...
proc `disabled=`(self: CollisionShape2D; val: bool) =
  if isNil(collisionShape2DSetDisabledMethodBind):
    collisionShape2DSetDisabledMethodBind = getMethod(
        cstring"CollisionShape2D", cstring"set_disabled")
  var
...
proc `shape=`(self: CollisionShape2D; val: Shape2D) =
  ...
  var ptrCallRet: pointer
  collisionShape2DSetShapeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
method shapeChanged(self: CollisionShape2D) =
  ...
  var ptrCallRet: pointer
  collisionShape2DUnderscoreshapeChangedMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

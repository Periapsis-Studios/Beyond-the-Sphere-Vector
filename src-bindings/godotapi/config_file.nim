...
proc clear*(self: ConfigFile) {.gcsafe, locks: 0.}
proc eraseSection*(self: ConfigFile; section: string) {.gcsafe, locks: 0.}
proc eraseSectionKey*(self: ConfigFile; section: string; key: string) {.gcsafe,
    locks: 0.}
...
proc setValue*(self: ConfigFile; section: string; key: string; value: Variant) {.
    gcsafe, locks: 0.}
var configFileClearMethodBind {.threadvar.}: ptr GodotMethodBind
proc clear(self: ConfigFile) =
  if isNil(configFileClearMethodBind):
    configFileClearMethodBind = getMethod(cstring"ConfigFile", cstring"clear")
  var ptrCallRet: pointer
  configFileClearMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc getSectionKeys(self: ConfigFile; section: string): PoolStringArray =
  ...
  configFileGetSectionKeysMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc load(self: ConfigFile; path: string): Error =
  ...
  ptrCallRet = addr(ptrCallVal)
  configFileLoadMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
...
proc save(self: ConfigFile; path: string): Error =
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  configFileSaveMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  result = Error(ptrCallVal)

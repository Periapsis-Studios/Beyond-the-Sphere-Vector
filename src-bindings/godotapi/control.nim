...
proc inputPassOnModalCloseClick*(self: Control): bool {.gcsafe, locks: 0.}
proc `inputPassOnModalCloseClick=`*(self: Control; val: bool) {.gcsafe, locks: 0.}
...
method makeCustomTooltip*(self: Control; forText: string): Control {.gcsafe,
    locks: 0, base.}
...
proc addColorOverride*(self: Control; name: string; color: Color) {.gcsafe,
    locks: 0.}
proc addConstantOverride*(self: Control; name: string; constant: int64) {.
    gcsafe, locks: 0.}
proc addFontOverride*(self: Control; name: string; font: Font) {.gcsafe,
    locks: 0.}
proc addIconOverride*(self: Control; name: string; texture: Texture) {.gcsafe,
    locks: 0.}
proc addShaderOverride*(self: Control; name: string; shader: Shader) {.gcsafe,
    locks: 0.}
...
method canDropData*(self: Control; position: Vector2; data: Variant): bool {.
    gcsafe, locks: 0, base.}
method dropData*(self: Control; position: Vector2; data: Variant) {.gcsafe,
    locks: 0, base.}
proc findNextValidFocus*(self: Control): Control {.gcsafe, locks: 0.}
proc findPrevValidFocus*(self: Control): Control {.gcsafe, locks: 0.}
proc forceDrag*(self: Control; data: Variant; preview: Control) {.gcsafe,
    locks: 0.}
proc getBegin*(self: Control): Vector2 {.gcsafe, locks: 0.}
proc getColor*(self: Control; name: string; themeType: string = ""): Color {.
    gcsafe, locks: 0.}
proc getCombinedMinimumSize*(self: Control): Vector2 {.gcsafe, locks: 0.}
proc getConstant*(self: Control; name: string; themeType: string = ""): int64 {.
    gcsafe, locks: 0.}
proc getCursorShape*(self: Control; position: Vector2 = vec2(0.0, 0.0)): int64 {.
    gcsafe, locks: 0.}
method getDragData*(self: Control; position: Vector2): Variant {.gcsafe,
    locks: 0, base.}
proc getEnd*(self: Control): Vector2 {.gcsafe, locks: 0.}
proc getFocusOwner*(self: Control): Control {.gcsafe, locks: 0.}
proc getFont*(self: Control; name: string; themeType: string = ""): Font {.
    gcsafe, locks: 0.}
proc getGlobalRect*(self: Control): Rect2 {.gcsafe, locks: 0.}
proc getIcon*(self: Control; name: string; themeType: string = ""): Texture {.
    gcsafe, locks: 0.}
...
proc getStylebox*(self: Control; name: string; themeType: string = ""): StyleBox {.
    gcsafe, locks: 0.}
proc getThemeDefaultFont*(self: Control): Font {.gcsafe, locks: 0.}
...
proc hasStylebox*(self: Control; name: string; themeType: string = ""): bool {.
    gcsafe, locks: 0.}
...
proc setMarginsPreset*(self: Control; preset: int64; resizeMode: int64 = 0'i64;
                       margin: int64 = 0'i64) {.gcsafe, locks: 0.}
proc setPositionImpl*(self: Control; position: Vector2;
                      keepMargins: bool = false) {.gcsafe, locks: 0.}
...
proc anchorBottom(self: Control): float64 =
  if isNil(controlGetAnchorMethodBind):
    controlGetAnchorMethodBind = getMethod(cstring"Control",
        cstring"get_anchor")
  var
...
proc focusNeighbourBottom(self: Control): NodePath =
  ...
  controlGetFocusNeighbourMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `focusNeighbourBottom=`(self: Control; val: NodePath) =
  ...
  var ptrCallRet: pointer
  controlSetFocusNeighbourMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `growHorizontal=`(self: Control; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  controlSetHGrowDirectionMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `growVertical=`(self: Control; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  controlSetVGrowDirectionMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc hintTooltip(self: Control): string =
  ...
  ptrCallRet = addr(ptrCallVal)
  controlUnderscoregetTooltipMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc `hintTooltip=`(self: Control; val: string) =
  if isNil(controlSetTooltipMethodBind):
    controlSetTooltipMethodBind = getMethod(cstring"Control",
        cstring"set_tooltip")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var controlGetPassOnModalCloseClickMethodBind {.threadvar.}: ptr GodotMethodBind
proc inputPassOnModalCloseClick(self: Control): bool =
  if isNil(controlGetPassOnModalCloseClickMethodBind):
    controlGetPassOnModalCloseClickMethodBind = getMethod(cstring"Control",
        cstring"get_pass_on_modal_close_click")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlGetPassOnModalCloseClickMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var controlSetPassOnModalCloseClickMethodBind {.threadvar.}: ptr GodotMethodBind
proc `inputPassOnModalCloseClick=`(self: Control; val: bool) =
  if isNil(controlSetPassOnModalCloseClickMethodBind):
    controlSetPassOnModalCloseClickMethodBind = getMethod(cstring"Control",
        cstring"set_pass_on_modal_close_click")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  controlSetPassOnModalCloseClickMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc marginBottom(self: Control): float64 =
  if isNil(controlGetMarginMethodBind):
    controlGetMarginMethodBind = getMethod(cstring"Control",
        cstring"get_margin")
  var
    godotBoundArg0 = 3'i64
    argsStatic: array[1, pointer]
...
proc `marginBottom=`(self: Control; val: float64) =
  if isNil(controlSetMarginMethodBind):
    controlSetMarginMethodBind = getMethod(cstring"Control",
        cstring"set_margin")
  var
    godotBoundArg0 = 3'i64
    argsStatic: array[2, pointer]
...
proc mouseDefaultCursorShape(self: Control): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlGetDefaultCursorShapeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc rectMinSize(self: Control): Vector2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlGetCustomMinimumSizeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `rectSize=`(self: Control; val: Vector2) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  controlUnderscoresetSizeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
method clipsInput(self: Control): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlUnderscoreclipsInputMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method makeCustomTooltip(self: Control; forText: string): Control =
...
method sizeChanged(self: Control) =
  ...
  var ptrCallRet: pointer
  controlUnderscoresizeChangedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc addShaderOverride(self: Control; name: string; shader: Shader) =
  ...
  argsToPassToGodot[][1] = argToPassToGodot1
  var ptrCallRet: pointer
  controlAddShaderOverrideMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
var controlFindNextValidFocusMethodBind {.threadvar.}: ptr GodotMethodBind
proc findNextValidFocus(self: Control): Control =
  if isNil(controlFindNextValidFocusMethodBind):
    controlFindNextValidFocusMethodBind = getMethod(cstring"Control",
        cstring"find_next_valid_focus")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  controlFindNextValidFocusMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)

var controlFindPrevValidFocusMethodBind {.threadvar.}: ptr GodotMethodBind
proc findPrevValidFocus(self: Control): Control =
  if isNil(controlFindPrevValidFocusMethodBind):
    controlFindPrevValidFocusMethodBind = getMethod(cstring"Control",
        cstring"find_prev_valid_focus")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  controlFindPrevValidFocusMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc forceDrag(self: Control; data: Variant; preview: Control) =
  if isNil(controlForceDragMethodBind):
    controlForceDragMethodBind = getMethod(cstring"Control",
        cstring"force_drag")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getColor(self: Control; name: string; themeType: string = ""): Color =
  ...
  var argToPassToGodot0 = toGodotString(name)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var argToPassToGodot1 = toGodotString(themeType)
...
proc getFont(self: Control; name: string; themeType: string = ""): Font =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  controlGetFontMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc getIcon(self: Control; name: string; themeType: string = ""): Texture =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  controlGetIconMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
var controlGetThemeDefaultFontMethodBind {.threadvar.}: ptr GodotMethodBind
proc getThemeDefaultFont(self: Control): Font =
  if isNil(controlGetThemeDefaultFontMethodBind):
    controlGetThemeDefaultFontMethodBind = getMethod(cstring"Control",
        cstring"get_theme_default_font")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  controlGetThemeDefaultFontMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)

var controlGetTooltipMethodBind {.threadvar.}: ptr GodotMethodBind
proc getTooltipImpl(self: Control; atPosition: Vector2 = vec2(0.0, 0.0)): string =
  if isNil(controlGetTooltipMethodBind):
    controlGetTooltipMethodBind = getMethod(cstring"Control",
        cstring"get_tooltip")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc grabFocus(self: Control) =
  if isNil(controlGrabFocusMethodBind):
    controlGrabFocusMethodBind = getMethod(cstring"Control",
        cstring"grab_focus")
  var ptrCallRet: pointer
...
proc hasFont(self: Control; name: string; themeType: string = ""): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlHasFontMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
...
proc hasIcon(self: Control; name: string; themeType: string = ""): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlHasIconMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
...
proc hasShaderOverride(self: Control; name: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  controlHasShaderOverrideMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc setAnchorImpl(self: Control; margin: int64; anchor: float64;
  ...
  if isNil(controlSetAnchorMethodBind):
    controlSetAnchorMethodBind = getMethod(cstring"Control",
        cstring"set_anchor")
  var
...
proc setDragForwarding(self: Control; target: Control) =
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  controlSetDragForwardingMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setEnd(self: Control; position: Vector2) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(position)
  var ptrCallRet: pointer
  controlSetEndMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc setGlobalPositionImpl(self: Control; position: Vector2;
  ...
  argsToPassToGodot[][1] = unsafeAddr(keepMargins)
  var ptrCallRet: pointer
  controlSetGlobalPositionMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setSizeImpl(self: Control; size: Vector2; keepMargins: bool = false) =
  ...
  argsToPassToGodot[][1] = unsafeAddr(keepMargins)
  var ptrCallRet: pointer
  controlSetSizeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc showModal(self: Control; exclusive: bool = false) =
  if isNil(controlShowModalMethodBind):
    controlShowModalMethodBind = getMethod(cstring"Control",
        cstring"show_modal")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc warpMouse(self: Control; toPosition: Vector2) =
  if isNil(controlWarpMouseMethodBind):
    controlWarpMouseMethodBind = getMethod(cstring"Control",
        cstring"warp_mouse")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

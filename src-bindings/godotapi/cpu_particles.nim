...
proc `angularVelocityRandom=`*(self: CPUParticles; val: float64) {.gcsafe,
    locks: 0.}
...
proc `emissionColors=`*(self: CPUParticles; val: PoolColorArray) {.gcsafe,
    locks: 0.}
proc emissionNormals*(self: CPUParticles): PoolVector3Array {.gcsafe, locks: 0.}
proc `emissionNormals=`*(self: CPUParticles; val: PoolVector3Array) {.gcsafe,
    locks: 0.}
proc emissionPoints*(self: CPUParticles): PoolVector3Array {.gcsafe, locks: 0.}
proc `emissionPoints=`*(self: CPUParticles; val: PoolVector3Array) {.gcsafe,
    locks: 0.}
proc emissionRingAxis*(self: CPUParticles): Vector3 {.gcsafe, locks: 0.}
proc `emissionRingAxis=`*(self: CPUParticles; val: Vector3) {.gcsafe, locks: 0.}
proc emissionRingHeight*(self: CPUParticles): float64 {.gcsafe, locks: 0.}
proc `emissionRingHeight=`*(self: CPUParticles; val: float64) {.gcsafe, locks: 0.}
proc emissionRingInnerRadius*(self: CPUParticles): float64 {.gcsafe, locks: 0.}
proc `emissionRingInnerRadius=`*(self: CPUParticles; val: float64) {.gcsafe,
    locks: 0.}
proc emissionRingRadius*(self: CPUParticles): float64 {.gcsafe, locks: 0.}
proc `emissionRingRadius=`*(self: CPUParticles; val: float64) {.gcsafe, locks: 0.}
...
proc `emissionSphereRadius=`*(self: CPUParticles; val: float64) {.gcsafe,
    locks: 0.}
...
proc `initialVelocityRandom=`*(self: CPUParticles; val: float64) {.gcsafe,
    locks: 0.}
...
proc `orbitVelocityRandom=`*(self: CPUParticles; val: float64) {.gcsafe,
    locks: 0.}
...
proc `tangentialAccelRandom=`*(self: CPUParticles; val: float64) {.gcsafe,
    locks: 0.}
method updateRenderThread*(self: CPUParticles) {.gcsafe, locks: 0, base.}
proc convertFromParticles*(self: CPUParticles; particles: Node) {.gcsafe,
    locks: 0.}
...
proc `colorRamp=`(self: CPUParticles; val: Gradient) =
  ...
  var ptrCallRet: pointer
  cPUParticlesSetColorRampMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `direction=`(self: CPUParticles; val: Vector3) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cPUParticlesSetDirectionMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `drawOrder=`(self: CPUParticles; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cPUParticlesSetDrawOrderMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc emissionColors(self: CPUParticles): PoolColorArray =
  ...
  initGodotPoolColorArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
...
var cPUParticlesGetEmissionRingAxisMethodBind {.threadvar.}: ptr GodotMethodBind
proc emissionRingAxis(self: CPUParticles): Vector3 =
  if isNil(cPUParticlesGetEmissionRingAxisMethodBind):
    cPUParticlesGetEmissionRingAxisMethodBind = getMethod(
        cstring"CPUParticles", cstring"get_emission_ring_axis")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cPUParticlesGetEmissionRingAxisMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var cPUParticlesSetEmissionRingAxisMethodBind {.threadvar.}: ptr GodotMethodBind
proc `emissionRingAxis=`(self: CPUParticles; val: Vector3) =
  if isNil(cPUParticlesSetEmissionRingAxisMethodBind):
    cPUParticlesSetEmissionRingAxisMethodBind = getMethod(
        cstring"CPUParticles", cstring"set_emission_ring_axis")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cPUParticlesSetEmissionRingAxisMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var cPUParticlesGetEmissionRingHeightMethodBind {.threadvar.}: ptr GodotMethodBind
proc emissionRingHeight(self: CPUParticles): float64 =
  if isNil(cPUParticlesGetEmissionRingHeightMethodBind):
    cPUParticlesGetEmissionRingHeightMethodBind = getMethod(
        cstring"CPUParticles", cstring"get_emission_ring_height")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cPUParticlesGetEmissionRingHeightMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var cPUParticlesSetEmissionRingHeightMethodBind {.threadvar.}: ptr GodotMethodBind
proc `emissionRingHeight=`(self: CPUParticles; val: float64) =
  if isNil(cPUParticlesSetEmissionRingHeightMethodBind):
    cPUParticlesSetEmissionRingHeightMethodBind = getMethod(
        cstring"CPUParticles", cstring"set_emission_ring_height")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cPUParticlesSetEmissionRingHeightMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var cPUParticlesGetEmissionRingInnerRadiusMethodBind {.threadvar.}: ptr GodotMethodBind
proc emissionRingInnerRadius(self: CPUParticles): float64 =
  if isNil(cPUParticlesGetEmissionRingInnerRadiusMethodBind):
    cPUParticlesGetEmissionRingInnerRadiusMethodBind = getMethod(
        cstring"CPUParticles", cstring"get_emission_ring_inner_radius")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cPUParticlesGetEmissionRingInnerRadiusMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

var cPUParticlesSetEmissionRingInnerRadiusMethodBind {.threadvar.}: ptr GodotMethodBind
proc `emissionRingInnerRadius=`(self: CPUParticles; val: float64) =
  if isNil(cPUParticlesSetEmissionRingInnerRadiusMethodBind):
    cPUParticlesSetEmissionRingInnerRadiusMethodBind = getMethod(
        cstring"CPUParticles", cstring"set_emission_ring_inner_radius")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cPUParticlesSetEmissionRingInnerRadiusMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var cPUParticlesGetEmissionRingRadiusMethodBind {.threadvar.}: ptr GodotMethodBind
proc emissionRingRadius(self: CPUParticles): float64 =
  if isNil(cPUParticlesGetEmissionRingRadiusMethodBind):
    cPUParticlesGetEmissionRingRadiusMethodBind = getMethod(
        cstring"CPUParticles", cstring"get_emission_ring_radius")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cPUParticlesGetEmissionRingRadiusMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var cPUParticlesSetEmissionRingRadiusMethodBind {.threadvar.}: ptr GodotMethodBind
proc `emissionRingRadius=`(self: CPUParticles; val: float64) =
  if isNil(cPUParticlesSetEmissionRingRadiusMethodBind):
    cPUParticlesSetEmissionRingRadiusMethodBind = getMethod(
        cstring"CPUParticles", cstring"set_emission_ring_radius")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cPUParticlesSetEmissionRingRadiusMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc emissionShape(self: CPUParticles): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cPUParticlesGetEmissionShapeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc convertFromParticles(self: CPUParticles; particles: Node) =
  if isNil(cPUParticlesConvertFromParticlesMethodBind):
    cPUParticlesConvertFromParticlesMethodBind = getMethod(
        cstring"CPUParticles", cstring"convert_from_particles")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

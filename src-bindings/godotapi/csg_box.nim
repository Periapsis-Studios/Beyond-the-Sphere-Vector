...
proc `depth=`(self: CSGBox; val: float64) =
  ...
  var ptrCallRet: pointer
  cSGBoxSetDepthMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc material(self: CSGBox): Material =
  if isNil(cSGBoxGetMaterialMethodBind):
    cSGBoxGetMaterialMethodBind = getMethod(cstring"CSGBox",
...
proc `material=`(self: CSGBox; val: Material) =
  if isNil(cSGBoxSetMaterialMethodBind):
    cSGBoxSetMaterialMethodBind = getMethod(cstring"CSGBox",
...
proc `width=`(self: CSGBox; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGBoxSetWidthMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

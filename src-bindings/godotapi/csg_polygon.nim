...
const
  ...
  MODE_PATH* = 2'i64
  MODE_SPIN* = 1'i64
  PATH_INTERVAL_DISTANCE* = 0'i64
  PATH_INTERVAL_SUBDIVIDE* = 1'i64
...
proc pathIntervalType*(self: CSGPolygon): int64 {.gcsafe, locks: 0.}
proc `pathIntervalType=`*(self: CSGPolygon; val: int64) {.gcsafe, locks: 0.}
...
proc pathSimplifyAngle*(self: CSGPolygon): float64 {.gcsafe, locks: 0.}
proc `pathSimplifyAngle=`*(self: CSGPolygon; val: float64) {.gcsafe, locks: 0.}
proc pathUDistance*(self: CSGPolygon): float64 {.gcsafe, locks: 0.}
proc `pathUDistance=`*(self: CSGPolygon; val: float64) {.gcsafe, locks: 0.}
...
method hasEditable3dPolygonNoDepth*(self: CSGPolygon): bool {.gcsafe, locks: 0,
    base.}
...
proc mode(self: CSGPolygon): int64 =
  if isNil(cSGPolygonGetModeMethodBind):
    cSGPolygonGetModeMethodBind = getMethod(cstring"CSGPolygon",
        cstring"get_mode")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `mode=`(self: CSGPolygon; val: int64) =
  if isNil(cSGPolygonSetModeMethodBind):
    cSGPolygonSetModeMethodBind = getMethod(cstring"CSGPolygon",
        cstring"set_mode")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc pathContinuousU(self: CSGPolygon): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cSGPolygonIsPathContinuousUMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var cSGPolygonGetPathIntervalTypeMethodBind {.threadvar.}: ptr GodotMethodBind
proc pathIntervalType(self: CSGPolygon): int64 =
  if isNil(cSGPolygonGetPathIntervalTypeMethodBind):
    cSGPolygonGetPathIntervalTypeMethodBind = getMethod(cstring"CSGPolygon",
        cstring"get_path_interval_type")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cSGPolygonGetPathIntervalTypeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var cSGPolygonSetPathIntervalTypeMethodBind {.threadvar.}: ptr GodotMethodBind
proc `pathIntervalType=`(self: CSGPolygon; val: int64) =
  if isNil(cSGPolygonSetPathIntervalTypeMethodBind):
    cSGPolygonSetPathIntervalTypeMethodBind = getMethod(cstring"CSGPolygon",
        cstring"set_path_interval_type")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGPolygonSetPathIntervalTypeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var cSGPolygonGetPathSimplifyAngleMethodBind {.threadvar.}: ptr GodotMethodBind
proc pathSimplifyAngle(self: CSGPolygon): float64 =
  if isNil(cSGPolygonGetPathSimplifyAngleMethodBind):
    cSGPolygonGetPathSimplifyAngleMethodBind = getMethod(cstring"CSGPolygon",
        cstring"get_path_simplify_angle")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cSGPolygonGetPathSimplifyAngleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var cSGPolygonSetPathSimplifyAngleMethodBind {.threadvar.}: ptr GodotMethodBind
proc `pathSimplifyAngle=`(self: CSGPolygon; val: float64) =
  if isNil(cSGPolygonSetPathSimplifyAngleMethodBind):
    cSGPolygonSetPathSimplifyAngleMethodBind = getMethod(cstring"CSGPolygon",
        cstring"set_path_simplify_angle")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGPolygonSetPathSimplifyAngleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var cSGPolygonGetPathUDistanceMethodBind {.threadvar.}: ptr GodotMethodBind
proc pathUDistance(self: CSGPolygon): float64 =
  if isNil(cSGPolygonGetPathUDistanceMethodBind):
    cSGPolygonGetPathUDistanceMethodBind = getMethod(cstring"CSGPolygon",
        cstring"get_path_u_distance")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  cSGPolygonGetPathUDistanceMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var cSGPolygonSetPathUDistanceMethodBind {.threadvar.}: ptr GodotMethodBind
proc `pathUDistance=`(self: CSGPolygon; val: float64) =
  if isNil(cSGPolygonSetPathUDistanceMethodBind):
    cSGPolygonSetPathUDistanceMethodBind = getMethod(cstring"CSGPolygon",
        cstring"set_path_u_distance")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGPolygonSetPathUDistanceMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `smoothFaces=`(self: CSGPolygon; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGPolygonSetSmoothFacesMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `spinDegrees=`(self: CSGPolygon; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGPolygonSetSpinDegreesMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

...
proc setCollisionLayerBit*(self: CSGShape; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
proc setCollisionMaskBit*(self: CSGShape; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
...
proc `collisionMask=`(self: CSGShape; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  cSGShapeSetCollisionMaskMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getMeshes(self: CSGShape): Array =
  if isNil(cSGShapeGetMeshesMethodBind):
    cSGShapeGetMeshesMethodBind = getMethod(cstring"CSGShape",
        cstring"get_meshes")
  var ptrCallRet: pointer

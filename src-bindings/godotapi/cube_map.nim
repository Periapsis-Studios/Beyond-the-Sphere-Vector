...
proc storageMode(self: CubeMap): int64 =
  if isNil(cubeMapGetStorageMethodBind):
    cubeMapGetStorageMethodBind = getMethod(cstring"CubeMap",
        cstring"get_storage")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `storageMode=`(self: CubeMap; val: int64) =
  if isNil(cubeMapSetStorageMethodBind):
    cubeMapSetStorageMethodBind = getMethod(cstring"CubeMap",
        cstring"set_storage")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getHeight(self: CubeMap): int64 =
  if isNil(cubeMapGetHeightMethodBind):
    cubeMapGetHeightMethodBind = getMethod(cstring"CubeMap",
        cstring"get_height")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getSide(self: CubeMap; side: int64): Image =
  ...
  cubeMapGetSideMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc setSide(self: CubeMap; side: int64; image: Image) =
  ...
  var ptrCallRet: pointer
  cubeMapSetSideMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

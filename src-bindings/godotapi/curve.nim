...
proc getPointRightTangent*(self: Curve; index: int64): float64 {.gcsafe,
    locks: 0.}
...
proc setPointLeftMode*(self: Curve; index: int64; mode: int64) {.gcsafe,
    locks: 0.}
...
proc setPointOffset*(self: Curve; index: int64; offset: float64): int64 {.
    gcsafe, locks: 0.}
proc setPointRightMode*(self: Curve; index: int64; mode: int64) {.gcsafe,
    locks: 0.}
...
proc maxValue(self: Curve): float64 =
  if isNil(curveGetMaxValueMethodBind):
    curveGetMaxValueMethodBind = getMethod(cstring"Curve",
        cstring"get_max_value")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `maxValue=`(self: Curve; val: float64) =
  if isNil(curveSetMaxValueMethodBind):
    curveSetMaxValueMethodBind = getMethod(cstring"Curve",
        cstring"set_max_value")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc minValue(self: Curve): float64 =
  if isNil(curveGetMinValueMethodBind):
    curveGetMinValueMethodBind = getMethod(cstring"Curve",
        cstring"get_min_value")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `minValue=`(self: Curve; val: float64) =
  if isNil(curveSetMinValueMethodBind):
    curveSetMinValueMethodBind = getMethod(cstring"Curve",
        cstring"set_min_value")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc addPoint(self: Curve; position: Vector2; leftTangent: float64 = 0.0;
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  curveAddPointMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc clearPoints(self: Curve) =
  if isNil(curveClearPointsMethodBind):
    curveClearPointsMethodBind = getMethod(cstring"Curve",
...
proc getPointLeftTangent(self: Curve; index: int64): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  curveGetPointLeftTangentMethodBind.ptrCall(self.godotObject,
...
proc removePoint(self: Curve; index: int64) =
  if isNil(curveRemovePointMethodBind):
    curveRemovePointMethodBind = getMethod(cstring"Curve",
...
proc setPointLeftTangent(self: Curve; index: int64; tangent: float64) =
  ...
  var ptrCallRet: pointer
  curveSetPointLeftTangentMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

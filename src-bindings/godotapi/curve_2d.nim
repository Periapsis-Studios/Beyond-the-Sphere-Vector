...
proc getClosestOffset*(self: Curve2D; toPoint: Vector2): float64 {.gcsafe,
    locks: 0.}
proc getClosestPoint*(self: Curve2D; toPoint: Vector2): Vector2 {.gcsafe,
    locks: 0.}
...
proc interpolate*(self: Curve2D; idx: int64; t: float64): Vector2 {.gcsafe,
    locks: 0.}
...
proc setPointIn*(self: Curve2D; idx: int64; position: Vector2) {.gcsafe,
    locks: 0.}
proc setPointOut*(self: Curve2D; idx: int64; position: Vector2) {.gcsafe,
    locks: 0.}
proc setPointPosition*(self: Curve2D; idx: int64; position: Vector2) {.gcsafe,
    locks: 0.}
proc tessellate*(self: Curve2D; maxStages: int64 = 5'i64;
                 toleranceDegrees: float64 = 4.0): PoolVector2Array {.gcsafe,
    locks: 0.}
...
proc `data=`(self: Curve2D; val: Dictionary) =
  ...
  var ptrCallRet: pointer
  curve2DUnderscoresetDataMethodBind.ptrCall(self.godotObject,
...
proc tessellate(self: Curve2D; maxStages: int64 = 5'i64;
  ...
  if isNil(curve2DTessellateMethodBind):
    curve2DTessellateMethodBind = getMethod(cstring"Curve2D",
        cstring"tessellate")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
proc addPoint*(self: Curve3D; position: Vector3;
               inn: Vector3 = vec3(0.0, 0.0, 0.0);
...
proc getClosestOffset*(self: Curve3D; toPoint: Vector3): float64 {.gcsafe,
    locks: 0.}
proc getClosestPoint*(self: Curve3D; toPoint: Vector3): Vector3 {.gcsafe,
    locks: 0.}
...
proc interpolate*(self: Curve3D; idx: int64; t: float64): Vector3 {.gcsafe,
    locks: 0.}
...
proc interpolateBakedUpVector*(self: Curve3D; offset: float64;
                               applyTilt: bool = false): Vector3 {.gcsafe,
    locks: 0.}
proc interpolatef*(self: Curve3D; fofs: float64): Vector3 {.gcsafe, locks: 0.}
proc removePoint*(self: Curve3D; idx: int64) {.gcsafe, locks: 0.}
proc setPointIn*(self: Curve3D; idx: int64; position: Vector3) {.gcsafe,
    locks: 0.}
proc setPointOut*(self: Curve3D; idx: int64; position: Vector3) {.gcsafe,
    locks: 0.}
proc setPointPosition*(self: Curve3D; idx: int64; position: Vector3) {.gcsafe,
    locks: 0.}
proc setPointTilt*(self: Curve3D; idx: int64; tilt: float64) {.gcsafe, locks: 0.}
proc tessellate*(self: Curve3D; maxStages: int64 = 5'i64;
                 toleranceDegrees: float64 = 4.0): PoolVector3Array {.gcsafe,
    locks: 0.}
...
proc `data=`(self: Curve3D; val: Dictionary) =
  ...
  var ptrCallRet: pointer
  curve3DUnderscoresetDataMethodBind.ptrCall(self.godotObject,
...
proc tessellate(self: Curve3D; maxStages: int64 = 5'i64;
  ...
  if isNil(curve3DTessellateMethodBind):
    curve3DTessellateMethodBind = getMethod(cstring"Curve3D",
        cstring"tessellate")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

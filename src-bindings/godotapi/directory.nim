...
proc rename*(self: Directory; fromm: string; to: string): Error {.gcsafe,
    locks: 0.}
...
proc copy(self: Directory; fromm: string; to: string): Error =
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  directoryCopyMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc getNext(self: Directory): string =
  if isNil(directoryGetNextMethodBind):
    directoryGetNextMethodBind = getMethod(cstring"_Directory",
        cstring"get_next")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
...
proc makeDir(self: Directory; path: string): Error =
  if isNil(directoryMakeDirMethodBind):
    directoryMakeDirMethodBind = getMethod(cstring"_Directory",
        cstring"make_dir")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc open(self: Directory; path: string): Error =
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  directoryOpenMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
  result = Error(ptrCallVal)

...
proc getAvailableChars*(self: DynamicFont): string {.gcsafe, locks: 0.}
proc getFallback*(self: DynamicFont; idx: int64): DynamicFontData {.gcsafe,
    locks: 0.}
...
proc `useMipmaps=`(self: DynamicFont; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  dynamicFontSetUseMipmapsMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var dynamicFontGetAvailableCharsMethodBind {.threadvar.}: ptr GodotMethodBind
proc getAvailableChars(self: DynamicFont): string =
  if isNil(dynamicFontGetAvailableCharsMethodBind):
    dynamicFontGetAvailableCharsMethodBind = getMethod(cstring"DynamicFont",
        cstring"get_available_chars")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  dynamicFontGetAvailableCharsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc getFallbackCount(self: DynamicFont): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  dynamicFontGetFallbackCountMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
proc antialiased(self: DynamicFontData): bool =
  if isNil(dynamicFontDataIsAntialiasedMethodBind):
    dynamicFontDataIsAntialiasedMethodBind = getMethod(
        cstring"DynamicFontData", cstring"is_antialiased")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  dynamicFontDataIsAntialiasedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `antialiased=`(self: DynamicFontData; val: bool) =
  if isNil(dynamicFontDataSetAntialiasedMethodBind):
    dynamicFontDataSetAntialiasedMethodBind = getMethod(
        cstring"DynamicFontData", cstring"set_antialiased")
  var

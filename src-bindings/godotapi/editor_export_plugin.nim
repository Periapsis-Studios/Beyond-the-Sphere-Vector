...
method exportBegin*(self: EditorExportPlugin; features: PoolStringArray;
                    isDebug: bool; path: string; flags: int64) {.gcsafe,
    locks: 0, base.}
...
proc addFile*(self: EditorExportPlugin; path: string; file: PoolByteArray;
              remap: bool) {.gcsafe, locks: 0.}
proc addIosBundleFile*(self: EditorExportPlugin; path: string) {.gcsafe,
    locks: 0.}
proc addIosCppCode*(self: EditorExportPlugin; code: string) {.gcsafe, locks: 0.}
proc addIosEmbeddedFramework*(self: EditorExportPlugin; path: string) {.gcsafe,
    locks: 0.}
proc addIosFramework*(self: EditorExportPlugin; path: string) {.gcsafe, locks: 0.}
proc addIosLinkerFlags*(self: EditorExportPlugin; flags: string) {.gcsafe,
    locks: 0.}
...
proc addIosProjectStaticLib*(self: EditorExportPlugin; path: string) {.gcsafe,
    locks: 0.}
proc addSharedObject*(self: EditorExportPlugin; path: string;
                      tags: PoolStringArray) {.gcsafe, locks: 0.}
...
proc addFile(self: EditorExportPlugin; path: string; file: PoolByteArray;
  ...
  if isNil(editorExportPluginAddFileMethodBind):
    editorExportPluginAddFileMethodBind = getMethod(
        cstring"EditorExportPlugin", cstring"add_file")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(path)
...
var editorExportPluginAddIosEmbeddedFrameworkMethodBind {.threadvar.}: ptr GodotMethodBind
proc addIosEmbeddedFramework(self: EditorExportPlugin; path: string) =
  if isNil(editorExportPluginAddIosEmbeddedFrameworkMethodBind):
    editorExportPluginAddIosEmbeddedFrameworkMethodBind = getMethod(
        cstring"EditorExportPlugin", cstring"add_ios_embedded_framework")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(path)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  editorExportPluginAddIosEmbeddedFrameworkMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

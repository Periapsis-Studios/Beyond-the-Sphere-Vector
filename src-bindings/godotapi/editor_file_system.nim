...
proc getFileType*(self: EditorFileSystem; path: string): string {.gcsafe,
    locks: 0.}
...
proc getFileType(self: EditorFileSystem; path: string): string =
  if isNil(editorFileSystemGetFileTypeMethodBind):
    editorFileSystemGetFileTypeMethodBind = getMethod(
        cstring"EditorFileSystem", cstring"get_file_type")
  var
...
proc scanSources(self: EditorFileSystem) =
  if isNil(editorFileSystemScanSourcesMethodBind):
    editorFileSystemScanSourcesMethodBind = getMethod(
        cstring"EditorFileSystem", cstring"scan_sources")
  var ptrCallRet: pointer
  editorFileSystemScanSourcesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

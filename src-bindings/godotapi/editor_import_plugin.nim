...
method getImporterName*(self: EditorImportPlugin): string {.gcsafe, locks: 0,
    base.}
...
method getRecognizedExtensions*(self: EditorImportPlugin): Array {.gcsafe,
    locks: 0, base.}
method getResourceType*(self: EditorImportPlugin): string {.gcsafe, locks: 0,
    base.}
method getSaveExtension*(self: EditorImportPlugin): string {.gcsafe, locks: 0,
    base.}
method getVisibleName*(self: EditorImportPlugin): string {.gcsafe, locks: 0,
    base.}
...
method importt(self: EditorImportPlugin; sourceFile: string; savePath: string;
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  editorImportPluginImportMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

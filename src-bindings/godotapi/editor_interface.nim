...
proc `distractionFreeMode=`*(self: EditorInterface; val: bool) {.gcsafe,
    locks: 0.}
proc editNode*(self: EditorInterface; node: Node) {.gcsafe, locks: 0.}
proc editResource*(self: EditorInterface; resource: Resource) {.gcsafe, locks: 0.}
proc editScript*(self: EditorInterface; script: Script; line: int64 = -1'i64;
                 column: int64 = 0'i64; grabFocus: bool = true) {.gcsafe,
    locks: 0.}
...
proc getEditorScale*(self: EditorInterface): float64 {.gcsafe, locks: 0.}
proc getEditorSettings*(self: EditorInterface): EditorSettings {.gcsafe,
    locks: 0.}
proc getEditorViewport*(self: EditorInterface): Control {.gcsafe, locks: 0.}
proc getFileSystemDock*(self: EditorInterface): FileSystemDock {.gcsafe,
    locks: 0.}
...
proc getPlayingScene*(self: EditorInterface): string {.gcsafe, locks: 0.}
...
proc inspectObject*(self: EditorInterface; objectt: Object;
                    forProperty: string = ""; inspectorOnly: bool = false) {.
    gcsafe, locks: 0.}
proc isPlayingScene*(self: EditorInterface): bool {.gcsafe, locks: 0.}
proc isPluginEnabled*(self: EditorInterface; plugin: string): bool {.gcsafe,
    locks: 0.}
...
proc openSceneFromPath*(self: EditorInterface; sceneFilepath: string) {.gcsafe,
    locks: 0.}
proc playCurrentScene*(self: EditorInterface) {.gcsafe, locks: 0.}
proc playCustomScene*(self: EditorInterface; sceneFilepath: string) {.gcsafe,
    locks: 0.}
proc playMainScene*(self: EditorInterface) {.gcsafe, locks: 0.}
...
proc setMainScreenEditor*(self: EditorInterface; name: string) {.gcsafe,
    locks: 0.}
proc setPluginEnabled*(self: EditorInterface; plugin: string; enabled: bool) {.
    gcsafe, locks: 0.}
proc stopPlayingScene*(self: EditorInterface) {.gcsafe, locks: 0.}
...
var editorInterfaceEditNodeMethodBind {.threadvar.}: ptr GodotMethodBind
proc editNode(self: EditorInterface; node: Node) =
  if isNil(editorInterfaceEditNodeMethodBind):
    editorInterfaceEditNodeMethodBind = getMethod(cstring"EditorInterface",
        cstring"edit_node")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  var ptrCallRet: pointer
  editorInterfaceEditNodeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
var editorInterfaceEditScriptMethodBind {.threadvar.}: ptr GodotMethodBind
proc editScript(self: EditorInterface; script: Script; line: int64 = -1'i64;
                column: int64 = 0'i64; grabFocus: bool = true) =
  if isNil(editorInterfaceEditScriptMethodBind):
    editorInterfaceEditScriptMethodBind = getMethod(cstring"EditorInterface",
        cstring"edit_script")
  var
  ...
  let argToPassToGodot0 = if not script.isNil:
    ...
    nil
  argsToPassToGodot[][0] = argToPassToGodot0
  argsToPassToGodot[][1] = unsafeAddr(line)
  argsToPassToGodot[][2] = unsafeAddr(column)
  argsToPassToGodot[][3] = unsafeAddr(grabFocus)
  var ptrCallRet: pointer
  editorInterfaceEditScriptMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getBaseControl(self: EditorInterface): Control =
  if isNil(editorInterfaceGetBaseControlMethodBind):
    editorInterfaceGetBaseControlMethodBind = getMethod(
        cstring"EditorInterface", cstring"get_base_control")
  var ptrCallRet: pointer
...
proc getCurrentPath(self: EditorInterface): string =
  if isNil(editorInterfaceGetCurrentPathMethodBind):
    editorInterfaceGetCurrentPathMethodBind = getMethod(
        cstring"EditorInterface", cstring"get_current_path")
  var ptrCallRet: pointer
...
var editorInterfaceGetEditorScaleMethodBind {.threadvar.}: ptr GodotMethodBind
proc getEditorScale(self: EditorInterface): float64 =
  if isNil(editorInterfaceGetEditorScaleMethodBind):
    editorInterfaceGetEditorScaleMethodBind = getMethod(
        cstring"EditorInterface", cstring"get_editor_scale")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  editorInterfaceGetEditorScaleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getInspector(self: EditorInterface): EditorInspector =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  editorInterfaceGetInspectorMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc getOpenScenes(self: EditorInterface): Array =
  if isNil(editorInterfaceGetOpenScenesMethodBind):
    editorInterfaceGetOpenScenesMethodBind = getMethod(
        cstring"EditorInterface", cstring"get_open_scenes")
  var ptrCallRet: pointer
  var ptrCallVal: GodotArray
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  editorInterfaceGetOpenScenesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = newArray(ptrCallVal)

var editorInterfaceGetPlayingSceneMethodBind {.threadvar.}: ptr GodotMethodBind
proc getPlayingScene(self: EditorInterface): string =
  if isNil(editorInterfaceGetPlayingSceneMethodBind):
    editorInterfaceGetPlayingSceneMethodBind = getMethod(
        cstring"EditorInterface", cstring"get_playing_scene")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  editorInterfaceGetPlayingSceneMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc getSelection(self: EditorInterface): EditorSelection =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  editorInterfaceGetSelectionMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc inspectObject(self: EditorInterface; objectt: Object;
  ...
  if isNil(editorInterfaceInspectObjectMethodBind):
    editorInterfaceInspectObjectMethodBind = getMethod(
        cstring"EditorInterface", cstring"inspect_object")
  var
  ...
  argsToPassToGodot[][2] = unsafeAddr(inspectorOnly)
  var ptrCallRet: pointer
  editorInterfaceInspectObjectMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot1)

var editorInterfaceIsPlayingSceneMethodBind {.threadvar.}: ptr GodotMethodBind
proc isPlayingScene(self: EditorInterface): bool =
  if isNil(editorInterfaceIsPlayingSceneMethodBind):
    editorInterfaceIsPlayingSceneMethodBind = getMethod(
        cstring"EditorInterface", cstring"is_playing_scene")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  editorInterfaceIsPlayingSceneMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var editorInterfacePlayCurrentSceneMethodBind {.threadvar.}: ptr GodotMethodBind
proc playCurrentScene(self: EditorInterface) =
  if isNil(editorInterfacePlayCurrentSceneMethodBind):
    editorInterfacePlayCurrentSceneMethodBind = getMethod(
        cstring"EditorInterface", cstring"play_current_scene")
  var ptrCallRet: pointer
  editorInterfacePlayCurrentSceneMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var editorInterfacePlayCustomSceneMethodBind {.threadvar.}: ptr GodotMethodBind
proc playCustomScene(self: EditorInterface; sceneFilepath: string) =
  if isNil(editorInterfacePlayCustomSceneMethodBind):
    editorInterfacePlayCustomSceneMethodBind = getMethod(
        cstring"EditorInterface", cstring"play_custom_scene")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(sceneFilepath)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  editorInterfacePlayCustomSceneMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

var editorInterfacePlayMainSceneMethodBind {.threadvar.}: ptr GodotMethodBind
proc playMainScene(self: EditorInterface) =
  if isNil(editorInterfacePlayMainSceneMethodBind):
    editorInterfacePlayMainSceneMethodBind = getMethod(
        cstring"EditorInterface", cstring"play_main_scene")
  var ptrCallRet: pointer
  editorInterfacePlayMainSceneMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var editorInterfaceStopPlayingSceneMethodBind {.threadvar.}: ptr GodotMethodBind
proc stopPlayingScene(self: EditorInterface) =
  if isNil(editorInterfaceStopPlayingSceneMethodBind):
    editorInterfaceStopPlayingSceneMethodBind = getMethod(
        cstring"EditorInterface", cstring"stop_playing_scene")
  var ptrCallRet: pointer
  editorInterfaceStopPlayingSceneMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

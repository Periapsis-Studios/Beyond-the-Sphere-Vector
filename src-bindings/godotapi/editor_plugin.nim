...
proc addAutoloadSingleton*(self: EditorPlugin; name: string; path: string) {.
    gcsafe, locks: 0.}
proc addControlToBottomPanel*(self: EditorPlugin; control: Control;
                              title: string): ToolButton {.gcsafe, locks: 0.}
proc addControlToContainer*(self: EditorPlugin; container: int64;
                            control: Control) {.gcsafe, locks: 0.}
proc addControlToDock*(self: EditorPlugin; slot: int64; control: Control) {.
    gcsafe, locks: 0.}
proc addCustomType*(self: EditorPlugin; typee: string; base: string;
                    script: Script; icon: Texture) {.gcsafe, locks: 0.}
...
proc addInspectorPlugin*(self: EditorPlugin; plugin: EditorInspectorPlugin) {.
    gcsafe, locks: 0.}
proc addSceneImportPlugin*(self: EditorPlugin;
                           sceneImporter: EditorSceneImporter) {.gcsafe,
    locks: 0.}
...
proc addToolMenuItem*(self: EditorPlugin; name: string; handler: Object;
                      callback: string; ud: Variant = newVariant()) {.gcsafe,
    locks: 0.}
...
method forwardCanvasGuiInput*(self: EditorPlugin; event: InputEvent): bool {.
    gcsafe, locks: 0, base.}
method forwardSpatialDrawOverViewport*(self: EditorPlugin; overlay: Control) {.
    gcsafe, locks: 0, base.}
method forwardSpatialForceDrawOverViewport*(self: EditorPlugin; overlay: Control) {.
    gcsafe, locks: 0, base.}
method forwardSpatialGuiInput*(self: EditorPlugin; camera: Camera;
                               event: InputEvent): bool {.gcsafe, locks: 0, base.}
method getBreakpoints*(self: EditorPlugin): PoolStringArray {.gcsafe, locks: 0,
    base.}
...
method getWindowLayout*(self: EditorPlugin; layout: ConfigFile) {.gcsafe,
    locks: 0, base.}
method handles*(self: EditorPlugin; objectt: Object): bool {.gcsafe, locks: 0,
    base.}
method hasMainScreen*(self: EditorPlugin): bool {.gcsafe, locks: 0, base.}
proc hideBottomPanel*(self: EditorPlugin) {.gcsafe, locks: 0.}
proc makeBottomPanelItemVisible*(self: EditorPlugin; item: Control) {.gcsafe,
    locks: 0.}
...
proc removeAutoloadSingleton*(self: EditorPlugin; name: string) {.gcsafe,
    locks: 0.}
...
proc removeControlFromContainer*(self: EditorPlugin; container: int64;
                                 control: Control) {.gcsafe, locks: 0.}
proc removeControlFromDocks*(self: EditorPlugin; control: Control) {.gcsafe,
    locks: 0.}
...
proc removeSceneImportPlugin*(self: EditorPlugin;
                              sceneImporter: EditorSceneImporter) {.gcsafe,
    locks: 0.}
proc removeSpatialGizmoPlugin*(self: EditorPlugin;
                               plugin: EditorSpatialGizmoPlugin) {.gcsafe,
    locks: 0.}
...
proc setInputEventForwardingAlwaysEnabled*(self: EditorPlugin) {.gcsafe,
    locks: 0.}
method setState*(self: EditorPlugin; state: Dictionary) {.gcsafe, locks: 0, base.}
method setWindowLayout*(self: EditorPlugin; layout: ConfigFile) {.gcsafe,
    locks: 0, base.}
...
proc addAutoloadSingleton(self: EditorPlugin; name: string; path: string) =
  if isNil(editorPluginAddAutoloadSingletonMethodBind):
    editorPluginAddAutoloadSingletonMethodBind = getMethod(
        cstring"EditorPlugin", cstring"add_autoload_singleton")
  var
...
proc addSceneImportPlugin(self: EditorPlugin; sceneImporter: EditorSceneImporter) =
  if isNil(editorPluginAddSceneImportPluginMethodBind):
    editorPluginAddSceneImportPluginMethodBind = getMethod(
        cstring"EditorPlugin", cstring"add_scene_import_plugin")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method build(self: EditorPlugin): bool =
  if isNil(editorPluginBuildMethodBind):
    editorPluginBuildMethodBind = getMethod(cstring"EditorPlugin",
        cstring"build")
  var ptrCallRet: pointer
...
method clear(self: EditorPlugin) =
  if isNil(editorPluginClearMethodBind):
    editorPluginClearMethodBind = getMethod(cstring"EditorPlugin",
        cstring"clear")
  var ptrCallRet: pointer
...
var editorPluginForwardSpatialDrawOverViewportMethodBind {.threadvar.}: ptr GodotMethodBind
method forwardSpatialDrawOverViewport(self: EditorPlugin; overlay: Control) =
  if isNil(editorPluginForwardSpatialDrawOverViewportMethodBind):
    editorPluginForwardSpatialDrawOverViewportMethodBind = getMethod(
        cstring"EditorPlugin", cstring"forward_spatial_draw_over_viewport")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not overlay.isNil:
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  editorPluginForwardSpatialDrawOverViewportMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var editorPluginForwardSpatialForceDrawOverViewportMethodBind {.threadvar.}: ptr GodotMethodBind
method forwardSpatialForceDrawOverViewport(self: EditorPlugin; overlay: Control) =
  if isNil(editorPluginForwardSpatialForceDrawOverViewportMethodBind):
    editorPluginForwardSpatialForceDrawOverViewportMethodBind = getMethod(
        cstring"EditorPlugin",
        cstring"forward_spatial_force_draw_over_viewport")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not overlay.isNil:
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  editorPluginForwardSpatialForceDrawOverViewportMethodBind.ptrCall(
      self.godotObject, argsToPassToGodot, ptrCallRet)
...
proc hideBottomPanel(self: EditorPlugin) =
  ...
  var ptrCallRet: pointer
  editorPluginHideBottomPanelMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc queueSaveLayout(self: EditorPlugin) =
  ...
  var ptrCallRet: pointer
  editorPluginQueueSaveLayoutMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc removeSceneImportPlugin(self: EditorPlugin;
                             sceneImporter: EditorSceneImporter) =
...
proc removeSpatialGizmoPlugin(self: EditorPlugin;
                              plugin: EditorSpatialGizmoPlugin) =
...
method saveExternalData(self: EditorPlugin) =
  ...
  var ptrCallRet: pointer
  editorPluginSaveExternalDataMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
method focusableFocused*(self: EditorProperty; arg0: int64) {.gcsafe, locks: 0,
    base.}
...
proc `checked=`(self: EditorProperty; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  editorPropertySetCheckedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `drawRed=`(self: EditorProperty; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  editorPropertySetDrawRedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getEditedObject(self: EditorProperty): Object =
  if isNil(editorPropertyGetEditedObjectMethodBind):
    editorPropertyGetEditedObjectMethodBind = getMethod(
        cstring"EditorProperty", cstring"get_edited_object")
  var ptrCallRet: pointer
...
proc getTooltipText(self: EditorProperty): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  editorPropertyGetTooltipTextMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc setBottomEditor(self: EditorProperty; editor: Control) =
  if isNil(editorPropertySetBottomEditorMethodBind):
    editorPropertySetBottomEditorMethodBind = getMethod(
        cstring"EditorProperty", cstring"set_bottom_editor")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method updateProperty(self: EditorProperty) =
  ...
  var ptrCallRet: pointer
  editorPropertyUpdatePropertyMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
proc getProjectMetadata*(self: EditorSettings; section: string; key: string;
                         default: Variant = newVariant()): Variant {.gcsafe,
    locks: 0.}
...
proc propertyCanRevert*(self: EditorSettings; name: string): bool {.gcsafe,
    locks: 0.}
proc propertyGetRevert*(self: EditorSettings; name: string): Variant {.gcsafe,
    locks: 0.}
proc setFavorites*(self: EditorSettings; dirs: PoolStringArray) {.gcsafe,
    locks: 0.}
...
proc setRecentDirs*(self: EditorSettings; dirs: PoolStringArray) {.gcsafe,
    locks: 0.}
proc setSetting*(self: EditorSettings; name: string; value: Variant) {.gcsafe,
    locks: 0.}
...
proc addPropertyInfo(self: EditorSettings; info: Dictionary) =
  if isNil(editorSettingsAddPropertyInfoMethodBind):
    editorSettingsAddPropertyInfoMethodBind = getMethod(
        cstring"EditorSettings", cstring"add_property_info")
  var
...
proc getRecentDirs(self: EditorSettings): PoolStringArray =
  ...
  initGodotPoolStringArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  editorSettingsGetRecentDirsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = newPoolStringArray(ptrCallVal)
...
proc getSetting(self: EditorSettings; name: string): Variant =
  ...
  initGodotVariant(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  editorSettingsGetSettingMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  result = newVariant(ptrCallVal)
...
proc getSettingsDir(self: EditorSettings): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  editorSettingsGetSettingsDirMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc hasSetting(self: EditorSettings; name: string): bool =
  ...
  editorSettingsHasSettingMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc setInitialValue(self: EditorSettings; name: string; value: Variant;
  ...
  if isNil(editorSettingsSetInitialValueMethodBind):
    editorSettingsSetInitialValueMethodBind = getMethod(
        cstring"EditorSettings", cstring"set_initial_value")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(name)
...
proc setSetting(self: EditorSettings; name: string; value: Variant) =
  ...
  argsToPassToGodot[][1] = value.godotVariant
  var ptrCallRet: pointer
  editorSettingsSetSettingMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

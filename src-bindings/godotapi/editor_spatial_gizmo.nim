...
proc addHandles*(self: EditorSpatialGizmo; handles: PoolVector3Array;
                 material: Material; billboard: bool = false;
                 secondary: bool = false) {.gcsafe, locks: 0.}
proc addLines*(self: EditorSpatialGizmo; lines: PoolVector3Array;
               material: Material; billboard: bool = false;
               modulate: Color = initColor(1.0, 1.0, 1.0, 1.0)) {.gcsafe,
    locks: 0.}
proc addMesh*(self: EditorSpatialGizmo; mesh: Mesh; billboard: bool = false;
              skeleton: SkinReference = nil; material: Material = nil) {.gcsafe,
    locks: 0.}
...
proc addLines(self: EditorSpatialGizmo; lines: PoolVector3Array;
  ...
  if isNil(editorSpatialGizmoAddLinesMethodBind):
    editorSpatialGizmoAddLinesMethodBind = getMethod(
        cstring"EditorSpatialGizmo", cstring"add_lines")
  var
    argsStatic: array[4, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc addMesh(self: EditorSpatialGizmo; mesh: Mesh; billboard: bool = false;
  ...
  if isNil(editorSpatialGizmoAddMeshMethodBind):
    editorSpatialGizmoAddMeshMethodBind = getMethod(
        cstring"EditorSpatialGizmo", cstring"add_mesh")
  var
    argsStatic: array[4, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getPlugin(self: EditorSpatialGizmo): EditorSpatialGizmoPlugin =
  ...
  ptrCallRet = addr(ptrCallVal)
  editorSpatialGizmoGetPluginMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

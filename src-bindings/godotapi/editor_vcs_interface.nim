...
method getModifiedFilesData*(self: EditorVCSInterface): Dictionary {.gcsafe,
    locks: 0, base.}
method getProjectName*(self: EditorVCSInterface): string {.gcsafe, locks: 0,
    base.}
...
method initialize*(self: EditorVCSInterface; projectRootPath: string): bool {.
    gcsafe, locks: 0, base.}
method isVcsInitialized*(self: EditorVCSInterface): bool {.gcsafe, locks: 0,
    base.}
method shutDown*(self: EditorVCSInterface): bool {.gcsafe, locks: 0, base.}
method stageFile*(self: EditorVCSInterface; filePath: string) {.gcsafe,
    locks: 0, base.}
method unstageFile*(self: EditorVCSInterface; filePath: string) {.gcsafe,
    locks: 0, base.}
...
proc stageFileImpl*(self: EditorVCSInterface; filePath: string) {.gcsafe,
    locks: 0.}
proc unstageFileImpl*(self: EditorVCSInterface; filePath: string) {.gcsafe,
    locks: 0.}
...
method getVcsName(self: EditorVCSInterface): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  editorVCSInterfaceUnderscoregetVcsNameMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc commitImpl(self: EditorVCSInterface; msg: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  editorVCSInterfaceCommitMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc getModifiedFilesDataImpl(self: EditorVCSInterface): Dictionary =
  ...
  initGodotDictionary(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  editorVCSInterfaceGetModifiedFilesDataMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
  result = newDictionary(ptrCallVal)
...
proc getVcsNameImpl(self: EditorVCSInterface): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  editorVCSInterfaceGetVcsNameMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc shutDownImpl(self: EditorVCSInterface): bool =
  if isNil(editorVCSInterfaceShutDownMethodBind):
    editorVCSInterfaceShutDownMethodBind = getMethod(
        cstring"EditorVCSInterface", cstring"shut_down")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

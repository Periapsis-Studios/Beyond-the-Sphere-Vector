...
proc isPrintingErrorMessages*(): bool {.gcsafe, locks: 0.}
...
proc setPrintErrorMessages*(enabled: bool) {.gcsafe, locks: 0.}
...
proc getIterationsPerSecond(): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  engineGetIterationsPerSecondMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getPhysicsInterpolationFraction(): float64 =
  ...
  if isNil(engineGetPhysicsInterpolationFractionMethodBind):
    engineGetPhysicsInterpolationFractionMethodBind = getMethod(
        cstring"_Engine", cstring"get_physics_interpolation_fraction")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
var engineIsPrintingErrorMessagesMethodBind {.threadvar.}: ptr GodotMethodBind
proc isPrintingErrorMessages(): bool =
  if isNil(singletonEngine):
    singletonEngine = getSingleton[Engine]()
  let self = singletonEngine
  if isNil(engineIsPrintingErrorMessagesMethodBind):
    engineIsPrintingErrorMessagesMethodBind = getMethod(cstring"_Engine",
        cstring"is_printing_error_messages")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  engineIsPrintingErrorMessagesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var engineSetPrintErrorMessagesMethodBind {.threadvar.}: ptr GodotMethodBind
proc setPrintErrorMessages(enabled: bool) =
  if isNil(singletonEngine):
    singletonEngine = getSingleton[Engine]()
  let self = singletonEngine
  if isNil(engineSetPrintErrorMessagesMethodBind):
    engineSetPrintErrorMessagesMethodBind = getMethod(cstring"_Engine",
        cstring"set_print_error_messages")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(enabled)
  var ptrCallRet: pointer
  engineSetPrintErrorMessagesMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

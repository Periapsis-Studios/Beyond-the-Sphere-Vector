...
proc `adjustmentBrightness=`*(self: Environment; val: float64) {.gcsafe,
    locks: 0.}
proc adjustmentColorCorrection*(self: Environment): Texture {.gcsafe, locks: 0.}
proc `adjustmentColorCorrection=`*(self: Environment; val: Texture) {.gcsafe,
    locks: 0.}
...
proc `adjustmentSaturation=`*(self: Environment; val: float64) {.gcsafe,
    locks: 0.}
...
proc `backgroundCameraFeedId=`*(self: Environment; val: int64) {.gcsafe,
    locks: 0.}
proc backgroundCanvasMaxLayer*(self: Environment): int64 {.gcsafe, locks: 0.}
proc `backgroundCanvasMaxLayer=`*(self: Environment; val: int64) {.gcsafe,
    locks: 0.}
...
proc `backgroundSkyCustomFov=`*(self: Environment; val: float64) {.gcsafe,
    locks: 0.}
proc backgroundSkyOrientation*(self: Environment): Basis {.gcsafe, locks: 0.}
proc `backgroundSkyOrientation=`*(self: Environment; val: Basis) {.gcsafe,
    locks: 0.}
proc backgroundSkyRotation*(self: Environment): Vector3 {.gcsafe, locks: 0.}
proc `backgroundSkyRotation=`*(self: Environment; val: Vector3) {.gcsafe,
    locks: 0.}
proc backgroundSkyRotationDegrees*(self: Environment): Vector3 {.gcsafe,
    locks: 0.}
...
proc `dofBlurFarTransition=`*(self: Environment; val: float64) {.gcsafe,
    locks: 0.}
...
proc `dofBlurNearTransition=`*(self: Environment; val: float64) {.gcsafe,
    locks: 0.}
...
proc glowHighQuality*(self: Environment): bool {.gcsafe, locks: 0.}
proc `glowHighQuality=`*(self: Environment; val: bool) {.gcsafe, locks: 0.}
...
proc `ssReflectionsFadeOut=`*(self: Environment; val: float64) {.gcsafe,
    locks: 0.}
...
proc adjustmentContrast(self: Environment): float64 =
  if isNil(environmentGetAdjustmentContrastMethodBind):
    environmentGetAdjustmentContrastMethodBind = getMethod(
        cstring"Environment", cstring"get_adjustment_contrast")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `adjustmentContrast=`(self: Environment; val: float64) =
  if isNil(environmentSetAdjustmentContrastMethodBind):
    environmentSetAdjustmentContrastMethodBind = getMethod(
        cstring"Environment", cstring"set_adjustment_contrast")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc ambientLightEnergy(self: Environment): float64 =
  if isNil(environmentGetAmbientLightEnergyMethodBind):
    environmentGetAmbientLightEnergyMethodBind = getMethod(
        cstring"Environment", cstring"get_ambient_light_energy")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `ambientLightEnergy=`(self: Environment; val: float64) =
  if isNil(environmentSetAmbientLightEnergyMethodBind):
    environmentSetAmbientLightEnergyMethodBind = getMethod(
        cstring"Environment", cstring"set_ambient_light_energy")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc autoExposureEnabled(self: Environment): bool =
  if isNil(environmentGetTonemapAutoExposureMethodBind):
    environmentGetTonemapAutoExposureMethodBind = getMethod(
        cstring"Environment", cstring"get_tonemap_auto_exposure")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `autoExposureEnabled=`(self: Environment; val: bool) =
  if isNil(environmentSetTonemapAutoExposureMethodBind):
    environmentSetTonemapAutoExposureMethodBind = getMethod(
        cstring"Environment", cstring"set_tonemap_auto_exposure")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc autoExposureSpeed(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetTonemapAutoExposureSpeedMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc backgroundCanvasMaxLayer(self: Environment): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetCanvasMaxLayerMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `backgroundMode=`(self: Environment; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetBackgroundMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc backgroundSky(self: Environment): Sky =
  if isNil(environmentGetSkyMethodBind):
    environmentGetSkyMethodBind = getMethod(cstring"Environment",
        cstring"get_sky")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc `backgroundSky=`(self: Environment; val: Sky) =
  if isNil(environmentSetSkyMethodBind):
    environmentSetSkyMethodBind = getMethod(cstring"Environment",
        cstring"set_sky")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc backgroundSkyOrientation(self: Environment): Basis =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetSkyOrientationMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc backgroundSkyRotationDegrees(self: Environment): Vector3 =
  if isNil(environmentGetSkyRotationDegreesMethodBind):
    environmentGetSkyRotationDegreesMethodBind = getMethod(
        cstring"Environment", cstring"get_sky_rotation_degrees")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `backgroundSkyRotationDegrees=`(self: Environment; val: Vector3) =
  if isNil(environmentSetSkyRotationDegreesMethodBind):
    environmentSetSkyRotationDegreesMethodBind = getMethod(
        cstring"Environment", cstring"set_sky_rotation_degrees")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc dofBlurFarDistance(self: Environment): float64 =
  if isNil(environmentGetDofBlurFarDistanceMethodBind):
    environmentGetDofBlurFarDistanceMethodBind = getMethod(
        cstring"Environment", cstring"get_dof_blur_far_distance")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `dofBlurFarDistance=`(self: Environment; val: float64) =
  if isNil(environmentSetDofBlurFarDistanceMethodBind):
    environmentSetDofBlurFarDistanceMethodBind = getMethod(
        cstring"Environment", cstring"set_dof_blur_far_distance")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc dofBlurNearDistance(self: Environment): float64 =
  if isNil(environmentGetDofBlurNearDistanceMethodBind):
    environmentGetDofBlurNearDistanceMethodBind = getMethod(
        cstring"Environment", cstring"get_dof_blur_near_distance")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `dofBlurNearDistance=`(self: Environment; val: float64) =
  if isNil(environmentSetDofBlurNearDistanceMethodBind):
    environmentSetDofBlurNearDistanceMethodBind = getMethod(
        cstring"Environment", cstring"set_dof_blur_near_distance")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc `dofBlurNearEnabled=`(self: Environment; val: bool) =
  if isNil(environmentSetDofBlurNearEnabledMethodBind):
    environmentSetDofBlurNearEnabledMethodBind = getMethod(
        cstring"Environment", cstring"set_dof_blur_near_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc dofBlurNearQuality(self: Environment): int64 =
  if isNil(environmentGetDofBlurNearQualityMethodBind):
    environmentGetDofBlurNearQualityMethodBind = getMethod(
        cstring"Environment", cstring"get_dof_blur_near_quality")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `dofBlurNearQuality=`(self: Environment; val: int64) =
  if isNil(environmentSetDofBlurNearQualityMethodBind):
    environmentSetDofBlurNearQualityMethodBind = getMethod(
        cstring"Environment", cstring"set_dof_blur_near_quality")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc fogDepthBegin(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetFogDepthBeginMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc fogDepthCurve(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetFogDepthCurveMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc fogDepthEnabled(self: Environment): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentIsFogDepthEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `fogEnabled=`(self: Environment; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetFogEnabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc fogHeightCurve(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetFogHeightCurveMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `fogTransmitEnabled=`(self: Environment; val: bool) =
  if isNil(environmentSetFogTransmitEnabledMethodBind):
    environmentSetFogTransmitEnabledMethodBind = getMethod(
        cstring"Environment", cstring"set_fog_transmit_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc glowBicubicUpscale(self: Environment): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentIsGlowBicubicUpscaleEnabledMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc `glowBicubicUpscale=`(self: Environment; val: bool) =
  if isNil(environmentSetGlowBicubicUpscaleMethodBind):
    environmentSetGlowBicubicUpscaleMethodBind = getMethod(
        cstring"Environment", cstring"set_glow_bicubic_upscale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc glowBlendMode(self: Environment): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetGlowBlendModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc glowHdrLuminanceCap(self: Environment): float64 =
  if isNil(environmentGetGlowHdrLuminanceCapMethodBind):
    environmentGetGlowHdrLuminanceCapMethodBind = getMethod(
        cstring"Environment", cstring"get_glow_hdr_luminance_cap")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `glowHdrLuminanceCap=`(self: Environment; val: float64) =
  if isNil(environmentSetGlowHdrLuminanceCapMethodBind):
    environmentSetGlowHdrLuminanceCapMethodBind = getMethod(
        cstring"Environment", cstring"set_glow_hdr_luminance_cap")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
var environmentIsGlowHighQualityEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc glowHighQuality(self: Environment): bool =
  if isNil(environmentIsGlowHighQualityEnabledMethodBind):
    environmentIsGlowHighQualityEnabledMethodBind = getMethod(
        cstring"Environment", cstring"is_glow_high_quality_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentIsGlowHighQualityEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var environmentSetGlowHighQualityMethodBind {.threadvar.}: ptr GodotMethodBind
proc `glowHighQuality=`(self: Environment; val: bool) =
  if isNil(environmentSetGlowHighQualityMethodBind):
    environmentSetGlowHighQualityMethodBind = getMethod(cstring"Environment",
        cstring"set_glow_high_quality")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetGlowHighQualityMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc glowIntensity(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetGlowIntensityMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `ssReflectionsEnabled=`(self: Environment; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetSsrEnabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `ssReflectionsFadeOut=`(self: Environment; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetSsrFadeOutMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc ssaoAoChannelAffect(self: Environment): float64 =
  if isNil(environmentGetSsaoAoChannelAffectMethodBind):
    environmentGetSsaoAoChannelAffectMethodBind = getMethod(
        cstring"Environment", cstring"get_ssao_ao_channel_affect")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `ssaoAoChannelAffect=`(self: Environment; val: float64) =
  if isNil(environmentSetSsaoAoChannelAffectMethodBind):
    environmentSetSsaoAoChannelAffectMethodBind = getMethod(
        cstring"Environment", cstring"set_ssao_ao_channel_affect")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc ssaoIntensity(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetSsaoIntensityMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc ssaoIntensity2(self: Environment): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  environmentGetSsaoIntensity2MethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `ssaoRadius=`(self: Environment; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetSsaoRadiusMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `tonemapMode=`(self: Environment; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  environmentSetTonemapperMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

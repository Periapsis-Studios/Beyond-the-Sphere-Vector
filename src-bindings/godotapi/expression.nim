...
proc execute*(self: Expression; inputs: Array = newArray();
              baseInstance: Object = nil; showError: bool = true): Variant {.
    gcsafe, locks: 0.}
...
proc execute(self: Expression; inputs: Array = newArray();
  ...
  if isNil(expressionExecuteMethodBind):
    expressionExecuteMethodBind = getMethod(cstring"Expression",
        cstring"execute")
  var

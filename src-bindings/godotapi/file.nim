...
proc flush*(self: godottypes.File) {.gcsafe, locks: 0.}
...
proc getBuffer*(self: godottypes.File; len: int64): PoolByteArray {.gcsafe,
    locks: 0.}
...
proc getModifiedTime*(self: godottypes.File; file: string): int64 {.gcsafe,
    locks: 0.}
...
proc open*(self: godottypes.File; path: string; flags: int64): Error {.gcsafe,
    locks: 0.}
...
proc openEncryptedWithPass*(self: godottypes.File; path: string;
                            modeFlags: int64; pass: string): Error {.gcsafe,
    locks: 0.}
...
proc storeBuffer*(self: godottypes.File; buffer: PoolByteArray) {.gcsafe,
    locks: 0.}
...
proc storePascalString*(self: godottypes.File; string: string) {.gcsafe,
    locks: 0.}
...
proc fileExists(self: godottypes.File; path: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  fileFileExistsMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)

var fileFlushMethodBind {.threadvar.}: ptr GodotMethodBind
proc flush(self: godottypes.File) =
  if isNil(fileFlushMethodBind):
    fileFlushMethodBind = getMethod(cstring"_File", cstring"flush")
  var ptrCallRet: pointer
  fileFlushMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc getBuffer(self: godottypes.File; len: int64): PoolByteArray =
  ...
  ptrCallRet = addr(ptrCallVal)
  fileGetBufferMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc getCsvLine(self: godottypes.File; delim: string = ","): PoolStringArray =
  ...
  ptrCallRet = addr(ptrCallVal)
  fileGetCsvLineMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
...
proc getSha256(self: godottypes.File; path: string): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  fileGetSha256MethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc openEncryptedWithPass(self: godottypes.File; path: string;
                           modeFlags: int64; pass: string): Error =
...
proc storeCsvLine(self: godottypes.File; values: PoolStringArray;
  ...
  if isNil(fileStoreCsvLineMethodBind):
    fileStoreCsvLineMethodBind = getMethod(cstring"_File",
        cstring"store_csv_line")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc storeFloat(self: godottypes.File; value: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  fileStoreFloatMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc storeLine(self: godottypes.File; line: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  fileStoreLineMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc storeReal(self: godottypes.File; value: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  fileStoreRealMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)

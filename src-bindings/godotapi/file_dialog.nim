...
method treeMultiSelected*(self: FileDialog; arg0: Object; arg1: int64;
...
proc `currentFile=`(self: FileDialog; val: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  fileDialogSetCurrentFileMethodBind.ptrCall(self.godotObject,
...
proc `currentPath=`(self: FileDialog; val: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  fileDialogSetCurrentPathMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc mode(self: FileDialog): int64 =
  if isNil(fileDialogGetModeMethodBind):
    fileDialogGetModeMethodBind = getMethod(cstring"FileDialog",
        cstring"get_mode")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `mode=`(self: FileDialog; val: int64) =
  if isNil(fileDialogSetModeMethodBind):
    fileDialogSetModeMethodBind = getMethod(cstring"FileDialog",
        cstring"set_mode")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method actionPressed(self: FileDialog) =
  if isNil(fileDialogUnderscoreactionPressedMethodBind):
    fileDialogUnderscoreactionPressedMethodBind = getMethod(
        cstring"FileDialog", cstring"_action_pressed")
  var ptrCallRet: pointer
...
method cancelPressed(self: FileDialog) =
  if isNil(fileDialogUnderscorecancelPressedMethodBind):
    fileDialogUnderscorecancelPressedMethodBind = getMethod(
        cstring"FileDialog", cstring"_cancel_pressed")
  var ptrCallRet: pointer
...
method filterSelected(self: FileDialog; arg0: int64) =
  if isNil(fileDialogUnderscorefilterSelectedMethodBind):
    fileDialogUnderscorefilterSelectedMethodBind = getMethod(
        cstring"FileDialog", cstring"_filter_selected")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method makeDir(self: FileDialog) =
  if isNil(fileDialogUnderscoremakeDirMethodBind):
    fileDialogUnderscoremakeDirMethodBind = getMethod(cstring"FileDialog",
        cstring"_make_dir")
  var ptrCallRet: pointer
  fileDialogUnderscoremakeDirMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method makeDirConfirm(self: FileDialog) =
  if isNil(fileDialogUnderscoremakeDirConfirmMethodBind):
    fileDialogUnderscoremakeDirConfirmMethodBind = getMethod(
        cstring"FileDialog", cstring"_make_dir_confirm")
  var ptrCallRet: pointer
...
method saveConfirmPressed(self: FileDialog) =
  ...
  var ptrCallRet: pointer
  fileDialogUnderscoresaveConfirmPressedMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
method unhandledInput(self: FileDialog; event: InputEvent) =
  if isNil(fileDialogUnderscoreunhandledInputMethodBind):
    fileDialogUnderscoreunhandledInputMethodBind = getMethod(
        cstring"FileDialog", cstring"_unhandled_input")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method updateFileList(self: FileDialog) =
  if isNil(fileDialogUnderscoreupdateFileListMethodBind):
    fileDialogUnderscoreupdateFileListMethodBind = getMethod(
        cstring"FileDialog", cstring"_update_file_list")
  var ptrCallRet: pointer
...
method updateFileName(self: FileDialog) =
  if isNil(fileDialogUnderscoreupdateFileNameMethodBind):
    fileDialogUnderscoreupdateFileNameMethodBind = getMethod(
        cstring"FileDialog", cstring"_update_file_name")
  var ptrCallRet: pointer
...
proc getVbox(self: FileDialog): VBoxContainer =
  if isNil(fileDialogGetVboxMethodBind):
    fileDialogGetVboxMethodBind = getMethod(cstring"FileDialog",
        cstring"get_vbox")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)

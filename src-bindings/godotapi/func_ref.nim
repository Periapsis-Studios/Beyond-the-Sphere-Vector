...
proc function*(self: FuncRef): string {.gcsafe, locks: 0.}
proc `function=`*(self: FuncRef; val: string) {.gcsafe, locks: 0.}
...
var funcRefGetFunctionMethodBind {.threadvar.}: ptr GodotMethodBind
proc function(self: FuncRef): string =
  if isNil(funcRefGetFunctionMethodBind):
    funcRefGetFunctionMethodBind = getMethod(cstring"FuncRef",
        cstring"get_function")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  funcRefGetFunctionMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)

var funcRefSetFunctionMethodBind {.threadvar.}: ptr GodotMethodBind
proc `function=`(self: FuncRef; val: string) =
...
proc callFuncv(self: FuncRef; argArray: Array): Variant =
  if isNil(funcRefCallFuncvMethodBind):
    funcRefCallFuncvMethodBind = getMethod(cstring"FuncRef",
        cstring"call_funcv")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
proc configFile(self: GDNativeLibrary): ConfigFile =
  if isNil(gDNativeLibraryGetConfigFileMethodBind):
    gDNativeLibraryGetConfigFileMethodBind = getMethod(
        cstring"GDNativeLibrary", cstring"get_config_file")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  gDNativeLibraryGetConfigFileMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `configFile=`(self: GDNativeLibrary; val: ConfigFile) =
  if isNil(gDNativeLibrarySetConfigFileMethodBind):
    gDNativeLibrarySetConfigFileMethodBind = getMethod(
        cstring"GDNativeLibrary", cstring"set_config_file")
  var
...
proc loadOnce(self: GDNativeLibrary): bool =
  if isNil(gDNativeLibraryShouldLoadOnceMethodBind):
    gDNativeLibraryShouldLoadOnceMethodBind = getMethod(
        cstring"GDNativeLibrary", cstring"should_load_once")
  var ptrCallRet: pointer
...
proc reloadable(self: GDNativeLibrary): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  gDNativeLibraryIsReloadableMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `reloadable=`(self: GDNativeLibrary; val: bool) =
  if isNil(gDNativeLibrarySetReloadableMethodBind):
    gDNativeLibrarySetReloadableMethodBind = getMethod(
        cstring"GDNativeLibrary", cstring"set_reloadable")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

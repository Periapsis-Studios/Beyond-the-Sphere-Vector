...
proc getDataDir*(): string {.gcsafe, locks: 0.}
...
proc getEnvironment*(variable: string): string {.gcsafe, locks: 0.}
...
proc getLocaleLanguage*(): string {.gcsafe, locks: 0.}
...
proc getNativeHandle*(handleType: int64): int64 {.gcsafe, locks: 0.}
...
proc getSystemDir*(dir: int64; sharedStorage: bool = true): string {.gcsafe,
    locks: 0.}
...
proc getThreadCallerId*(): int64 {.gcsafe, locks: 0.}
...
proc hasEnvironment*(variable: string): bool {.gcsafe, locks: 0.}
...
proc isDeltaSmoothingEnabled*(): bool {.gcsafe, locks: 0.}
...
proc setDeltaSmoothing*(deltaSmoothingEnabled: bool) {.gcsafe, locks: 0.}
proc setEnvironment*(variable: string; value: string): bool {.gcsafe, locks: 0.}
...
proc setWindowMousePassthrough*(region: PoolVector2Array) {.gcsafe, locks: 0.}
...
proc showVirtualKeyboard*(existingText: string = ""; multiline: bool = false) {.
    gcsafe, locks: 0.}
...
proc canUseThreads(): bool =
  ...
  if isNil(oSCanUseThreadsMethodBind):
    oSCanUseThreadsMethodBind = getMethod(cstring"_OS",
        cstring"can_use_threads")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc findScancodeFromString(string: string): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSFindScancodeFromStringMethodBind.ptrCall(self.godotObject,
...
var oSGetCacheDirMethodBind {.threadvar.}: ptr GodotMethodBind
proc getCacheDir(): string =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetCacheDirMethodBind):
    oSGetCacheDirMethodBind = getMethod(cstring"_OS", cstring"get_cache_dir")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  oSGetCacheDirMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc getCmdlineArgs(): PoolStringArray =
  ...
  if isNil(oSGetCmdlineArgsMethodBind):
    oSGetCmdlineArgsMethodBind = getMethod(cstring"_OS",
        cstring"get_cmdline_args")
  var ptrCallRet: pointer
...
var oSGetConfigDirMethodBind {.threadvar.}: ptr GodotMethodBind
proc getConfigDir(): string =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetConfigDirMethodBind):
    oSGetConfigDirMethodBind = getMethod(cstring"_OS", cstring"get_config_dir")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  oSGetConfigDirMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
var oSGetDataDirMethodBind {.threadvar.}: ptr GodotMethodBind
proc getDataDir(): string =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetDataDirMethodBind):
    oSGetDataDirMethodBind = getMethod(cstring"_OS", cstring"get_data_dir")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  oSGetDataDirMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc getDatetime(utc: bool = false): Dictionary =
  ...
  initGodotDictionary(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  oSGetDatetimeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  result = newDictionary(ptrCallVal)
...
proc getEnvironment(variable: string): string =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetEnvironmentMethodBind):
    oSGetEnvironmentMethodBind = getMethod(cstring"_OS",
        cstring"get_environment")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(variable)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
...
var oSGetLocaleLanguageMethodBind {.threadvar.}: ptr GodotMethodBind
proc getLocaleLanguage(): string =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetLocaleLanguageMethodBind):
    oSGetLocaleLanguageMethodBind = getMethod(cstring"_OS",
        cstring"get_locale_language")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  oSGetLocaleLanguageMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
var oSGetNativeHandleMethodBind {.threadvar.}: ptr GodotMethodBind
proc getNativeHandle(handleType: int64): int64 =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetNativeHandleMethodBind):
    oSGetNativeHandleMethodBind = getMethod(cstring"_OS",
        cstring"get_native_handle")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(handleType)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSGetNativeHandleMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                      ptrCallRet)
...
proc getPowerState(): int64 =
  ...
  if isNil(oSGetPowerStateMethodBind):
    oSGetPowerStateMethodBind = getMethod(cstring"_OS",
        cstring"get_power_state")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getScreenCount(): int64 =
  ...
  if isNil(oSGetScreenCountMethodBind):
    oSGetScreenCountMethodBind = getMethod(cstring"_OS",
        cstring"get_screen_count")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getScreenDpi(screen: int64 = -1'i64): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSGetScreenDpiMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

var oSGetScreenMaxScaleMethodBind {.threadvar.}: ptr GodotMethodBind
proc getScreenMaxScale(): float64 =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetScreenMaxScaleMethodBind):
    oSGetScreenMaxScaleMethodBind = getMethod(cstring"_OS",
        cstring"get_screen_max_scale")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSGetScreenMaxScaleMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
var oSGetScreenScaleMethodBind {.threadvar.}: ptr GodotMethodBind
proc getScreenScale(screen: int64 = -1'i64): float64 =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetScreenScaleMethodBind):
    oSGetScreenScaleMethodBind = getMethod(cstring"_OS",
        cstring"get_screen_scale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(screen)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSGetScreenScaleMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                     ptrCallRet)
...
proc getScreenSize(screen: int64 = -1'i64): Vector2 =
  ...
  if isNil(oSGetScreenSizeMethodBind):
    oSGetScreenSizeMethodBind = getMethod(cstring"_OS",
        cstring"get_screen_size")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(screen)
...
proc getSystemDir(dir: int64; sharedStorage: bool = true): string =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  ...
  argsToPassToGodot[][1] = unsafeAddr(sharedStorage)
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  oSGetSystemDirMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
var oSGetThreadCallerIdMethodBind {.threadvar.}: ptr GodotMethodBind
proc getThreadCallerId(): int64 =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSGetThreadCallerIdMethodBind):
    oSGetThreadCallerIdMethodBind = getMethod(cstring"_OS",
        cstring"get_thread_caller_id")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSGetThreadCallerIdMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc getWindowPerPixelTransparencyEnabled(): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSGetWindowPerPixelTransparencyEnabledMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc getWindowSize(): Vector2 =
  ...
  if isNil(oSGetWindowSizeMethodBind):
    oSGetWindowSizeMethodBind = getMethod(cstring"_OS",
        cstring"get_window_size")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc globalMenuAddSeparator(menu: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  oSGlobalMenuAddSeparatorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc hasEnvironment(variable: string): bool =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSHasEnvironmentMethodBind):
    oSHasEnvironmentMethodBind = getMethod(cstring"_OS",
        cstring"has_environment")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(variable)
...
var oSIsDeltaSmoothingEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc isDeltaSmoothingEnabled(): bool =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSIsDeltaSmoothingEnabledMethodBind):
    oSIsDeltaSmoothingEnabledMethodBind = getMethod(cstring"_OS",
        cstring"is_delta_smoothing_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSIsDeltaSmoothingEnabledMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc isInLowProcessorUsageMode(): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSIsInLowProcessorUsageModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc isVsyncEnabled(): bool =
  ...
  if isNil(oSIsVsyncEnabledMethodBind):
    oSIsVsyncEnabledMethodBind = getMethod(cstring"_OS",
        cstring"is_vsync_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc openMidiInputs() =
  ...
  if isNil(oSOpenMidiInputsMethodBind):
    oSOpenMidiInputsMethodBind = getMethod(cstring"_OS",
        cstring"open_midi_inputs")
  var ptrCallRet: pointer
...
proc setClipboard(clipboard: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  oSSetClipboardMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
...
proc setCurrentTabletDriver(name: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  oSSetCurrentTabletDriverMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

var oSSetDeltaSmoothingMethodBind {.threadvar.}: ptr GodotMethodBind
proc setDeltaSmoothing(deltaSmoothingEnabled: bool) =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSSetDeltaSmoothingMethodBind):
    oSSetDeltaSmoothingMethodBind = getMethod(cstring"_OS",
        cstring"set_delta_smoothing")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(deltaSmoothingEnabled)
  var ptrCallRet: pointer
  oSSetDeltaSmoothingMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                        ptrCallRet)

var oSSetEnvironmentMethodBind {.threadvar.}: ptr GodotMethodBind
proc setEnvironment(variable: string; value: string): bool =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSSetEnvironmentMethodBind):
    oSSetEnvironmentMethodBind = getMethod(cstring"_OS",
  ...
  var argToPassToGodot0 = toGodotString(variable)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var argToPassToGodot1 = toGodotString(value)
  argsToPassToGodot[][1] = unsafeAddr(argToPassToGodot1)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  oSSetEnvironmentMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                     ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
...
proc setExitCode(code: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(code)
  var ptrCallRet: pointer
  oSSetExitCodeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc setImeActive(active: bool) =
  ...
  var ptrCallRet: pointer
  oSSetImeActiveMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc setImePosition(position: Vector2) =
  ...
  if isNil(oSSetImePositionMethodBind):
    oSSetImePositionMethodBind = getMethod(cstring"_OS",
        cstring"set_ime_position")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setNativeIcon(filename: string) =
  ...
  if isNil(oSSetNativeIconMethodBind):
    oSSetNativeIconMethodBind = getMethod(cstring"_OS",
        cstring"set_native_icon")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setThreadName(name: string): Error =
  ...
  if isNil(oSSetThreadNameMethodBind):
    oSSetThreadNameMethodBind = getMethod(cstring"_OS",
        cstring"set_thread_name")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(name)
...
proc setUseVsync(enable: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(enable)
  var ptrCallRet: pointer
  oSSetUseVsyncMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
var oSSetWindowMousePassthroughMethodBind {.threadvar.}: ptr GodotMethodBind
proc setWindowMousePassthrough(region: PoolVector2Array) =
  if isNil(singletonOS):
    singletonOS = getSingleton[OS]()
  let self = singletonOS
  if isNil(oSSetWindowMousePassthroughMethodBind):
    oSSetWindowMousePassthroughMethodBind = getMethod(cstring"_OS",
        cstring"set_window_mouse_passthrough")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = region.godotPoolVector2Array
  var ptrCallRet: pointer
  oSSetWindowMousePassthroughMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setWindowSize(size: Vector2) =
  ...
  if isNil(oSSetWindowSizeMethodBind):
    oSSetWindowSizeMethodBind = getMethod(cstring"_OS",
        cstring"set_window_size")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(size)
...
proc setWindowTitle(title: string) =
  ...
  if isNil(oSSetWindowTitleMethodBind):
    oSSetWindowTitleMethodBind = getMethod(cstring"_OS",
        cstring"set_window_title")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc showVirtualKeyboard(existingText: string = ""; multiline: bool = false) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  argsToPassToGodot[][1] = unsafeAddr(multiline)
  var ptrCallRet: pointer

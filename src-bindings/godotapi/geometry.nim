...
proc buildCapsulePlanes*(radius: float64; height: float64; sides: int64;
                         lats: int64; axis: int64 = 2'i64): Array {.gcsafe,
    locks: 0.}
...
proc clipPolylineWithPolygon2d*(polyline: PoolVector2Array;
    ...
    locks: 0.}
proc convexHull2d*(points: PoolVector2Array): PoolVector2Array {.gcsafe,
    locks: 0.}
...
proc getClosestPointToSegmentUncapped*(point: Vector3; s1: Vector3; s2: Vector3): Vector3 {.
    gcsafe, locks: 0.}
proc getClosestPointToSegmentUncapped2d*(point: Vector2; s1: Vector2;
    s2: Vector2): Vector2 {.gcsafe, locks: 0.}
...
proc intersectPolylineWithPolygon2d*(polyline: PoolVector2Array;
                                     polygon: PoolVector2Array): Array {.gcsafe,
    locks: 0.}
proc isPointInCircle*(point: Vector2; circlePosition: Vector2;
                      circleRadius: float64): bool {.gcsafe, locks: 0.}
...
proc lineIntersectsLine2d*(fromA: Vector2; dirA: Vector2; fromB: Vector2;
                           dirB: Vector2): Variant {.gcsafe, locks: 0.}
...
proc pointIsInsideTriangle*(point: Vector2; a: Vector2; b: Vector2; c: Vector2): bool {.
    gcsafe, locks: 0.}
proc rayIntersectsTriangle*(fromm: Vector3; dir: Vector3; a: Vector3;
                            b: Vector3; c: Vector3): Variant {.gcsafe, locks: 0.}
...
proc segmentIntersectsCylinder*(fromm: Vector3; to: Vector3; height: float64;
                                radius: float64): PoolVector3Array {.gcsafe,
    locks: 0.}
proc segmentIntersectsSegment2d*(fromA: Vector2; toA: Vector2; fromB: Vector2;
                                 toB: Vector2): Variant {.gcsafe, locks: 0.}
proc segmentIntersectsSphere*(fromm: Vector3; to: Vector3;
                              spherePosition: Vector3; sphereRadius: float64): PoolVector3Array {.
    gcsafe, locks: 0.}
proc segmentIntersectsTriangle*(fromm: Vector3; to: Vector3; a: Vector3;
                                b: Vector3; c: Vector3): Variant {.gcsafe,
    locks: 0.}
proc triangulateDelaunay2d*(points: PoolVector2Array): PoolIntArray {.gcsafe,
    locks: 0.}
proc triangulatePolygon*(polygon: PoolVector2Array): PoolIntArray {.gcsafe,
    locks: 0.}
...
proc buildCapsulePlanes(radius: float64; height: float64; sides: int64;
                        lats: int64; axis: int64 = 2'i64): Array =
  if isNil(singletonGeometry):
    singletonGeometry = getSingleton[Geometry]()
  let self = singletonGeometry
...
proc getClosestPointToSegment2d(point: Vector2; s1: Vector2; s2: Vector2): Vector2 =
  ...
  if isNil(geometryGetClosestPointToSegment2dMethodBind):
    geometryGetClosestPointToSegment2dMethodBind = getMethod(
        cstring"_Geometry", cstring"get_closest_point_to_segment_2d")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(point)
...
proc getUv84NormalBit(normal: Vector3): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  geometryGetUv84NormalBitMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc isPointInPolygon(point: Vector2; polygon: PoolVector2Array): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  geometryIsPointInPolygonMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc offsetPolyline2d(polyline: PoolVector2Array; delta: float64;
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  geometryOffsetPolyline2dMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  result = newArray(ptrCallVal)
...
proc segmentIntersectsSegment2d(fromA: Vector2; toA: Vector2; fromB: Vector2;
  ...
  if isNil(geometrySegmentIntersectsSegment2dMethodBind):
    geometrySegmentIntersectsSegment2dMethodBind = getMethod(
        cstring"_Geometry", cstring"segment_intersects_segment_2d")
  var
    argsStatic: array[4, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(fromA)

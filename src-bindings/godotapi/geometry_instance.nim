...
const
  ...
  LIGHTMAP_SCALE_1X* = 0'i64
  LIGHTMAP_SCALE_2X* = 1'i64
  LIGHTMAP_SCALE_4X* = 2'i64
  LIGHTMAP_SCALE_8X* = 3'i64
  LIGHTMAP_SCALE_MAX* = 4'i64
...
proc `extraCullMargin=`*(self: GeometryInstance; val: float64) {.gcsafe,
    locks: 0.}
proc generateLightmap*(self: GeometryInstance): bool {.gcsafe, locks: 0.}
proc `generateLightmap=`*(self: GeometryInstance; val: bool) {.gcsafe, locks: 0.}
proc lightmapScale*(self: GeometryInstance): int64 {.gcsafe, locks: 0.}
proc `lightmapScale=`*(self: GeometryInstance; val: int64) {.gcsafe, locks: 0.}
...
proc `lodMaxHysteresis=`*(self: GeometryInstance; val: float64) {.gcsafe,
    locks: 0.}
...
proc `lodMinHysteresis=`*(self: GeometryInstance; val: float64) {.gcsafe,
    locks: 0.}
proc materialOverride*(self: GeometryInstance): Material {.gcsafe, locks: 0.}
proc `materialOverride=`*(self: GeometryInstance; val: Material) {.gcsafe,
    locks: 0.}
...
var geometryInstanceGetGenerateLightmapMethodBind {.threadvar.}: ptr GodotMethodBind
proc generateLightmap(self: GeometryInstance): bool =
  if isNil(geometryInstanceGetGenerateLightmapMethodBind):
    geometryInstanceGetGenerateLightmapMethodBind = getMethod(
        cstring"GeometryInstance", cstring"get_generate_lightmap")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  geometryInstanceGetGenerateLightmapMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var geometryInstanceSetGenerateLightmapMethodBind {.threadvar.}: ptr GodotMethodBind
proc `generateLightmap=`(self: GeometryInstance; val: bool) =
  if isNil(geometryInstanceSetGenerateLightmapMethodBind):
    geometryInstanceSetGenerateLightmapMethodBind = getMethod(
        cstring"GeometryInstance", cstring"set_generate_lightmap")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  geometryInstanceSetGenerateLightmapMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var geometryInstanceGetLightmapScaleMethodBind {.threadvar.}: ptr GodotMethodBind
proc lightmapScale(self: GeometryInstance): int64 =
  if isNil(geometryInstanceGetLightmapScaleMethodBind):
    geometryInstanceGetLightmapScaleMethodBind = getMethod(
        cstring"GeometryInstance", cstring"get_lightmap_scale")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  geometryInstanceGetLightmapScaleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var geometryInstanceSetLightmapScaleMethodBind {.threadvar.}: ptr GodotMethodBind
proc `lightmapScale=`(self: GeometryInstance; val: int64) =
  if isNil(geometryInstanceSetLightmapScaleMethodBind):
    geometryInstanceSetLightmapScaleMethodBind = getMethod(
        cstring"GeometryInstance", cstring"set_lightmap_scale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  geometryInstanceSetLightmapScaleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

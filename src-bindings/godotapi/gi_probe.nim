...
proc `bias=`(self: GIProbe; val: float64) =
  ...
  var ptrCallRet: pointer
  gIProbeSetBiasMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc energy(self: GIProbe): float64 =
  if isNil(gIProbeGetEnergyMethodBind):
    gIProbeGetEnergyMethodBind = getMethod(cstring"GIProbe",
        cstring"get_energy")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `energy=`(self: GIProbe; val: float64) =
  if isNil(gIProbeSetEnergyMethodBind):
    gIProbeSetEnergyMethodBind = getMethod(cstring"GIProbe",
        cstring"set_energy")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc extents(self: GIProbe): Vector3 =
  if isNil(gIProbeGetExtentsMethodBind):
    gIProbeGetExtentsMethodBind = getMethod(cstring"GIProbe",
        cstring"get_extents")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `extents=`(self: GIProbe; val: Vector3) =
  if isNil(gIProbeSetExtentsMethodBind):
    gIProbeSetExtentsMethodBind = getMethod(cstring"GIProbe",
        cstring"set_extents")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc interior(self: GIProbe): bool =
  if isNil(gIProbeIsInteriorMethodBind):
    gIProbeIsInteriorMethodBind = getMethod(cstring"GIProbe",
        cstring"is_interior")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc subdiv(self: GIProbe): int64 =
  if isNil(gIProbeGetSubdivMethodBind):
    gIProbeGetSubdivMethodBind = getMethod(cstring"GIProbe",
        cstring"get_subdiv")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `subdiv=`(self: GIProbe; val: int64) =
  if isNil(gIProbeSetSubdivMethodBind):
    gIProbeSetSubdivMethodBind = getMethod(cstring"GIProbe",
        cstring"set_subdiv")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc debugBake(self: GIProbe) =
  if isNil(gIProbeDebugBakeMethodBind):
    gIProbeDebugBakeMethodBind = getMethod(cstring"GIProbe",
        cstring"debug_bake")
  var ptrCallRet: pointer

...
proc removePoint*(self: Gradient; point: int64) {.gcsafe, locks: 0.}
proc setColor*(self: Gradient; point: int64; color: Color) {.gcsafe, locks: 0.}
proc setOffset*(self: Gradient; point: int64; offset: float64) {.gcsafe,
    locks: 0.}
...
proc colors(self: Gradient): PoolColorArray =
  if isNil(gradientGetColorsMethodBind):
    gradientGetColorsMethodBind = getMethod(cstring"Gradient",
        cstring"get_colors")
...
proc `colors=`(self: Gradient; val: PoolColorArray) =
  if isNil(gradientSetColorsMethodBind):
    gradientSetColorsMethodBind = getMethod(cstring"Gradient",
        cstring"set_colors")
  var
...
proc addPoint(self: Gradient; offset: float64; color: Color) =
  if isNil(gradientAddPointMethodBind):
    gradientAddPointMethodBind = getMethod(cstring"Gradient",
        cstring"add_point")
  var
...
proc getColor(self: Gradient; point: int64): Color =
  if isNil(gradientGetColorMethodBind):
    gradientGetColorMethodBind = getMethod(cstring"Gradient",
        cstring"get_color")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getOffset(self: Gradient; point: int64): float64 =
  if isNil(gradientGetOffsetMethodBind):
    gradientGetOffsetMethodBind = getMethod(cstring"Gradient",
        cstring"get_offset")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc removePoint(self: Gradient; point: int64) =
...
proc setColor(self: Gradient; point: int64; color: Color) =
  if isNil(gradientSetColorMethodBind):
    gradientSetColorMethodBind = getMethod(cstring"Gradient",
        cstring"set_color")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setOffset(self: Gradient; point: int64; offset: float64) =
  if isNil(gradientSetOffsetMethodBind):
    gradientSetOffsetMethodBind = getMethod(cstring"Gradient",
        cstring"set_offset")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

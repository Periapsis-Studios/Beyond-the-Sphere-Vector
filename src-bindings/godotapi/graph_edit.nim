...
proc minimapEnabled*(self: GraphEdit): bool {.gcsafe, locks: 0.}
proc `minimapEnabled=`*(self: GraphEdit; val: bool) {.gcsafe, locks: 0.}
proc minimapOpacity*(self: GraphEdit): float64 {.gcsafe, locks: 0.}
proc `minimapOpacity=`*(self: GraphEdit; val: float64) {.gcsafe, locks: 0.}
proc minimapSize*(self: GraphEdit): Vector2 {.gcsafe, locks: 0.}
proc `minimapSize=`*(self: GraphEdit; val: Vector2) {.gcsafe, locks: 0.}
...
proc showZoomLabel*(self: GraphEdit): bool {.gcsafe, locks: 0.}
proc `showZoomLabel=`*(self: GraphEdit; val: bool) {.gcsafe, locks: 0.}
...
proc zoomMax*(self: GraphEdit): float64 {.gcsafe, locks: 0.}
proc `zoomMax=`*(self: GraphEdit; val: float64) {.gcsafe, locks: 0.}
proc zoomMin*(self: GraphEdit): float64 {.gcsafe, locks: 0.}
proc `zoomMin=`*(self: GraphEdit; val: float64) {.gcsafe, locks: 0.}
proc zoomStep*(self: GraphEdit): float64 {.gcsafe, locks: 0.}
proc `zoomStep=`*(self: GraphEdit; val: float64) {.gcsafe, locks: 0.}
...
method graphNodeSlotUpdated*(self: GraphEdit; arg0: int64; arg1: Node) {.gcsafe,
    locks: 0, base.}
method guiInput*(self: GraphEdit; event: InputEvent) {.gcsafe, locks: 0.}
method minimapDraw*(self: GraphEdit) {.gcsafe, locks: 0, base.}
method minimapToggled*(self: GraphEdit) {.gcsafe, locks: 0, base.}
method scrollMoved*(self: GraphEdit; arg0: float64) {.gcsafe, locks: 0, base.}
method snapToggled*(self: GraphEdit) {.gcsafe, locks: 0, base.}
method snapValueChanged*(self: GraphEdit; arg0: float64) {.gcsafe, locks: 0,
    base.}
method topLayerDraw*(self: GraphEdit) {.gcsafe, locks: 0, base.}
method topLayerInput*(self: GraphEdit; arg0: InputEvent) {.gcsafe, locks: 0,
    base.}
...
proc addValidConnectionType*(self: GraphEdit; fromType: int64; toType: int64) {.
    gcsafe, locks: 0.}
proc addValidLeftDisconnectType*(self: GraphEdit; typee: int64) {.gcsafe,
    locks: 0.}
proc addValidRightDisconnectType*(self: GraphEdit; typee: int64) {.gcsafe,
    locks: 0.}
...
proc disconnectNode*(self: GraphEdit; fromm: string; fromPort: int64;
                     to: string; toPort: int64) {.gcsafe, locks: 0.}
proc getConnectionList*(self: GraphEdit): Array {.gcsafe, locks: 0.}
proc getZoomHbox*(self: GraphEdit): HBoxContainer {.gcsafe, locks: 0.}
proc isNodeConnected*(self: GraphEdit; fromm: string; fromPort: int64;
                      to: string; toPort: int64): bool {.gcsafe, locks: 0.}
...
proc removeValidConnectionType*(self: GraphEdit; fromType: int64; toType: int64) {.
    gcsafe, locks: 0.}
proc removeValidLeftDisconnectType*(self: GraphEdit; typee: int64) {.gcsafe,
    locks: 0.}
proc removeValidRightDisconnectType*(self: GraphEdit; typee: int64) {.gcsafe,
    locks: 0.}
proc setConnectionActivity*(self: GraphEdit; fromm: string; fromPort: int64;
                            to: string; toPort: int64; amount: float64) {.
    gcsafe, locks: 0.}
proc setSelected*(self: GraphEdit; node: Node) {.gcsafe, locks: 0.}
var graphEditIsMinimapEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc minimapEnabled(self: GraphEdit): bool =
  if isNil(graphEditIsMinimapEnabledMethodBind):
    graphEditIsMinimapEnabledMethodBind = getMethod(cstring"GraphEdit",
        cstring"is_minimap_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditIsMinimapEnabledMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var graphEditSetMinimapEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc `minimapEnabled=`(self: GraphEdit; val: bool) =
  if isNil(graphEditSetMinimapEnabledMethodBind):
    graphEditSetMinimapEnabledMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_minimap_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetMinimapEnabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var graphEditGetMinimapOpacityMethodBind {.threadvar.}: ptr GodotMethodBind
proc minimapOpacity(self: GraphEdit): float64 =
  if isNil(graphEditGetMinimapOpacityMethodBind):
    graphEditGetMinimapOpacityMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_minimap_opacity")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditGetMinimapOpacityMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var graphEditSetMinimapOpacityMethodBind {.threadvar.}: ptr GodotMethodBind
proc `minimapOpacity=`(self: GraphEdit; val: float64) =
  if isNil(graphEditSetMinimapOpacityMethodBind):
    graphEditSetMinimapOpacityMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_minimap_opacity")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetMinimapOpacityMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var graphEditGetMinimapSizeMethodBind {.threadvar.}: ptr GodotMethodBind
proc minimapSize(self: GraphEdit): Vector2 =
  if isNil(graphEditGetMinimapSizeMethodBind):
    graphEditGetMinimapSizeMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_minimap_size")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditGetMinimapSizeMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var graphEditSetMinimapSizeMethodBind {.threadvar.}: ptr GodotMethodBind
proc `minimapSize=`(self: GraphEdit; val: Vector2) =
  if isNil(graphEditSetMinimapSizeMethodBind):
    graphEditSetMinimapSizeMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_minimap_size")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetMinimapSizeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
proc rightDisconnects(self: GraphEdit): bool =
  if isNil(graphEditIsRightDisconnectsEnabledMethodBind):
    graphEditIsRightDisconnectsEnabledMethodBind = getMethod(
        cstring"GraphEdit", cstring"is_right_disconnects_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
var graphEditIsShowingZoomLabelMethodBind {.threadvar.}: ptr GodotMethodBind
proc showZoomLabel(self: GraphEdit): bool =
  if isNil(graphEditIsShowingZoomLabelMethodBind):
    graphEditIsShowingZoomLabelMethodBind = getMethod(cstring"GraphEdit",
        cstring"is_showing_zoom_label")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditIsShowingZoomLabelMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var graphEditSetShowZoomLabelMethodBind {.threadvar.}: ptr GodotMethodBind
proc `showZoomLabel=`(self: GraphEdit; val: bool) =
  if isNil(graphEditSetShowZoomLabelMethodBind):
    graphEditSetShowZoomLabelMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_show_zoom_label")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetShowZoomLabelMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc snapDistance(self: GraphEdit): int64 =
  if isNil(graphEditGetSnapMethodBind):
    graphEditGetSnapMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_snap")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `snapDistance=`(self: GraphEdit; val: int64) =
  if isNil(graphEditSetSnapMethodBind):
    graphEditSetSnapMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_snap")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc zoom(self: GraphEdit): float64 =
  if isNil(graphEditGetZoomMethodBind):
    graphEditGetZoomMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_zoom")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `zoom=`(self: GraphEdit; val: float64) =
  if isNil(graphEditSetZoomMethodBind):
    graphEditSetZoomMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_zoom")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var graphEditGetZoomMaxMethodBind {.threadvar.}: ptr GodotMethodBind
proc zoomMax(self: GraphEdit): float64 =
  if isNil(graphEditGetZoomMaxMethodBind):
    graphEditGetZoomMaxMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_zoom_max")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditGetZoomMaxMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var graphEditSetZoomMaxMethodBind {.threadvar.}: ptr GodotMethodBind
proc `zoomMax=`(self: GraphEdit; val: float64) =
  if isNil(graphEditSetZoomMaxMethodBind):
    graphEditSetZoomMaxMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_zoom_max")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetZoomMaxMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                        ptrCallRet)

var graphEditGetZoomMinMethodBind {.threadvar.}: ptr GodotMethodBind
proc zoomMin(self: GraphEdit): float64 =
  if isNil(graphEditGetZoomMinMethodBind):
    graphEditGetZoomMinMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_zoom_min")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditGetZoomMinMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var graphEditSetZoomMinMethodBind {.threadvar.}: ptr GodotMethodBind
proc `zoomMin=`(self: GraphEdit; val: float64) =
  if isNil(graphEditSetZoomMinMethodBind):
    graphEditSetZoomMinMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_zoom_min")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetZoomMinMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                        ptrCallRet)

var graphEditGetZoomStepMethodBind {.threadvar.}: ptr GodotMethodBind
proc zoomStep(self: GraphEdit): float64 =
  if isNil(graphEditGetZoomStepMethodBind):
    graphEditGetZoomStepMethodBind = getMethod(cstring"GraphEdit",
        cstring"get_zoom_step")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditGetZoomStepMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var graphEditSetZoomStepMethodBind {.threadvar.}: ptr GodotMethodBind
proc `zoomStep=`(self: GraphEdit; val: float64) =
  if isNil(graphEditSetZoomStepMethodBind):
    graphEditSetZoomStepMethodBind = getMethod(cstring"GraphEdit",
        cstring"set_zoom_step")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  graphEditSetZoomStepMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
method graphNodeRaised(self: GraphEdit; arg0: Node) =
  if isNil(graphEditUnderscoregraphNodeRaisedMethodBind):
    graphEditUnderscoregraphNodeRaisedMethodBind = getMethod(
        cstring"GraphEdit", cstring"_graph_node_raised")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not arg0.isNil:
...
var graphEditUnderscoregraphNodeSlotUpdatedMethodBind {.threadvar.}: ptr GodotMethodBind
method graphNodeSlotUpdated(self: GraphEdit; arg0: int64; arg1: Node) =
  if isNil(graphEditUnderscoregraphNodeSlotUpdatedMethodBind):
    graphEditUnderscoregraphNodeSlotUpdatedMethodBind = getMethod(
        cstring"GraphEdit", cstring"_graph_node_slot_updated")
  var
  ...
  var ptrCallRet: pointer
  graphEditUnderscoregraphNodeSlotUpdatedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var graphEditUnderscoreminimapDrawMethodBind {.threadvar.}: ptr GodotMethodBind
method minimapDraw(self: GraphEdit) =
  if isNil(graphEditUnderscoreminimapDrawMethodBind):
    graphEditUnderscoreminimapDrawMethodBind = getMethod(cstring"GraphEdit",
        cstring"_minimap_draw")
  var ptrCallRet: pointer
  graphEditUnderscoreminimapDrawMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var graphEditUnderscoreminimapToggledMethodBind {.threadvar.}: ptr GodotMethodBind
method minimapToggled(self: GraphEdit) =
  if isNil(graphEditUnderscoreminimapToggledMethodBind):
    graphEditUnderscoreminimapToggledMethodBind = getMethod(cstring"GraphEdit",
        cstring"_minimap_toggled")
  var ptrCallRet: pointer
  graphEditUnderscoreminimapToggledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method snapValueChanged(self: GraphEdit; arg0: float64) =
  if isNil(graphEditUnderscoresnapValueChangedMethodBind):
    graphEditUnderscoresnapValueChangedMethodBind = getMethod(
        cstring"GraphEdit", cstring"_snap_value_changed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(arg0)
...
method zoomMinus(self: GraphEdit) =
  if isNil(graphEditUnderscorezoomMinusMethodBind):
    graphEditUnderscorezoomMinusMethodBind = getMethod(cstring"GraphEdit",
        cstring"_zoom_minus")
  var ptrCallRet: pointer
  graphEditUnderscorezoomMinusMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method zoomPlus(self: GraphEdit) =
  if isNil(graphEditUnderscorezoomPlusMethodBind):
    graphEditUnderscorezoomPlusMethodBind = getMethod(cstring"GraphEdit",
        cstring"_zoom_plus")
  var ptrCallRet: pointer
  graphEditUnderscorezoomPlusMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method zoomReset(self: GraphEdit) =
  if isNil(graphEditUnderscorezoomResetMethodBind):
    graphEditUnderscorezoomResetMethodBind = getMethod(cstring"GraphEdit",
        cstring"_zoom_reset")
  var ptrCallRet: pointer
  graphEditUnderscorezoomResetMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc addValidLeftDisconnectType(self: GraphEdit; typee: int64) =
  if isNil(graphEditAddValidLeftDisconnectTypeMethodBind):
    graphEditAddValidLeftDisconnectTypeMethodBind = getMethod(
        cstring"GraphEdit", cstring"add_valid_left_disconnect_type")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc isNodeConnected(self: GraphEdit; fromm: string; fromPort: int64;
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  graphEditIsNodeConnectedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot2)
...
proc removeValidConnectionType(self: GraphEdit; fromType: int64; toType: int64) =
  if isNil(graphEditRemoveValidConnectionTypeMethodBind):
    graphEditRemoveValidConnectionTypeMethodBind = getMethod(
        cstring"GraphEdit", cstring"remove_valid_connection_type")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(fromType)

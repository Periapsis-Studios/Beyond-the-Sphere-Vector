...
proc useInBakedLight*(self: GridMap): bool {.gcsafe, locks: 0.}
proc `useInBakedLight=`*(self: GridMap; val: bool) {.gcsafe, locks: 0.}
...
proc getCellItem*(self: GridMap; x: int64; y: int64; z: int64): int64 {.gcsafe,
    locks: 0.}
...
proc mapToWorld*(self: GridMap; x: int64; y: int64; z: int64): Vector3 {.gcsafe,
    locks: 0.}
...
proc setClip*(self: GridMap; enabled: bool; clipabove: bool = true;
              floor: int64 = 0'i64; axis: int64 = 0'i64) {.gcsafe, locks: 0.}
proc setCollisionLayerBit*(self: GridMap; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
proc setCollisionMaskBit*(self: GridMap; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
...
proc `collisionLayer=`(self: GridMap; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  gridMapSetCollisionLayerMethodBind.ptrCall(self.godotObject,
...
var gridMapGetUseInBakedLightMethodBind {.threadvar.}: ptr GodotMethodBind
proc useInBakedLight(self: GridMap): bool =
  if isNil(gridMapGetUseInBakedLightMethodBind):
    gridMapGetUseInBakedLightMethodBind = getMethod(cstring"GridMap",
        cstring"get_use_in_baked_light")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  gridMapGetUseInBakedLightMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var gridMapSetUseInBakedLightMethodBind {.threadvar.}: ptr GodotMethodBind
proc `useInBakedLight=`(self: GridMap; val: bool) =
  if isNil(gridMapSetUseInBakedLightMethodBind):
    gridMapSetUseInBakedLightMethodBind = getMethod(cstring"GridMap",
        cstring"set_use_in_baked_light")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  gridMapSetUseInBakedLightMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
method updateOctantsCallback(self: GridMap) =
  ...
  var ptrCallRet: pointer
  gridMapUnderscoreupdateOctantsCallbackMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc getMeshes(self: GridMap): Array =
  if isNil(gridMapGetMeshesMethodBind):
    gridMapGetMeshesMethodBind = getMethod(cstring"GridMap",
        cstring"get_meshes")
  var ptrCallRet: pointer
  var ptrCallVal: GodotArray
  initGodotArray(ptrCallVal)
...
proc setClip(self: GridMap; enabled: bool; clipabove: bool = true;
             floor: int64 = 0'i64; axis: int64 = 0'i64) =
  ...
  var ptrCallRet: pointer
  gridMapSetClipMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

...
proc `angularLimitrelaxation=`*(self: HingeJoint; val: float64) {.gcsafe,
    locks: 0.}
...
proc angularLimitenable(self: HingeJoint): bool =
  if isNil(hingeJointGetFlagMethodBind):
    hingeJointGetFlagMethodBind = getMethod(cstring"HingeJoint",
        cstring"get_flag")
  var
...
proc `angularLimitenable=`(self: HingeJoint; val: bool) =
  if isNil(hingeJointSetFlagMethodBind):
    hingeJointSetFlagMethodBind = getMethod(cstring"HingeJoint",
        cstring"set_flag")
  var
    godotBoundArg0 = 0'i64
    argsStatic: array[2, pointer]
...
proc angularLimitlower(self: HingeJoint): float64 =
  if isNil(hingeJointUnderscoregetLowerLimitMethodBind):
    hingeJointUnderscoregetLowerLimitMethodBind = getMethod(
        cstring"HingeJoint", cstring"_get_lower_limit")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `angularLimitlower=`(self: HingeJoint; val: float64) =
  if isNil(hingeJointUnderscoresetLowerLimitMethodBind):
    hingeJointUnderscoresetLowerLimitMethodBind = getMethod(
        cstring"HingeJoint", cstring"_set_lower_limit")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc angularLimitupper(self: HingeJoint): float64 =
  if isNil(hingeJointUnderscoregetUpperLimitMethodBind):
    hingeJointUnderscoregetUpperLimitMethodBind = getMethod(
        cstring"HingeJoint", cstring"_get_upper_limit")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `angularLimitupper=`(self: HingeJoint; val: float64) =
  if isNil(hingeJointUnderscoresetUpperLimitMethodBind):
    hingeJointUnderscoresetUpperLimitMethodBind = getMethod(
        cstring"HingeJoint", cstring"_set_upper_limit")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

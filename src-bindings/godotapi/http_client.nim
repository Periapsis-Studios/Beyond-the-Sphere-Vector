...
proc getResponseHeadersAsDictionary*(self: HTTPClient): Dictionary {.gcsafe,
    locks: 0.}
...
proc request*(self: HTTPClient; methodd: int64; url: string;
              headers: PoolStringArray; body: string = ""): Error {.gcsafe,
    locks: 0.}
...
proc getResponseHeaders(self: HTTPClient): PoolStringArray =
  ...
  ptrCallRet = addr(ptrCallVal)
  hTTPClientGetResponseHeadersMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc isResponseChunked(self: HTTPClient): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  hTTPClientIsResponseChunkedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc request(self: HTTPClient; methodd: int64; url: string;
             headers: PoolStringArray; body: string = ""): Error =
  if isNil(hTTPClientRequestMethodBind):
    hTTPClientRequestMethodBind = getMethod(cstring"HTTPClient",
        cstring"request")
  var
    argsStatic: array[4, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

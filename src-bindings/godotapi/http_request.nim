...
method redirectRequest*(self: HTTPRequest; arg0: string) {.gcsafe, locks: 0,
    base.}
method requestDone*(self: HTTPRequest; arg0: int64; arg1: int64;
                    arg2: PoolStringArray; arg3: PoolByteArray) {.gcsafe,
    locks: 0, base.}
...
proc requestRaw*(self: HTTPRequest; url: string;
                 customHeaders: PoolStringArray = newPoolStringArray();
                 sslValidateDomain: bool = true; methodd: int64 = 0'i64;
                 requestDataRaw: PoolByteArray = newPoolByteArray()): Error {.
    gcsafe, locks: 0.}
...
proc bodySizeLimit(self: HTTPRequest): int64 =
  ...
  hTTPRequestGetBodySizeLimitMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `useThreads=`(self: HTTPRequest; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  hTTPRequestSetUseThreadsMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
method requestDone(self: HTTPRequest; arg0: int64; arg1: int64;
                   arg2: PoolStringArray; arg3: PoolByteArray) =
  if isNil(hTTPRequestUnderscorerequestDoneMethodBind):
    hTTPRequestUnderscorerequestDoneMethodBind = getMethod(
        cstring"HTTPRequest", cstring"_request_done")
  var
...
method timeout(self: HTTPRequest) =
  if isNil(hTTPRequestUnderscoretimeoutMethodBind):
    hTTPRequestUnderscoretimeoutMethodBind = getMethod(cstring"HTTPRequest",
        cstring"_timeout")
  var ptrCallRet: pointer
  hTTPRequestUnderscoretimeoutMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var hTTPRequestRequestRawMethodBind {.threadvar.}: ptr GodotMethodBind
proc requestRaw(self: HTTPRequest; url: string;
                customHeaders: PoolStringArray = newPoolStringArray();
                sslValidateDomain: bool = true; methodd: int64 = 0'i64;
                requestDataRaw: PoolByteArray = newPoolByteArray()): Error =
  if isNil(hTTPRequestRequestRawMethodBind):
    hTTPRequestRequestRawMethodBind = getMethod(cstring"HTTPRequest",
        cstring"request_raw")
  var
    argsStatic: array[5, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  argsToPassToGodot[][2] = unsafeAddr(sslValidateDomain)
  argsToPassToGodot[][3] = unsafeAddr(methodd)
  argsToPassToGodot[][4] = requestDataRaw.godotPoolByteArray
  var ptrCallRet: pointer
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  hTTPRequestRequestRawMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
  deinit(argToPassToGodot0)
  result = Error(ptrCallVal)

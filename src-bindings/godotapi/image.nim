...
proc blendRect*(self: Image; src: Image; srcRect: Rect2; dst: Vector2) {.gcsafe,
    locks: 0.}
proc blendRectMask*(self: Image; src: Image; mask: Image; srcRect: Rect2;
                    dst: Vector2) {.gcsafe, locks: 0.}
proc blitRect*(self: Image; src: Image; srcRect: Rect2; dst: Vector2) {.gcsafe,
    locks: 0.}
proc blitRectMask*(self: Image; src: Image; mask: Image; srcRect: Rect2;
                   dst: Vector2) {.gcsafe, locks: 0.}
proc bumpmapToNormalmap*(self: Image; bumpScale: float64 = 1.0) {.gcsafe,
    locks: 0.}
...
proc create*(self: Image; width: int64; height: int64; useMipmaps: bool;
             format: int64) {.gcsafe, locks: 0.}
...
proc generateMipmaps*(self: Image; renormalize: bool = false): Error {.gcsafe,
    locks: 0.}
...
proc loadBmpFromBuffer*(self: Image; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
proc loadJpgFromBuffer*(self: Image; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
proc loadPngFromBuffer*(self: Image; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
proc loadTgaFromBuffer*(self: Image; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
proc loadWebpFromBuffer*(self: Image; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
...
proc resize*(self: Image; width: int64; height: int64;
             interpolation: int64 = 1'i64) {.gcsafe, locks: 0.}
...
proc blendRect(self: Image; src: Image; srcRect: Rect2; dst: Vector2) =
  ...
  var ptrCallRet: pointer
  imageBlendRectMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc blitRect(self: Image; src: Image; srcRect: Rect2; dst: Vector2) =
  ...
  argsToPassToGodot[][2] = unsafeAddr(dst)
  var ptrCallRet: pointer
  imageBlitRectMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc clearMipmaps(self: Image) =
  if isNil(imageClearMipmapsMethodBind):
    imageClearMipmapsMethodBind = getMethod(cstring"Image",
        cstring"clear_mipmaps")
  var ptrCallRet: pointer
...
proc compress(self: Image; mode: int64; source: int64; lossyQuality: float64): Error =
  ...
  ptrCallRet = addr(ptrCallVal)
  imageCompressMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc copyFrom(self: Image; src: Image) =
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  imageCopyFromMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc detectAlpha(self: Image): int64 =
  if isNil(imageDetectAlphaMethodBind):
    imageDetectAlphaMethodBind = getMethod(cstring"Image",
        cstring"detect_alpha")
  var ptrCallRet: pointer
...
proc getPixel(self: Image; x: int64; y: int64): Color =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  imageGetPixelMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc getPixelv(self: Image; src: Vector2): Color =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  imageGetPixelvMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc getUsedRect(self: Image): Rect2 =
  if isNil(imageGetUsedRectMethodBind):
    imageGetUsedRectMethodBind = getMethod(cstring"Image",
...
proc isCompressed(self: Image): bool =
  if isNil(imageIsCompressedMethodBind):
    imageIsCompressedMethodBind = getMethod(cstring"Image",
...
proc isInvisible(self: Image): bool =
  if isNil(imageIsInvisibleMethodBind):
    imageIsInvisibleMethodBind = getMethod(cstring"Image",
        cstring"is_invisible")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
var imageLoadBmpFromBufferMethodBind {.threadvar.}: ptr GodotMethodBind
proc loadBmpFromBuffer(self: Image; buffer: PoolByteArray): Error =
  if isNil(imageLoadBmpFromBufferMethodBind):
    imageLoadBmpFromBufferMethodBind = getMethod(cstring"Image",
        cstring"load_bmp_from_buffer")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  imageLoadBmpFromBufferMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
  result = Error(ptrCallVal)
...
var imageLoadTgaFromBufferMethodBind {.threadvar.}: ptr GodotMethodBind
proc loadTgaFromBuffer(self: Image; buffer: PoolByteArray): Error =
  if isNil(imageLoadTgaFromBufferMethodBind):
    imageLoadTgaFromBufferMethodBind = getMethod(cstring"Image",
        cstring"load_tga_from_buffer")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = buffer.godotPoolByteArray
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  imageLoadTgaFromBufferMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
  result = Error(ptrCallVal)
...
proc resizeToPo2(self: Image; square: bool = false; interpolation: int64 = 1'i64) =
  if isNil(imageResizeToPo2MethodBind):
    imageResizeToPo2MethodBind = getMethod(cstring"Image",
        cstring"resize_to_po2")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setPixel(self: Image; x: int64; y: int64; color: Color) =
  ...
  var ptrCallRet: pointer
  imageSetPixelMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc setPixelv(self: Image; dst: Vector2; color: Color) =
  ...
  var ptrCallRet: pointer
  imageSetPixelvMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

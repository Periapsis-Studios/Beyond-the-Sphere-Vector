...
proc addSphere*(self: ImmediateGeometry; lats: int64; lons: int64;
                radius: float64; addUv: bool = true) {.gcsafe, locks: 0.}
...
proc addSphere(self: ImmediateGeometry; lats: int64; lons: int64;
               radius: float64; addUv: bool = true) =
  if isNil(immediateGeometryAddSphereMethodBind):
    immediateGeometryAddSphereMethodBind = getMethod(
        cstring"ImmediateGeometry", cstring"add_sphere")
  var
...
proc addVertex(self: ImmediateGeometry; position: Vector3) =
  if isNil(immediateGeometryAddVertexMethodBind):
    immediateGeometryAddVertexMethodBind = getMethod(
        cstring"ImmediateGeometry", cstring"add_vertex")
  var
...
proc setNormal(self: ImmediateGeometry; normal: Vector3) =
  if isNil(immediateGeometrySetNormalMethodBind):
    immediateGeometrySetNormalMethodBind = getMethod(
        cstring"ImmediateGeometry", cstring"set_normal")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setTangent(self: ImmediateGeometry; tangent: Plane) =
  if isNil(immediateGeometrySetTangentMethodBind):
    immediateGeometrySetTangentMethodBind = getMethod(
        cstring"ImmediateGeometry", cstring"set_tangent")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

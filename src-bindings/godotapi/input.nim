...
proc addJoyMapping*(mapping: string; updateExisting: bool = false) {.gcsafe,
    locks: 0.}
proc flushBufferedEvents*() {.gcsafe, locks: 0.}
proc getAccelerometer*(): Vector3 {.gcsafe, locks: 0.}
proc getActionRawStrength*(action: string; exact: bool = false): float64 {.
    gcsafe, locks: 0.}
proc getActionStrength*(action: string; exact: bool = false): float64 {.gcsafe,
    locks: 0.}
proc getAxis*(negativeAction: string; positiveAction: string): float64 {.gcsafe,
    locks: 0.}
...
proc getVector*(negativeX: string; positiveX: string; negativeY: string;
                positiveY: string; deadzone: float64 = -1.0): Vector2 {.gcsafe,
    locks: 0.}
proc isActionJustPressed*(action: string; exact: bool = false): bool {.gcsafe,
    locks: 0.}
proc isActionJustReleased*(action: string; exact: bool = false): bool {.gcsafe,
    locks: 0.}
proc isActionPressed*(action: string; exact: bool = false): bool {.gcsafe,
    locks: 0.}
...
proc isPhysicalKeyPressed*(scancode: int64): bool {.gcsafe, locks: 0.}
proc joyConnectionChanged*(device: int64; connected: bool; name: string;
                           guid: string) {.gcsafe, locks: 0.}
proc parseInputEvent*(event: InputEvent) {.gcsafe, locks: 0.}
proc removeJoyMapping*(guid: string) {.gcsafe, locks: 0.}
proc setAccelerometer*(value: Vector3) {.gcsafe, locks: 0.}
...
proc setGravity*(value: Vector3) {.gcsafe, locks: 0.}
proc setGyroscope*(value: Vector3) {.gcsafe, locks: 0.}
proc setMagnetometer*(value: Vector3) {.gcsafe, locks: 0.}
...
proc actionPress(action: string; strength: float64 = 1.0) =
  ...
  if isNil(inputActionPressMethodBind):
    inputActionPressMethodBind = getMethod(cstring"Input",
        cstring"action_press")
  var
...
var inputFlushBufferedEventsMethodBind {.threadvar.}: ptr GodotMethodBind
proc flushBufferedEvents() =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputFlushBufferedEventsMethodBind):
    inputFlushBufferedEventsMethodBind = getMethod(cstring"Input",
        cstring"flush_buffered_events")
  var ptrCallRet: pointer
  inputFlushBufferedEventsMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
var inputGetActionRawStrengthMethodBind {.threadvar.}: ptr GodotMethodBind
proc getActionRawStrength(action: string; exact: bool = false): float64 =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputGetActionRawStrengthMethodBind):
    inputGetActionRawStrengthMethodBind = getMethod(cstring"Input",
        cstring"get_action_raw_strength")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(action)
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputGetActionRawStrengthMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

var inputGetActionStrengthMethodBind {.threadvar.}: ptr GodotMethodBind
proc getActionStrength(action: string; exact: bool = false): float64 =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
...
var inputGetAxisMethodBind {.threadvar.}: ptr GodotMethodBind
proc getAxis(negativeAction: string; positiveAction: string): float64 =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputGetAxisMethodBind):
    inputGetAxisMethodBind = getMethod(cstring"Input", cstring"get_axis")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(negativeAction)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var argToPassToGodot1 = toGodotString(positiveAction)
  argsToPassToGodot[][1] = unsafeAddr(argToPassToGodot1)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputGetAxisMethodBind.ptrCall(self.godotObject, argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc getGyroscope(): Vector3 =
  ...
  if isNil(inputGetGyroscopeMethodBind):
    inputGetGyroscopeMethodBind = getMethod(cstring"Input",
        cstring"get_gyroscope")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
var inputGetVectorMethodBind {.threadvar.}: ptr GodotMethodBind
proc getVector(negativeX: string; positiveX: string; negativeY: string;
               positiveY: string; deadzone: float64 = -1.0): Vector2 =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputGetVectorMethodBind):
    inputGetVectorMethodBind = getMethod(cstring"Input", cstring"get_vector")
  var
    argsStatic: array[5, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(negativeX)
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var argToPassToGodot1 = toGodotString(positiveX)
  argsToPassToGodot[][1] = unsafeAddr(argToPassToGodot1)
  var argToPassToGodot2 = toGodotString(negativeY)
  argsToPassToGodot[][2] = unsafeAddr(argToPassToGodot2)
  var argToPassToGodot3 = toGodotString(positiveY)
  argsToPassToGodot[][3] = unsafeAddr(argToPassToGodot3)
  argsToPassToGodot[][4] = unsafeAddr(deadzone)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputGetVectorMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)
...
proc isActionJustPressed(action: string; exact: bool = false): bool =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputIsActionJustPressedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

var inputIsActionJustReleasedMethodBind {.threadvar.}: ptr GodotMethodBind
proc isActionJustReleased(action: string; exact: bool = false): bool =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
...
proc isActionPressed(action: string; exact: bool = false): bool =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
...
var inputIsPhysicalKeyPressedMethodBind {.threadvar.}: ptr GodotMethodBind
proc isPhysicalKeyPressed(scancode: int64): bool =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputIsPhysicalKeyPressedMethodBind):
    inputIsPhysicalKeyPressedMethodBind = getMethod(cstring"Input",
        cstring"is_physical_key_pressed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(scancode)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputIsPhysicalKeyPressedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var inputSetAccelerometerMethodBind {.threadvar.}: ptr GodotMethodBind
proc setAccelerometer(value: Vector3) =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputSetAccelerometerMethodBind):
    inputSetAccelerometerMethodBind = getMethod(cstring"Input",
        cstring"set_accelerometer")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  inputSetAccelerometerMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
var inputSetGravityMethodBind {.threadvar.}: ptr GodotMethodBind
proc setGravity(value: Vector3) =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputSetGravityMethodBind):
    inputSetGravityMethodBind = getMethod(cstring"Input", cstring"set_gravity")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  inputSetGravityMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                    ptrCallRet)

var inputSetGyroscopeMethodBind {.threadvar.}: ptr GodotMethodBind
proc setGyroscope(value: Vector3) =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputSetGyroscopeMethodBind):
    inputSetGyroscopeMethodBind = getMethod(cstring"Input",
        cstring"set_gyroscope")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  inputSetGyroscopeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                      ptrCallRet)

var inputSetMagnetometerMethodBind {.threadvar.}: ptr GodotMethodBind
proc setMagnetometer(value: Vector3) =
  if isNil(singletonInput):
    singletonInput = getSingleton[Input]()
  let self = singletonInput
  if isNil(inputSetMagnetometerMethodBind):
    inputSetMagnetometerMethodBind = getMethod(cstring"Input",
        cstring"set_magnetometer")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  inputSetMagnetometerMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)

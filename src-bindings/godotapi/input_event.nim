...
proc accumulate*(self: InputEvent; withEvent: InputEvent): bool {.gcsafe,
    locks: 0.}
proc asText*(self: InputEvent): string {.gcsafe, locks: 0.}
proc getActionStrength*(self: InputEvent; action: string;
                        exactMatch: bool = false): float64 {.gcsafe, locks: 0.}
proc isAction*(self: InputEvent; action: string; exactMatch: bool = false): bool {.
    gcsafe, locks: 0.}
proc isActionPressed*(self: InputEvent; action: string; allowEcho: bool = false;
                      exactMatch: bool = false): bool {.gcsafe, locks: 0.}
proc isActionReleased*(self: InputEvent; action: string;
                       exactMatch: bool = false): bool {.gcsafe, locks: 0.}
...
proc shortcutMatch*(self: InputEvent; event: InputEvent; exactMatch: bool = true): bool {.
    gcsafe, locks: 0.}
proc xformedBy*(self: InputEvent; xform: Transform2D;
                localOfs: Vector2 = vec2(0.0, 0.0)): InputEvent {.gcsafe,
    locks: 0.}
...
proc asText(self: InputEvent): string =
  if isNil(inputEventAsTextMethodBind):
    inputEventAsTextMethodBind = getMethod(cstring"InputEvent",
        cstring"as_text")
  var ptrCallRet: pointer
...
proc getActionStrength(self: InputEvent; action: string;
  ...
  argsToPassToGodot[][1] = unsafeAddr(exactMatch)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc isActionPressed(self: InputEvent; action: string; allowEcho: bool = false;
  ...
  argsToPassToGodot[][2] = unsafeAddr(exactMatch)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc isActionReleased(self: InputEvent; action: string; exactMatch: bool = false): bool =
...
proc isEcho(self: InputEvent): bool =
  if isNil(inputEventIsEchoMethodBind):
    inputEventIsEchoMethodBind = getMethod(cstring"InputEvent",
        cstring"is_echo")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

...
proc strength(self: InputEventAction): float64 =
  if isNil(inputEventActionGetStrengthMethodBind):
    inputEventActionGetStrengthMethodBind = getMethod(
        cstring"InputEventAction", cstring"get_strength")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputEventActionGetStrengthMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `strength=`(self: InputEventAction; val: float64) =
  if isNil(inputEventActionSetStrengthMethodBind):
    inputEventActionSetStrengthMethodBind = getMethod(
        cstring"InputEventAction", cstring"set_strength")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

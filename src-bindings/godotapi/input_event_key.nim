...
proc physicalScancode*(self: InputEventKey): int64 {.gcsafe, locks: 0.}
proc `physicalScancode=`*(self: InputEventKey; val: int64) {.gcsafe, locks: 0.}
...
proc getPhysicalScancodeWithModifiers*(self: InputEventKey): int64 {.gcsafe,
    locks: 0.}
...
var inputEventKeyGetPhysicalScancodeMethodBind {.threadvar.}: ptr GodotMethodBind
proc physicalScancode(self: InputEventKey): int64 =
  if isNil(inputEventKeyGetPhysicalScancodeMethodBind):
    inputEventKeyGetPhysicalScancodeMethodBind = getMethod(
        cstring"InputEventKey", cstring"get_physical_scancode")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputEventKeyGetPhysicalScancodeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var inputEventKeySetPhysicalScancodeMethodBind {.threadvar.}: ptr GodotMethodBind
proc `physicalScancode=`(self: InputEventKey; val: int64) =
  if isNil(inputEventKeySetPhysicalScancodeMethodBind):
    inputEventKeySetPhysicalScancodeMethodBind = getMethod(
        cstring"InputEventKey", cstring"set_physical_scancode")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  inputEventKeySetPhysicalScancodeMethodBind.ptrCall(self.godotObject,
...
proc `scancode=`(self: InputEventKey; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  inputEventKeySetScancodeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var inputEventKeyGetPhysicalScancodeWithModifiersMethodBind {.threadvar.}: ptr GodotMethodBind
proc getPhysicalScancodeWithModifiers(self: InputEventKey): int64 =
  if isNil(inputEventKeyGetPhysicalScancodeWithModifiersMethodBind):
    inputEventKeyGetPhysicalScancodeWithModifiersMethodBind = getMethod(
        cstring"InputEventKey", cstring"get_physical_scancode_with_modifiers")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputEventKeyGetPhysicalScancodeWithModifiersMethodBind.ptrCall(

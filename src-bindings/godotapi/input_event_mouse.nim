...
proc buttonMask(self: InputEventMouse): int64 =
  if isNil(inputEventMouseGetButtonMaskMethodBind):
    inputEventMouseGetButtonMaskMethodBind = getMethod(
        cstring"InputEventMouse", cstring"get_button_mask")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  inputEventMouseGetButtonMaskMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `buttonMask=`(self: InputEventMouse; val: int64) =
  if isNil(inputEventMouseSetButtonMaskMethodBind):
    inputEventMouseSetButtonMaskMethodBind = getMethod(
        cstring"InputEventMouse", cstring"set_button_mask")
  var

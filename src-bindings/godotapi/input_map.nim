...
proc actionGetDeadzone*(action: string): float64 {.gcsafe, locks: 0.}
...
proc eventIsAction*(event: InputEvent; action: string; exactMatch: bool = false): bool {.
    gcsafe, locks: 0.}
...
proc actionEraseEvent(action: string; event: InputEvent) =
  ...
  argsToPassToGodot[][1] = argToPassToGodot1
  var ptrCallRet: pointer
  inputMapActionEraseEventMethodBind.ptrCall(self.godotObject,
...
var inputMapActionGetDeadzoneMethodBind {.threadvar.}: ptr GodotMethodBind
proc actionGetDeadzone(action: string): float64 =
  if isNil(singletonInputMap):
    singletonInputMap = getSingleton[InputMap]()
  let self = singletonInputMap
  if isNil(inputMapActionGetDeadzoneMethodBind):
    inputMapActionGetDeadzoneMethodBind = getMethod(cstring"InputMap",
        cstring"action_get_deadzone")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(action)
  ...
  inputMapActionGetDeadzoneMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc addAction(action: string; deadzone: float64 = 0.5) =
  ...
  if isNil(inputMapAddActionMethodBind):
    inputMapAddActionMethodBind = getMethod(cstring"InputMap",
        cstring"add_action")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc hasAction(action: string): bool =
  ...
  if isNil(inputMapHasActionMethodBind):
    inputMapHasActionMethodBind = getMethod(cstring"InputMap",
        cstring"has_action")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
proc speed(self: InterpolatedCamera): float64 =
  if isNil(interpolatedCameraGetSpeedMethodBind):
    interpolatedCameraGetSpeedMethodBind = getMethod(
        cstring"InterpolatedCamera", cstring"get_speed")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `speed=`(self: InterpolatedCamera; val: float64) =
  if isNil(interpolatedCameraSetSpeedMethodBind):
    interpolatedCameraSetSpeedMethodBind = getMethod(
        cstring"InterpolatedCamera", cstring"set_speed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

...
proc getResolveItemAddresses*(id: int64): Array {.gcsafe, locks: 0.}
proc getResolveItemStatus*(id: int64): int64 {.gcsafe, locks: 0.}
proc resolveHostname*(host: string; ipType: int64 = 3'i64): string {.gcsafe,
    locks: 0.}
proc resolveHostnameAddresses*(host: string; ipType: int64 = 3'i64): Array {.
    gcsafe, locks: 0.}
...
var iPGetResolveItemAddressesMethodBind {.threadvar.}: ptr GodotMethodBind
proc getResolveItemAddresses(id: int64): Array =
  if isNil(singletonIP):
    singletonIP = getSingleton[IP]()
  let self = singletonIP
  if isNil(iPGetResolveItemAddressesMethodBind):
    iPGetResolveItemAddressesMethodBind = getMethod(cstring"IP",
        cstring"get_resolve_item_addresses")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(id)
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  iPGetResolveItemAddressesMethodBind.ptrCall(self.godotObject,
...
proc resolveHostname(host: string; ipType: int64 = 3'i64): string =
  ...
  if isNil(iPResolveHostnameMethodBind):
    iPResolveHostnameMethodBind = getMethod(cstring"IP",
        cstring"resolve_hostname")
  var
...
var iPResolveHostnameAddressesMethodBind {.threadvar.}: ptr GodotMethodBind
proc resolveHostnameAddresses(host: string; ipType: int64 = 3'i64): Array =
  if isNil(singletonIP):
    singletonIP = getSingleton[IP]()
  let self = singletonIP
  if isNil(iPResolveHostnameAddressesMethodBind):
    iPResolveHostnameAddressesMethodBind = getMethod(cstring"IP",
        cstring"resolve_hostname_addresses")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(host)
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  iPResolveHostnameAddressesMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  result = newArray(ptrCallVal)

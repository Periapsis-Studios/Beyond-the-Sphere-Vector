...
method bodyExitTree*(self: Joint) {.gcsafe, locks: 0, base.}
...
proc `nodesnodeA=`(self: Joint; val: NodePath) =
  ...
  var ptrCallRet: pointer
  jointSetNodeAMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc `nodesnodeB=`(self: Joint; val: NodePath) =
  ...
  argsToPassToGodot[][0] = val.godotNodePath
  var ptrCallRet: pointer
  jointSetNodeBMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
var jointUnderscorebodyExitTreeMethodBind {.threadvar.}: ptr GodotMethodBind
method bodyExitTree(self: Joint) =
  if isNil(jointUnderscorebodyExitTreeMethodBind):
    jointUnderscorebodyExitTreeMethodBind = getMethod(cstring"Joint",
        cstring"_body_exit_tree")
  var ptrCallRet: pointer
  jointUnderscorebodyExitTreeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
method bodyExitTree*(self: Joint2D) {.gcsafe, locks: 0, base.}
...
proc `bias=`(self: Joint2D; val: float64) =
  ...
  var ptrCallRet: pointer
  joint2DSetBiasMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
var joint2DUnderscorebodyExitTreeMethodBind {.threadvar.}: ptr GodotMethodBind
method bodyExitTree(self: Joint2D) =
  if isNil(joint2DUnderscorebodyExitTreeMethodBind):
    joint2DUnderscorebodyExitTreeMethodBind = getMethod(cstring"Joint2D",
        cstring"_body_exit_tree")
  var ptrCallRet: pointer
  joint2DUnderscorebodyExitTreeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

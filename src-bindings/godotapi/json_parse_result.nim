...
proc errorLine(self: JSONParseResult): int64 =
  ...
  jSONParseResultGetErrorLineMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc errorString(self: JSONParseResult): string =
  if isNil(jSONParseResultGetErrorStringMethodBind):
    jSONParseResultGetErrorStringMethodBind = getMethod(
        cstring"JSONParseResult", cstring"get_error_string")
  var ptrCallRet: pointer
...
proc `errorString=`(self: JSONParseResult; val: string) =
  if isNil(jSONParseResultSetErrorStringMethodBind):
    jSONParseResultSetErrorStringMethodBind = getMethod(
        cstring"JSONParseResult", cstring"set_error_string")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `resultVal=`(self: JSONParseResult; val: Variant) =
  ...
  var ptrCallRet: pointer
  jSONParseResultSetResultMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

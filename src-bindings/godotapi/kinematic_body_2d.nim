...
proc `collisionsafeMargin=`*(self: KinematicBody2D; val: float64) {.gcsafe,
    locks: 0.}
proc motionsyncToPhysics*(self: KinematicBody2D): bool {.gcsafe, locks: 0.}
proc `motionsyncToPhysics=`*(self: KinematicBody2D; val: bool) {.gcsafe,
    locks: 0.}
method directStateChanged*(self: KinematicBody2D; arg0: Object) {.gcsafe,
    locks: 0, base.}
proc getFloorAngle*(self: KinematicBody2D;
                    upDirection: Vector2 = vec2(0.0, -1.0)): float64 {.gcsafe,
    locks: 0.}
...
proc getLastSlideCollision*(self: KinematicBody2D): KinematicCollision2D {.
    gcsafe, locks: 0.}
...
proc moveAndCollide*(self: KinematicBody2D; relVec: Vector2;
                     ...
                     excludeRaycastShapes: bool = true; testOnly: bool = false): KinematicCollision2D {.
    gcsafe, locks: 0.}
proc moveAndSlide*(self: KinematicBody2D; linearVelocity: Vector2;
                   upDirection: Vector2 = vec2(0.0, 0.0);
...
proc moveAndSlideWithSnap*(self: KinematicBody2D; linearVelocity: Vector2;
                           ...
                           stopOnSlope: bool = false; maxSlides: int64 = 4'i64;
                           floorMaxAngle: float64 = 0.785398;
                           infiniteInertia: bool = true): Vector2 {.gcsafe,
    locks: 0.}
...
proc collisionsafeMargin(self: KinematicBody2D): float64 =
  if isNil(kinematicBody2DGetSafeMarginMethodBind):
    kinematicBody2DGetSafeMarginMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"get_safe_margin")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  kinematicBody2DGetSafeMarginMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `collisionsafeMargin=`(self: KinematicBody2D; val: float64) =
  if isNil(kinematicBody2DSetSafeMarginMethodBind):
    kinematicBody2DSetSafeMarginMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"set_safe_margin")
  var
...
var kinematicBody2DGetFloorAngleMethodBind {.threadvar.}: ptr GodotMethodBind
proc getFloorAngle(self: KinematicBody2D; upDirection: Vector2 = vec2(0.0, -1.0)): float64 =
  if isNil(kinematicBody2DGetFloorAngleMethodBind):
    kinematicBody2DGetFloorAngleMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"get_floor_angle")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  kinematicBody2DGetFloorAngleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getFloorNormal(self: KinematicBody2D): Vector2 =
  if isNil(kinematicBody2DGetFloorNormalMethodBind):
    kinematicBody2DGetFloorNormalMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"get_floor_normal")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
var kinematicBody2DGetLastSlideCollisionMethodBind {.threadvar.}: ptr GodotMethodBind
proc getLastSlideCollision(self: KinematicBody2D): KinematicCollision2D =
  if isNil(kinematicBody2DGetLastSlideCollisionMethodBind):
    kinematicBody2DGetLastSlideCollisionMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"get_last_slide_collision")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  kinematicBody2DGetLastSlideCollisionMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getSlideCount(self: KinematicBody2D): int64 =
  if isNil(kinematicBody2DGetSlideCountMethodBind):
    kinematicBody2DGetSlideCountMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"get_slide_count")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  kinematicBody2DGetSlideCountMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc moveAndCollide(self: KinematicBody2D; relVec: Vector2;
  ...
  if isNil(kinematicBody2DMoveAndCollideMethodBind):
    kinematicBody2DMoveAndCollideMethodBind = getMethod(
        cstring"KinematicBody2D", cstring"move_and_collide")
  var

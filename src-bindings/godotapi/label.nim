...
proc `align=`(self: Label; val: int64) =
  ...
  var ptrCallRet: pointer
  labelSetAlignMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc autowrap(self: Label): bool =
  if isNil(labelHasAutowrapMethodBind):
    labelHasAutowrapMethodBind = getMethod(cstring"Label",
...
proc `autowrap=`(self: Label; val: bool) =
  if isNil(labelSetAutowrapMethodBind):
    labelSetAutowrapMethodBind = getMethod(cstring"Label",
...
proc `clipText=`(self: Label; val: bool) =
  if isNil(labelSetClipTextMethodBind):
    labelSetClipTextMethodBind = getMethod(cstring"Label",
...
proc uppercase(self: Label): bool =
  if isNil(labelIsUppercaseMethodBind):
    labelIsUppercaseMethodBind = getMethod(cstring"Label",
        cstring"is_uppercase")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `uppercase=`(self: Label; val: bool) =
  if isNil(labelSetUppercaseMethodBind):
    labelSetUppercaseMethodBind = getMethod(cstring"Label",
        cstring"set_uppercase")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc `valign=`(self: Label; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  labelSetValignMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc getTotalCharacterCount(self: Label): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  labelGetTotalCharacterCountMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

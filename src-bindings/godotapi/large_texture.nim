...
proc getPieceTexture*(self: LargeTexture; idx: int64): Texture {.gcsafe,
    locks: 0.}
proc setPieceOffset*(self: LargeTexture; idx: int64; ofs: Vector2) {.gcsafe,
    locks: 0.}
...
proc clear(self: LargeTexture) =
  if isNil(largeTextureClearMethodBind):
    largeTextureClearMethodBind = getMethod(cstring"LargeTexture",
        cstring"clear")
  var ptrCallRet: pointer

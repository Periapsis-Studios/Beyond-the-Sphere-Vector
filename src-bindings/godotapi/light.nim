...
const
  ...
  BAKE_DISABLED* = 0'i64
  BAKE_INDIRECT* = 1'i64
  PARAM_ATTENUATION* = 5'i64
  PARAM_CONTACT_SHADOW_SIZE* = 8'i64
  PARAM_ENERGY* = 0'i64
  PARAM_INDIRECT_ENERGY* = 1'i64
  PARAM_MAX* = 16'i64
  PARAM_RANGE* = 4'i64
  PARAM_SHADOW_BIAS* = 14'i64
  PARAM_SHADOW_BIAS_SPLIT_SCALE* = 15'i64
  PARAM_SHADOW_MAX_DISTANCE* = 9'i64
  PARAM_SHADOW_NORMAL_BIAS* = 13'i64
  PARAM_SHADOW_SPLIT_1_OFFSET* = 10'i64
  PARAM_SHADOW_SPLIT_2_OFFSET* = 11'i64
  PARAM_SHADOW_SPLIT_3_OFFSET* = 12'i64
  PARAM_SIZE* = 2'i64
  PARAM_SPECULAR* = 3'i64
  PARAM_SPOT_ANGLE* = 6'i64
  PARAM_SPOT_ATTENUATION* = 7'i64
...
proc lightSize*(self: Light): float64 {.gcsafe, locks: 0.}
proc `lightSize=`*(self: Light; val: float64) {.gcsafe, locks: 0.}
...
proc lightBakeMode(self: Light): int64 =
  if isNil(lightGetBakeModeMethodBind):
    lightGetBakeModeMethodBind = getMethod(cstring"Light",
...
proc `lightBakeMode=`(self: Light; val: int64) =
  if isNil(lightSetBakeModeMethodBind):
    lightSetBakeModeMethodBind = getMethod(cstring"Light",
...
proc `lightColor=`(self: Light; val: Color) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  lightSetColorMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc lightCullMask(self: Light): int64 =
  if isNil(lightGetCullMaskMethodBind):
    lightGetCullMaskMethodBind = getMethod(cstring"Light",
...
proc `lightCullMask=`(self: Light; val: int64) =
  if isNil(lightSetCullMaskMethodBind):
    lightSetCullMaskMethodBind = getMethod(cstring"Light",
...
proc lightEnergy(self: Light): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lightGetParamMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc `lightEnergy=`(self: Light; val: float64) =
  ...
  var ptrCallRet: pointer
  lightSetParamMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc `lightNegative=`(self: Light; val: bool) =
  if isNil(lightSetNegativeMethodBind):
    lightSetNegativeMethodBind = getMethod(cstring"Light",
        cstring"set_negative")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc lightSize(self: Light): float64 =
  if isNil(lightGetParamMethodBind):
    lightGetParamMethodBind = getMethod(cstring"Light", cstring"get_param")
  var
...
proc `lightSize=`(self: Light; val: float64) =
  if isNil(lightSetParamMethodBind):
    lightSetParamMethodBind = getMethod(cstring"Light", cstring"set_param")
  var
    godotBoundArg0 = 2'i64
...
proc `shadowEnabled=`(self: Light; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  lightSetShadowMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

...
proc enabled(self: Light2D): bool =
  if isNil(light2DIsEnabledMethodBind):
    light2DIsEnabledMethodBind = getMethod(cstring"Light2D",
        cstring"is_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `enabled=`(self: Light2D; val: bool) =
  if isNil(light2DSetEnabledMethodBind):
    light2DSetEnabledMethodBind = getMethod(cstring"Light2D",
        cstring"set_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc energy(self: Light2D): float64 =
  if isNil(light2DGetEnergyMethodBind):
    light2DGetEnergyMethodBind = getMethod(cstring"Light2D",
        cstring"get_energy")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `energy=`(self: Light2D; val: float64) =
  if isNil(light2DSetEnergyMethodBind):
    light2DSetEnergyMethodBind = getMethod(cstring"Light2D",
        cstring"set_energy")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `mode=`(self: Light2D; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  light2DSetModeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc rangeHeight(self: Light2D): float64 =
  if isNil(light2DGetHeightMethodBind):
    light2DGetHeightMethodBind = getMethod(cstring"Light2D",
        cstring"get_height")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `rangeHeight=`(self: Light2D; val: float64) =
  if isNil(light2DSetHeightMethodBind):
    light2DSetHeightMethodBind = getMethod(cstring"Light2D",
        cstring"set_height")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc shadowItemCullMask(self: Light2D): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  light2DGetItemShadowCullMaskMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc texture(self: Light2D): Texture =
  if isNil(light2DGetTextureMethodBind):
    light2DGetTextureMethodBind = getMethod(cstring"Light2D",
        cstring"get_texture")
  var ptrCallRet: pointer
...
proc `texture=`(self: Light2D; val: Texture) =
  if isNil(light2DSetTextureMethodBind):
    light2DSetTextureMethodBind = getMethod(cstring"Light2D",
        cstring"set_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

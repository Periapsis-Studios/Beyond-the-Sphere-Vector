...
proc setPointPosition*(self: Line2D; i: int64; position: Vector2) {.gcsafe,
    locks: 0.}
...
proc gradient(self: Line2D): Gradient =
  if isNil(line2DGetGradientMethodBind):
    line2DGetGradientMethodBind = getMethod(cstring"Line2D",
...
proc `gradient=`(self: Line2D; val: Gradient) =
  if isNil(line2DSetGradientMethodBind):
    line2DSetGradientMethodBind = getMethod(cstring"Line2D",
...
proc texture(self: Line2D): Texture =
  if isNil(line2DGetTextureMethodBind):
    line2DGetTextureMethodBind = getMethod(cstring"Line2D",
        cstring"get_texture")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc `texture=`(self: Line2D; val: Texture) =
  if isNil(line2DSetTextureMethodBind):
    line2DSetTextureMethodBind = getMethod(cstring"Line2D",
        cstring"set_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `width=`(self: Line2D; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  line2DSetWidthMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc `widthCurve=`(self: Line2D; val: Curve) =
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  line2DSetCurveMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
method curveChanged(self: Line2D) =
  ...
  var ptrCallRet: pointer
  line2DUnderscorecurveChangedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc addPoint(self: Line2D; position: Vector2; atPosition: int64 = -1'i64) =
  ...
  var ptrCallRet: pointer
  line2DAddPointMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc clearPoints(self: Line2D) =
  if isNil(line2DClearPointsMethodBind):
    line2DClearPointsMethodBind = getMethod(cstring"Line2D",
...
proc removePoint(self: Line2D; i: int64) =
  if isNil(line2DRemovePointMethodBind):
    line2DRemovePointMethodBind = getMethod(cstring"Line2D",

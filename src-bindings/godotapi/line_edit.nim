...
proc virtualKeyboardEnabled*(self: LineEdit): bool {.gcsafe, locks: 0.}
proc `virtualKeyboardEnabled=`*(self: LineEdit; val: bool) {.gcsafe, locks: 0.}
...
proc deleteText*(self: LineEdit; fromColumn: int64; toColumn: int64) {.gcsafe,
    locks: 0.}
proc deselect*(self: LineEdit) {.gcsafe, locks: 0.}
proc getMenu*(self: LineEdit): PopupMenu {.gcsafe, locks: 0.}
proc getScrollOffset*(self: LineEdit): int64 {.gcsafe, locks: 0.}
proc menuOption*(self: LineEdit; option: int64) {.gcsafe, locks: 0.}
proc select*(self: LineEdit; fromm: int64 = 0'i64; to: int64 = -1'i64) {.gcsafe,
    locks: 0.}
...
proc align(self: LineEdit): int64 =
  if isNil(lineEditGetAlignMethodBind):
    lineEditGetAlignMethodBind = getMethod(cstring"LineEdit",
        cstring"get_align")
...
proc `align=`(self: LineEdit; val: int64) =
  if isNil(lineEditSetAlignMethodBind):
    lineEditSetAlignMethodBind = getMethod(cstring"LineEdit",
        cstring"set_align")
  var
...
proc caretBlinkSpeed(self: LineEdit): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lineEditCursorGetBlinkSpeedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc clearButtonEnabled(self: LineEdit): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lineEditIsClearButtonEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc contextMenuEnabled(self: LineEdit): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lineEditIsContextMenuEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc placeholderAlpha(self: LineEdit): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lineEditGetPlaceholderAlphaMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc secret(self: LineEdit): bool =
  if isNil(lineEditIsSecretMethodBind):
    lineEditIsSecretMethodBind = getMethod(cstring"LineEdit",
        cstring"is_secret")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `secret=`(self: LineEdit; val: bool) =
  if isNil(lineEditSetSecretMethodBind):
    lineEditSetSecretMethodBind = getMethod(cstring"LineEdit",
        cstring"set_secret")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var lineEditIsVirtualKeyboardEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc virtualKeyboardEnabled(self: LineEdit): bool =
  if isNil(lineEditIsVirtualKeyboardEnabledMethodBind):
    lineEditIsVirtualKeyboardEnabledMethodBind = getMethod(cstring"LineEdit",
        cstring"is_virtual_keyboard_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lineEditIsVirtualKeyboardEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var lineEditSetVirtualKeyboardEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc `virtualKeyboardEnabled=`(self: LineEdit; val: bool) =
  if isNil(lineEditSetVirtualKeyboardEnabledMethodBind):
    lineEditSetVirtualKeyboardEnabledMethodBind = getMethod(cstring"LineEdit",
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  lineEditSetVirtualKeyboardEnabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var lineEditGetScrollOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc getScrollOffset(self: LineEdit): int64 =
  if isNil(lineEditGetScrollOffsetMethodBind):
    lineEditGetScrollOffsetMethodBind = getMethod(cstring"LineEdit",
        cstring"get_scroll_offset")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  lineEditGetScrollOffsetMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc select(self: LineEdit; fromm: int64 = 0'i64; to: int64 = -1'i64) =
  ...
  var ptrCallRet: pointer
  lineEditSelectMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc selectAll(self: LineEdit) =
  if isNil(lineEditSelectAllMethodBind):
    lineEditSelectAllMethodBind = getMethod(cstring"LineEdit",
        cstring"select_all")
  var ptrCallRet: pointer

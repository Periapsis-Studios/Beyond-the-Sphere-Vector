...
method globalMenuAction*(self: MainLoop; id: Variant; meta: Variant) {.gcsafe,
    locks: 0, base.}
...
method idle(self: MainLoop; delta: float64): bool =
  if isNil(mainLoopUnderscoreidleMethodBind):
    mainLoopUnderscoreidleMethodBind = getMethod(cstring"MainLoop",
        cstring"_idle")
  var
...
method initialize(self: MainLoop) =
  if isNil(mainLoopUnderscoreinitializeMethodBind):
    mainLoopUnderscoreinitializeMethodBind = getMethod(cstring"MainLoop",
        cstring"_initialize")
  var ptrCallRet: pointer
  mainLoopUnderscoreinitializeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc inputTextImpl(self: MainLoop; text: string) =
  if isNil(mainLoopInputTextMethodBind):
    mainLoopInputTextMethodBind = getMethod(cstring"MainLoop",
        cstring"input_text")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc iterationImpl(self: MainLoop; delta: float64): bool =
  if isNil(mainLoopIterationMethodBind):
    mainLoopIterationMethodBind = getMethod(cstring"MainLoop",
        cstring"iteration")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
proc createConvexShape*(self: Mesh; clean: bool = true; simplify: bool = false): Shape {.
    gcsafe, locks: 0.}
...
proc surfaceGetBlendShapeArrays*(self: Mesh; surfIdx: int64): Array {.gcsafe,
    locks: 0.}
proc surfaceGetMaterial*(self: Mesh; surfIdx: int64): Material {.gcsafe,
    locks: 0.}
...
proc createConvexShape(self: Mesh; clean: bool = true; simplify: bool = false): Shape =
  ...
  meshCreateConvexShapeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
proc createOutline(self: Mesh; margin: float64): Mesh =
  if isNil(meshCreateOutlineMethodBind):
    meshCreateOutlineMethodBind = getMethod(cstring"Mesh",
        cstring"create_outline")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

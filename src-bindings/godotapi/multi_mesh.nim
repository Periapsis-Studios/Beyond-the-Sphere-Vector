...
proc `customDataArray=`*(self: MultiMesh; val: PoolColorArray) {.gcsafe,
    locks: 0.}
...
proc `transform2dArray=`*(self: MultiMesh; val: PoolVector2Array) {.gcsafe,
    locks: 0.}
proc transformArray*(self: MultiMesh): PoolVector3Array {.gcsafe, locks: 0.}
proc `transformArray=`*(self: MultiMesh; val: PoolVector3Array) {.gcsafe,
    locks: 0.}
...
proc getInstanceColor*(self: MultiMesh; instance: int64): Color {.gcsafe,
    locks: 0.}
proc getInstanceCustomData*(self: MultiMesh; instance: int64): Color {.gcsafe,
    locks: 0.}
...
proc setInstanceCustomData*(self: MultiMesh; instance: int64; customData: Color) {.
    gcsafe, locks: 0.}
proc setInstanceTransform*(self: MultiMesh; instance: int64;
                           transform: Transform) {.gcsafe, locks: 0.}
proc setInstanceTransform2d*(self: MultiMesh; instance: int64;
...
proc customDataFormat(self: MultiMesh): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  multiMeshGetCustomDataFormatMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc mesh(self: MultiMesh): Mesh =
  if isNil(multiMeshGetMeshMethodBind):
    multiMeshGetMeshMethodBind = getMethod(cstring"MultiMesh",
        cstring"get_mesh")
  var ptrCallRet: pointer
...
proc `mesh=`(self: MultiMesh; val: Mesh) =
  if isNil(multiMeshSetMeshMethodBind):
    multiMeshSetMeshMethodBind = getMethod(cstring"MultiMesh",
        cstring"set_mesh")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc transform2dArray(self: MultiMesh): PoolVector2Array =
  ...
  ptrCallRet = addr(ptrCallVal)
  multiMeshUnderscoregetTransform2dArrayMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc transformFormat(self: MultiMesh): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  multiMeshGetTransformFormatMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getAabb(self: MultiMesh): AABB =
  if isNil(multiMeshGetAabbMethodBind):
    multiMeshGetAabbMethodBind = getMethod(cstring"MultiMesh",
        cstring"get_aabb")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

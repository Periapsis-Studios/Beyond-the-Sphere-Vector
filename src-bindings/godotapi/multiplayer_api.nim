...
proc networkPeer*(self: MultiplayerAPI): NetworkedMultiplayerPeer {.gcsafe,
    locks: 0.}
...
proc `refuseNewNetworkConnections=`*(self: MultiplayerAPI; val: bool) {.gcsafe,
    locks: 0.}
proc rootNode*(self: MultiplayerAPI): Node {.gcsafe, locks: 0.}
proc `rootNode=`*(self: MultiplayerAPI; val: Node) {.gcsafe, locks: 0.}
...
proc getNetworkConnectedPeers*(self: MultiplayerAPI): PoolIntArray {.gcsafe,
    locks: 0.}
...
proc networkPeer(self: MultiplayerAPI): NetworkedMultiplayerPeer =
  ...
  multiplayerAPIGetNetworkPeerMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var multiplayerAPIGetRootNodeMethodBind {.threadvar.}: ptr GodotMethodBind
proc rootNode(self: MultiplayerAPI): Node =
  if isNil(multiplayerAPIGetRootNodeMethodBind):
    multiplayerAPIGetRootNodeMethodBind = getMethod(cstring"MultiplayerAPI",
        cstring"get_root_node")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  multiplayerAPIGetRootNodeMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)

var multiplayerAPISetRootNodeMethodBind {.threadvar.}: ptr GodotMethodBind
proc `rootNode=`(self: MultiplayerAPI; val: Node) =
...
proc getNetworkConnectedPeers(self: MultiplayerAPI): PoolIntArray =
  ...
  ptrCallRet = addr(ptrCallVal)
  multiplayerAPIGetNetworkConnectedPeersMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc getRpcSenderId(self: MultiplayerAPI): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  multiplayerAPIGetRpcSenderIdMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc hasNetworkPeer(self: MultiplayerAPI): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  multiplayerAPIHasNetworkPeerMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc isNetworkServer(self: MultiplayerAPI): bool =
  if isNil(multiplayerAPIIsNetworkServerMethodBind):
    multiplayerAPIIsNetworkServerMethodBind = getMethod(
        cstring"MultiplayerAPI", cstring"is_network_server")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

...
proc getClosestPoint*(self: Navigation2D; toPoint: Vector2): Vector2 {.gcsafe,
    locks: 0.}
...
proc navpolyAdd*(self: Navigation2D; mesh: NavigationPolygon;
                 xform: Transform2D; owner: Object = nil): int64 {.gcsafe,
    locks: 0.}
...
proc getClosestPointOwner(self: Navigation2D; toPoint: Vector2): Object =
  if isNil(navigation2DGetClosestPointOwnerMethodBind):
    navigation2DGetClosestPointOwnerMethodBind = getMethod(
        cstring"Navigation2D", cstring"get_closest_point_owner")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(toPoint)
...
proc navpolySetTransform(self: Navigation2D; id: int64; xform: Transform2D) =
  if isNil(navigation2DNavpolySetTransformMethodBind):
    navigation2DNavpolySetTransformMethodBind = getMethod(
        cstring"Navigation2D", cstring"navpoly_set_transform")
  var

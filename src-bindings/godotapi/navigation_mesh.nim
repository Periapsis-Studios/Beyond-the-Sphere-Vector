...
const
  ...
  PARSED_GEOMETRY_MAX* = 3'i64
  PARSED_GEOMETRY_MESH_INSTANCES* = 0'i64
  PARSED_GEOMETRY_STATIC_COLLIDERS* = 1'i64
  SAMPLE_PARTITION_LAYERS* = 2'i64
  SAMPLE_PARTITION_MAX* = 3'i64
  SAMPLE_PARTITION_MONOTONE* = 1'i64
  SAMPLE_PARTITION_WATERSHED* = 0'i64
  SOURCE_GEOMETRY_GROUPS_EXPLICIT* = 2'i64
  SOURCE_GEOMETRY_GROUPS_WITH_CHILDREN* = 1'i64
  SOURCE_GEOMETRY_MAX* = 3'i64
  SOURCE_GEOMETRY_NAVMESH_CHILDREN* = 0'i64
...
proc `detailsampleDistance=`*(self: NavigationMesh; val: float64) {.gcsafe,
    locks: 0.}
proc detailsampleMaxError*(self: NavigationMesh): float64 {.gcsafe, locks: 0.}
proc `detailsampleMaxError=`*(self: NavigationMesh; val: float64) {.gcsafe,
    locks: 0.}
...
proc `filterlowHangingObstacles=`*(self: NavigationMesh; val: bool) {.gcsafe,
    locks: 0.}
proc geometrycollisionMask*(self: NavigationMesh): int64 {.gcsafe, locks: 0.}
proc `geometrycollisionMask=`*(self: NavigationMesh; val: int64) {.gcsafe,
    locks: 0.}
...
proc `geometrysourceGroupName=`*(self: NavigationMesh; val: string) {.gcsafe,
    locks: 0.}
proc polygonvertsPerPoly*(self: NavigationMesh): float64 {.gcsafe, locks: 0.}
proc `polygonvertsPerPoly=`*(self: NavigationMesh; val: float64) {.gcsafe,
    locks: 0.}
...
proc `vertices=`*(self: NavigationMesh; val: PoolVector3Array) {.gcsafe,
    locks: 0.}
...
proc getCollisionMaskBit*(self: NavigationMesh; bit: int64): bool {.gcsafe,
    locks: 0.}
proc getPolygon*(self: NavigationMesh; idx: int64): PoolIntArray {.gcsafe,
    locks: 0.}
...
proc agentheight(self: NavigationMesh): float64 =
  ...
  navigationMeshGetAgentHeightMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc agentmaxClimb(self: NavigationMesh): float64 =
  if isNil(navigationMeshGetAgentMaxClimbMethodBind):
    navigationMeshGetAgentMaxClimbMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_agent_max_climb")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `agentmaxClimb=`(self: NavigationMesh; val: float64) =
  if isNil(navigationMeshSetAgentMaxClimbMethodBind):
    navigationMeshSetAgentMaxClimbMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_agent_max_climb")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc agentmaxSlope(self: NavigationMesh): float64 =
  if isNil(navigationMeshGetAgentMaxSlopeMethodBind):
    navigationMeshGetAgentMaxSlopeMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_agent_max_slope")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `agentmaxSlope=`(self: NavigationMesh; val: float64) =
  if isNil(navigationMeshSetAgentMaxSlopeMethodBind):
    navigationMeshSetAgentMaxSlopeMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_agent_max_slope")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc agentradius(self: NavigationMesh): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  navigationMeshGetAgentRadiusMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc cellheight(self: NavigationMesh): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  navigationMeshGetCellHeightMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc edgemaxError(self: NavigationMesh): float64 =
  if isNil(navigationMeshGetEdgeMaxErrorMethodBind):
    navigationMeshGetEdgeMaxErrorMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_edge_max_error")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `edgemaxError=`(self: NavigationMesh; val: float64) =
  if isNil(navigationMeshSetEdgeMaxErrorMethodBind):
    navigationMeshSetEdgeMaxErrorMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_edge_max_error")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc edgemaxLength(self: NavigationMesh): float64 =
  if isNil(navigationMeshGetEdgeMaxLengthMethodBind):
    navigationMeshGetEdgeMaxLengthMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_edge_max_length")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `edgemaxLength=`(self: NavigationMesh; val: float64) =
  if isNil(navigationMeshSetEdgeMaxLengthMethodBind):
    navigationMeshSetEdgeMaxLengthMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_edge_max_length")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc geometrycollisionMask(self: NavigationMesh): int64 =
  if isNil(navigationMeshGetCollisionMaskMethodBind):
    navigationMeshGetCollisionMaskMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_collision_mask")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `geometrycollisionMask=`(self: NavigationMesh; val: int64) =
  if isNil(navigationMeshSetCollisionMaskMethodBind):
    navigationMeshSetCollisionMaskMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_collision_mask")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc polygonvertsPerPoly(self: NavigationMesh): float64 =
  if isNil(navigationMeshGetVertsPerPolyMethodBind):
    navigationMeshGetVertsPerPolyMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_verts_per_poly")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `polygonvertsPerPoly=`(self: NavigationMesh; val: float64) =
  if isNil(navigationMeshSetVertsPerPolyMethodBind):
    navigationMeshSetVertsPerPolyMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_verts_per_poly")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc regionminSize(self: NavigationMesh): float64 =
  if isNil(navigationMeshGetRegionMinSizeMethodBind):
    navigationMeshGetRegionMinSizeMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_region_min_size")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `regionminSize=`(self: NavigationMesh; val: float64) =
  if isNil(navigationMeshSetRegionMinSizeMethodBind):
    navigationMeshSetRegionMinSizeMethodBind = getMethod(
        cstring"NavigationMesh", cstring"set_region_min_size")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc addPolygon(self: NavigationMesh; polygon: PoolIntArray) =
  ...
  var ptrCallRet: pointer
  navigationMeshAddPolygonMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc clearPolygons(self: NavigationMesh) =
  ...
  var ptrCallRet: pointer
  navigationMeshClearPolygonsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getPolygon(self: NavigationMesh; idx: int64): PoolIntArray =
  ...
  ptrCallRet = addr(ptrCallVal)
  navigationMeshGetPolygonMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getPolygonCount(self: NavigationMesh): int64 =
  if isNil(navigationMeshGetPolygonCountMethodBind):
    navigationMeshGetPolygonCountMethodBind = getMethod(
        cstring"NavigationMesh", cstring"get_polygon_count")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

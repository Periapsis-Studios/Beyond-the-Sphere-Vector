...
proc refuseNewConnections*(self: NetworkedMultiplayerPeer): bool {.gcsafe,
    locks: 0.}
...
proc `transferMode=`*(self: NetworkedMultiplayerPeer; val: int64) {.gcsafe,
    locks: 0.}
proc getConnectionStatus*(self: NetworkedMultiplayerPeer): int64 {.gcsafe,
    locks: 0.}
...
proc setTargetPeer*(self: NetworkedMultiplayerPeer; id: int64) {.gcsafe,
    locks: 0.}
...
proc poll(self: NetworkedMultiplayerPeer) =
  ...
  var ptrCallRet: pointer
  networkedMultiplayerPeerPollMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

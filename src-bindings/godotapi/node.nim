...
const
  ...
  NOTIFICATION_POST_ENTER_TREE* = 27'i64
...
method unhandledKeyInput*(self: Node; event: InputEventKey) {.gcsafe, locks: 0,
    base.}
...
proc addToGroup*(self: Node; group: string; persistent: bool = false) {.gcsafe,
    locks: 0.}
proc canProcess*(self: Node): bool {.gcsafe, locks: 0.}
proc duplicate*(self: Node; flags: int64 = 15'i64): Node {.gcsafe, locks: 0.}
proc findNode*(self: Node; mask: string; recursive: bool = true;
               owned: bool = true): Node {.gcsafe, locks: 0.}
...
proc moveChild*(self: Node; childNode: Node; toPosition: int64) {.gcsafe,
    locks: 0.}
...
proc replaceBy*(self: Node; node: Node; keepData: bool = false) {.gcsafe,
    locks: 0.}
...
proc rpcId*(self: Node; peerId: int64; methodd: string;
...
proc rsetId*(self: Node; peerId: int64; property: string; value: Variant) {.
    gcsafe, locks: 0.}
proc rsetUnreliable*(self: Node; property: string; value: Variant) {.gcsafe,
    locks: 0.}
proc rsetUnreliableId*(self: Node; peerId: int64; property: string;
                       value: Variant) {.gcsafe, locks: 0.}
proc setDisplayFolded*(self: Node; fold: bool) {.gcsafe, locks: 0.}
proc setNetworkMaster*(self: Node; id: int64; recursive: bool = true) {.gcsafe,
    locks: 0.}
...
proc importPath(self: Node): NodePath =
  ...
  nodeUnderscoregetImportPathMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `customMultiplayer=`(self: Node; val: MultiplayerAPI) =
  ...
  var ptrCallRet: pointer
  nodeSetCustomMultiplayerMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc pauseMode(self: Node): int64 =
  if isNil(nodeGetPauseModeMethodBind):
    nodeGetPauseModeMethodBind = getMethod(cstring"Node",
        cstring"get_pause_mode")
  var ptrCallRet: pointer
...
proc `pauseMode=`(self: Node; val: int64) =
  if isNil(nodeSetPauseModeMethodBind):
    nodeSetPauseModeMethodBind = getMethod(cstring"Node",
        cstring"set_pause_mode")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method process(self: Node; delta: float64) =
  if isNil(nodeUnderscoreprocessMethodBind):
    nodeUnderscoreprocessMethodBind = getMethod(cstring"Node",
        cstring"_process")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc addToGroup(self: Node; group: string; persistent: bool = false) =
  ...
  var ptrCallRet: pointer
  nodeAddToGroupMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
...
proc duplicate(self: Node; flags: int64 = 15'i64): Node =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  nodeDuplicateMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc findParent(self: Node; mask: string): Node =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  nodeFindParentMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc getPathTo(self: Node; node: Node): NodePath =
  ...
  var ptrCallVal: GodotNodePath
  ptrCallRet = addr(ptrCallVal)
  nodeGetPathToMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  result = newNodePath(ptrCallVal)
...
proc isAParentOf(self: Node; node: Node): bool =
  if isNil(nodeIsAParentOfMethodBind):
    nodeIsAParentOfMethodBind = getMethod(cstring"Node",
        cstring"is_a_parent_of")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not node.isNil:
...
proc isInGroup(self: Node; group: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  nodeIsInGroupMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc isInsideTree(self: Node): bool =
  if isNil(nodeIsInsideTreeMethodBind):
    nodeIsInsideTreeMethodBind = getMethod(cstring"Node",
        cstring"is_inside_tree")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc isProcessing(self: Node): bool =
  if isNil(nodeIsProcessingMethodBind):
    nodeIsProcessingMethodBind = getMethod(cstring"Node",
        cstring"is_processing")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc moveChild(self: Node; childNode: Node; toPosition: int64) =
  ...
  var ptrCallRet: pointer
  nodeMoveChildMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc propagateCall(self: Node; methodd: string; args: Array = newArray();
  ...
  if isNil(nodePropagateCallMethodBind):
    nodePropagateCallMethodBind = getMethod(cstring"Node",
        cstring"propagate_call")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc replaceBy(self: Node; node: Node; keepData: bool = false) =
  ...
  var ptrCallRet: pointer
  nodeReplaceByMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc requestReady(self: Node) =
  if isNil(nodeRequestReadyMethodBind):
    nodeRequestReadyMethodBind = getMethod(cstring"Node",
        cstring"request_ready")
  var ptrCallRet: pointer
...
proc rpc(self: Node; methodd: string; variantArgs: varargs[Variant]): Variant =
  ...
  while idx < cint(1 + variantArgs.len):
    ...
    inc idx
  let callRet {.used.} = nodeRpcMethodBind.call(self.godotObject,
...
proc rpcConfig(self: Node; methodd: string; mode: int64) =
  ...
  var ptrCallRet: pointer
  nodeRpcConfigMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc rpcUnreliable(self: Node; methodd: string; variantArgs: varargs[Variant]): Variant =
  if isNil(nodeRpcUnreliableMethodBind):
    nodeRpcUnreliableMethodBind = getMethod(cstring"Node",
        cstring"rpc_unreliable")
  var
    callError: VariantCallError
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, ptr GodotVariant]](godotAlloc(
        cint(sizeof(ptr GodotVariant) * (cint(1 + variantArgs.len)))))
...
proc rsetConfig(self: Node; property: string; mode: int64) =
  ...
  argsToPassToGodot[][1] = unsafeAddr(mode)
  var ptrCallRet: pointer
  nodeRsetConfigMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)
...
proc setProcess(self: Node; enable: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(enable)
  var ptrCallRet: pointer
  nodeSetProcessMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

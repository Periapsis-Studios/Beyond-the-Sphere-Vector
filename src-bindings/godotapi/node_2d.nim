...
proc moveLocalX*(self: Node2D; delta: float64; scaled: bool = false) {.gcsafe,
    locks: 0.}
proc moveLocalY*(self: Node2D; delta: float64; scaled: bool = false) {.gcsafe,
    locks: 0.}
...
proc `globalTransform=`(self: Node2D; val: Transform2D) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  node2DSetGlobalTransformMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc position(self: Node2D): Vector2 =
  if isNil(node2DGetPositionMethodBind):
    node2DGetPositionMethodBind = getMethod(cstring"Node2D",
...
proc `position=`(self: Node2D; val: Vector2) =
  if isNil(node2DSetPositionMethodBind):
    node2DSetPositionMethodBind = getMethod(cstring"Node2D",
...
proc rotation(self: Node2D): float64 =
  if isNil(node2DGetRotationMethodBind):
    node2DGetRotationMethodBind = getMethod(cstring"Node2D",
...
proc `rotation=`(self: Node2D; val: float64) =
  if isNil(node2DSetRotationMethodBind):
    node2DSetRotationMethodBind = getMethod(cstring"Node2D",
...
proc `rotationDegrees=`(self: Node2D; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  node2DSetRotationDegreesMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `scale=`(self: Node2D; val: Vector2) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  node2DSetScaleMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc applyScale(self: Node2D; ratio: Vector2) =
  if isNil(node2DApplyScaleMethodBind):
    node2DApplyScaleMethodBind = getMethod(cstring"Node2D",
        cstring"apply_scale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getAngleTo(self: Node2D; point: Vector2): float64 =
  if isNil(node2DGetAngleToMethodBind):
    node2DGetAngleToMethodBind = getMethod(cstring"Node2D",
        cstring"get_angle_to")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc moveLocalX(self: Node2D; delta: float64; scaled: bool = false) =
  if isNil(node2DMoveLocalXMethodBind):
    node2DMoveLocalXMethodBind = getMethod(cstring"Node2D",
        cstring"move_local_x")
  var
...
proc moveLocalY(self: Node2D; delta: float64; scaled: bool = false) =
  if isNil(node2DMoveLocalYMethodBind):
    node2DMoveLocalYMethodBind = getMethod(cstring"Node2D",
        cstring"move_local_y")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(delta)
...
proc toGlobal(self: Node2D; localPoint: Vector2): Vector2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  node2DToGlobalMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc toLocal(self: Node2D; globalPoint: Vector2): Vector2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  node2DToLocalMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)

...
proc noiseOffset*(self: NoiseTexture): Vector2 {.gcsafe, locks: 0.}
proc `noiseOffset=`*(self: NoiseTexture; val: Vector2) {.gcsafe, locks: 0.}
...
proc bumpStrength(self: NoiseTexture): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  noiseTextureGetBumpStrengthMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var noiseTextureGetNoiseOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc noiseOffset(self: NoiseTexture): Vector2 =
  if isNil(noiseTextureGetNoiseOffsetMethodBind):
    noiseTextureGetNoiseOffsetMethodBind = getMethod(cstring"NoiseTexture",
        cstring"get_noise_offset")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  noiseTextureGetNoiseOffsetMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var noiseTextureSetNoiseOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc `noiseOffset=`(self: NoiseTexture; val: Vector2) =
  if isNil(noiseTextureSetNoiseOffsetMethodBind):
    noiseTextureSetNoiseOffsetMethodBind = getMethod(cstring"NoiseTexture",
        cstring"set_noise_offset")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  noiseTextureSetNoiseOffsetMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
method threadDone(self: NoiseTexture; image: Image) =
  if isNil(noiseTextureUnderscorethreadDoneMethodBind):
    noiseTextureUnderscorethreadDoneMethodBind = getMethod(
        cstring"NoiseTexture", cstring"_thread_done")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

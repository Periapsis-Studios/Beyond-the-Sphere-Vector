...
method set*(self: Object; property: string; value: Variant): bool {.gcsafe,
    locks: 0, base.}
...
proc callDeferred*(self: Object; methodd: string; variantArgs: varargs[Variant]) {.
    gcsafe, locks: 0.}
proc callv*(self: Object; methodd: string; argArray: Array): Variant {.gcsafe,
    locks: 0.}
...
proc connect*(self: Object; signal: string; target: Object; methodd: string;
              binds: Array = newArray(); flags: int64 = 0'i64): Error {.gcsafe,
    locks: 0.}
...
proc getSignalConnectionList*(self: Object; signal: string): Array {.gcsafe,
    locks: 0.}
...
proc setDeferred*(self: Object; property: string; value: Variant) {.gcsafe,
    locks: 0.}
proc setIndexed*(self: Object; property: NodePath; value: Variant) {.gcsafe,
    locks: 0.}
...
proc connect(self: Object; signal: string; target: Object; methodd: string;
  ...
  ptrCallRet = addr(ptrCallVal)
  objectConnectMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc emitSignal(self: Object; signal: string; variantArgs: varargs[Variant]) =
  if isNil(objectEmitSignalMethodBind):
    objectEmitSignalMethodBind = getMethod(cstring"Object",
        cstring"emit_signal")
  var
    callError: VariantCallError
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, ptr GodotVariant]](godotAlloc(
...
proc getIncomingConnections(self: Object): Array =
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  objectGetIncomingConnectionsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = newArray(ptrCallVal)
...
proc getIndexed(self: Object; property: NodePath): Variant =
  if isNil(objectGetIndexedMethodBind):
    objectGetIndexedMethodBind = getMethod(cstring"Object",
        cstring"get_indexed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getMeta(self: Object; name: string): Variant =
  ...
  initGodotVariant(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  objectGetMetaMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
  result = newVariant(ptrCallVal)
...
proc hasMeta(self: Object; name: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  objectHasMetaMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc isClass(self: Object; class: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  objectIsClassMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc isConnected(self: Object; signal: string; target: Object; methodd: string): bool =
  if isNil(objectIsConnectedMethodBind):
    objectIsConnectedMethodBind = getMethod(cstring"Object",
        cstring"is_connected")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(signal)
...
proc removeMeta(self: Object; name: string) =
  if isNil(objectRemoveMetaMethodBind):
    objectRemoveMetaMethodBind = getMethod(cstring"Object",
        cstring"remove_meta")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setDeferred(self: Object; property: string; value: Variant) =
  if isNil(objectSetDeferredMethodBind):
    objectSetDeferredMethodBind = getMethod(cstring"Object",
        cstring"set_deferred")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(property)
...
proc setIndexed(self: Object; property: NodePath; value: Variant) =
  if isNil(objectSetIndexedMethodBind):
    objectSetIndexedMethodBind = getMethod(cstring"Object",
        cstring"set_indexed")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setMeta(self: Object; name: string; value: Variant) =
  ...
  argsToPassToGodot[][1] = value.godotVariant
  var ptrCallRet: pointer
  objectSetMetaMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)

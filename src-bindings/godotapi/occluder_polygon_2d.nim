...
proc `polygon=`*(self: OccluderPolygon2D; val: PoolVector2Array) {.gcsafe,
    locks: 0.}
...
proc `closed=`(self: OccluderPolygon2D; val: bool) =
  if isNil(occluderPolygon2DSetClosedMethodBind):
    occluderPolygon2DSetClosedMethodBind = getMethod(
        cstring"OccluderPolygon2D", cstring"set_closed")
  var
...
proc cullMode(self: OccluderPolygon2D): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  occluderPolygon2DGetCullModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc polygon(self: OccluderPolygon2D): PoolVector2Array =
  if isNil(occluderPolygon2DGetPolygonMethodBind):
    occluderPolygon2DGetPolygonMethodBind = getMethod(
        cstring"OccluderPolygon2D", cstring"get_polygon")
  var ptrCallRet: pointer
  var ptrCallVal: GodotPoolVector2Array
  initGodotPoolVector2Array(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  occluderPolygon2DGetPolygonMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `polygon=`(self: OccluderPolygon2D; val: PoolVector2Array) =
  if isNil(occluderPolygon2DSetPolygonMethodBind):
    occluderPolygon2DSetPolygonMethodBind = getMethod(
        cstring"OccluderPolygon2D", cstring"set_polygon")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

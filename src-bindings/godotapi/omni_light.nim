...
proc omniAttenuation(self: OmniLight): float64 =
  if isNil(omniLightGetParamMethodBind):
    omniLightGetParamMethodBind = getMethod(cstring"OmniLight",
        cstring"get_param")
  var
...
proc `omniAttenuation=`(self: OmniLight; val: float64) =
  if isNil(omniLightSetParamMethodBind):
    omniLightSetParamMethodBind = getMethod(cstring"OmniLight",
        cstring"set_param")
  var
    godotBoundArg0 = 5'i64
...
proc `omniShadowDetail=`(self: OmniLight; val: int64) =
  ...
  var ptrCallRet: pointer
  omniLightSetShadowDetailMethodBind.ptrCall(self.godotObject,

...
proc getImage*(self: OpenSimplexNoise; width: int64; height: int64;
               noiseOffset: Vector2 = vec2(0.0, 0.0)): Image {.gcsafe, locks: 0.}
...
proc getNoise2d*(self: OpenSimplexNoise; x: float64; y: float64): float64 {.
    gcsafe, locks: 0.}
proc getNoise2dv*(self: OpenSimplexNoise; pos: Vector2): float64 {.gcsafe,
    locks: 0.}
proc getNoise3d*(self: OpenSimplexNoise; x: float64; y: float64; z: float64): float64 {.
    gcsafe, locks: 0.}
proc getNoise3dv*(self: OpenSimplexNoise; pos: Vector3): float64 {.gcsafe,
    locks: 0.}
proc getNoise4d*(self: OpenSimplexNoise; x: float64; y: float64; z: float64;
                 w: float64): float64 {.gcsafe, locks: 0.}
proc getSeamlessImage*(self: OpenSimplexNoise; size: int64): Image {.gcsafe,
    locks: 0.}
...
proc getImage(self: OpenSimplexNoise; width: int64; height: int64;
              noiseOffset: Vector2 = vec2(0.0, 0.0)): Image =
  ...
  argsToPassToGodot[][2] = unsafeAddr(noiseOffset)
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  openSimplexNoiseGetImageMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getNoise2dv(self: OpenSimplexNoise; pos: Vector2): float64 =
  if isNil(openSimplexNoiseGetNoise2dvMethodBind):
    openSimplexNoiseGetNoise2dvMethodBind = getMethod(
        cstring"OpenSimplexNoise", cstring"get_noise_2dv")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getNoise3dv(self: OpenSimplexNoise; pos: Vector3): float64 =
  if isNil(openSimplexNoiseGetNoise3dvMethodBind):
    openSimplexNoiseGetNoise3dvMethodBind = getMethod(
        cstring"OpenSimplexNoise", cstring"get_noise_3dv")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(pos)

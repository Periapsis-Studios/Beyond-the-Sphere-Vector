...
proc addItem*(self: OptionButton; label: string; id: int64 = -1'i64) {.gcsafe,
    locks: 0.}
...
proc getItemMetadata*(self: OptionButton; idx: int64): Variant {.gcsafe,
    locks: 0.}
...
proc setItemDisabled*(self: OptionButton; idx: int64; disabled: bool) {.gcsafe,
    locks: 0.}
proc setItemIcon*(self: OptionButton; idx: int64; texture: Texture) {.gcsafe,
    locks: 0.}
...
proc setItemMetadata*(self: OptionButton; idx: int64; metadata: Variant) {.
    gcsafe, locks: 0.}
proc setItemText*(self: OptionButton; idx: int64; text: string) {.gcsafe,
    locks: 0.}
...
method selectInt(self: OptionButton; arg0: int64) =
  if isNil(optionButtonUnderscoreselectIntMethodBind):
    optionButtonUnderscoreselectIntMethodBind = getMethod(
        cstring"OptionButton", cstring"_select_int")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(arg0)
...
proc clear(self: OptionButton) =
  if isNil(optionButtonClearMethodBind):
    optionButtonClearMethodBind = getMethod(cstring"OptionButton",
        cstring"clear")
  var ptrCallRet: pointer
...
proc getItemIndex(self: OptionButton; id: int64): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  optionButtonGetItemIndexMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getSelectedMetadata(self: OptionButton): Variant =
  if isNil(optionButtonGetSelectedMetadataMethodBind):
    optionButtonGetSelectedMetadataMethodBind = getMethod(
        cstring"OptionButton", cstring"get_selected_metadata")
  var ptrCallRet: pointer
  var ptrCallVal: GodotVariant
  initGodotVariant(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)

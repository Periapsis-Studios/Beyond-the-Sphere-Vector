...
proc getVar*(self: PacketPeer; allowObjects: bool = false): Variant {.gcsafe,
    locks: 0.}
proc putPacket*(self: PacketPeer; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
...
proc allowObjectDecoding(self: PacketPeer): bool =
  if isNil(packetPeerIsObjectDecodingAllowedMethodBind):
    packetPeerIsObjectDecodingAllowedMethodBind = getMethod(
        cstring"PacketPeer", cstring"is_object_decoding_allowed")
...
proc getAvailablePacketCount(self: PacketPeer): int64 =
  if isNil(packetPeerGetAvailablePacketCountMethodBind):
    packetPeerGetAvailablePacketCountMethodBind = getMethod(
        cstring"PacketPeer", cstring"get_available_packet_count")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getVar(self: PacketPeer; allowObjects: bool = false): Variant =
  if isNil(packetPeerGetVarMethodBind):
    packetPeerGetVarMethodBind = getMethod(cstring"PacketPeer",
        cstring"get_var")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc putVar(self: PacketPeer; varr: Variant; fullObjects: bool = false): Error =
  if isNil(packetPeerPutVarMethodBind):
    packetPeerPutVarMethodBind = getMethod(cstring"PacketPeer",
        cstring"put_var")
  var

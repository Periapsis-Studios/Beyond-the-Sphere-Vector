...
proc explosiveness(self: Particles2D): float64 =
  if isNil(particles2DGetExplosivenessRatioMethodBind):
    particles2DGetExplosivenessRatioMethodBind = getMethod(
        cstring"Particles2D", cstring"get_explosiveness_ratio")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `explosiveness=`(self: Particles2D; val: float64) =
  if isNil(particles2DSetExplosivenessRatioMethodBind):
    particles2DSetExplosivenessRatioMethodBind = getMethod(
        cstring"Particles2D", cstring"set_explosiveness_ratio")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc localCoords(self: Particles2D): bool =
  if isNil(particles2DGetUseLocalCoordinatesMethodBind):
    particles2DGetUseLocalCoordinatesMethodBind = getMethod(
        cstring"Particles2D", cstring"get_use_local_coordinates")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `localCoords=`(self: Particles2D; val: bool) =
  if isNil(particles2DSetUseLocalCoordinatesMethodBind):
    particles2DSetUseLocalCoordinatesMethodBind = getMethod(
        cstring"Particles2D", cstring"set_use_local_coordinates")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc preprocess(self: Particles2D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  particles2DGetPreProcessTimeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `speedScale=`(self: Particles2D; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  particles2DSetSpeedScaleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc visibilityRect(self: Particles2D): Rect2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  particles2DGetVisibilityRectMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

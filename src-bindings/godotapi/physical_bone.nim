...
method directStateChanged*(self: PhysicalBone; arg0: Object) {.gcsafe, locks: 0,
    base.}
proc applyCentralImpulse*(self: PhysicalBone; impulse: Vector3) {.gcsafe,
    locks: 0.}
...
proc gravityScale(self: PhysicalBone): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  physicalBoneGetGravityScaleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `jointType=`(self: PhysicalBone; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  physicalBoneSetJointTypeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc applyCentralImpulse(self: PhysicalBone; impulse: Vector3) =
  if isNil(physicalBoneApplyCentralImpulseMethodBind):
    physicalBoneApplyCentralImpulseMethodBind = getMethod(
        cstring"PhysicalBone", cstring"apply_central_impulse")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc applyImpulse(self: PhysicalBone; position: Vector3; impulse: Vector3) =
  ...
  var ptrCallRet: pointer
  physicalBoneApplyImpulseMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc isSimulatingPhysics(self: PhysicalBone): bool =
  if isNil(physicalBoneIsSimulatingPhysicsMethodBind):
    physicalBoneIsSimulatingPhysicsMethodBind = getMethod(
        cstring"PhysicalBone", cstring"is_simulating_physics")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

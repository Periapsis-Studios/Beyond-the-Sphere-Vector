...
proc centerOffset*(self: PlaneMesh): Vector3 {.gcsafe, locks: 0.}
proc `centerOffset=`*(self: PlaneMesh; val: Vector3) {.gcsafe, locks: 0.}
...
var planeMeshGetCenterOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc centerOffset(self: PlaneMesh): Vector3 =
  if isNil(planeMeshGetCenterOffsetMethodBind):
    planeMeshGetCenterOffsetMethodBind = getMethod(cstring"PlaneMesh",
        cstring"get_center_offset")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  planeMeshGetCenterOffsetMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var planeMeshSetCenterOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc `centerOffset=`(self: PlaneMesh; val: Vector3) =
  if isNil(planeMeshSetCenterOffsetMethodBind):
    planeMeshSetCenterOffsetMethodBind = getMethod(cstring"PlaneMesh",
  ...
  var ptrCallRet: pointer
  planeMeshSetCenterOffsetMethodBind.ptrCall(self.godotObject,
...
proc size(self: PlaneMesh): Vector2 =
  if isNil(planeMeshGetSizeMethodBind):
    planeMeshGetSizeMethodBind = getMethod(cstring"PlaneMesh",
        cstring"get_size")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `size=`(self: PlaneMesh; val: Vector2) =
  if isNil(planeMeshSetSizeMethodBind):
    planeMeshSetSizeMethodBind = getMethod(cstring"PlaneMesh",
        cstring"set_size")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
proc `textureRotationDegrees=`*(self: Polygon2D; val: float64) {.gcsafe,
    locks: 0.}
...
proc getBoneWeights*(self: Polygon2D; index: int64): PoolRealArray {.gcsafe,
    locks: 0.}
proc setBonePath*(self: Polygon2D; index: int64; path: NodePath) {.gcsafe,
    locks: 0.}
...
proc bones(self: Polygon2D): Array =
  ...
  polygon2DUnderscoregetBonesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc color(self: Polygon2D): Color =
  if isNil(polygon2DGetColorMethodBind):
    polygon2DGetColorMethodBind = getMethod(cstring"Polygon2D",
        cstring"get_color")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `color=`(self: Polygon2D; val: Color) =
  if isNil(polygon2DSetColorMethodBind):
    polygon2DSetColorMethodBind = getMethod(cstring"Polygon2D",
        cstring"set_color")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `invertBorder=`(self: Polygon2D; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  polygon2DSetInvertBorderMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc textureRotation(self: Polygon2D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  polygon2DGetTextureRotationMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc textureRotationDegrees(self: Polygon2D): float64 =
  if isNil(polygon2DGetTextureRotationDegreesMethodBind):
    polygon2DGetTextureRotationDegreesMethodBind = getMethod(
        cstring"Polygon2D", cstring"get_texture_rotation_degrees")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `textureRotationDegrees=`(self: Polygon2D; val: float64) =
  if isNil(polygon2DSetTextureRotationDegreesMethodBind):
    polygon2DSetTextureRotationDegreesMethodBind = getMethod(
        cstring"Polygon2D", cstring"set_texture_rotation_degrees")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc `textureScale=`(self: Polygon2D; val: Vector2) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  polygon2DSetTextureScaleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc `uv=`(self: Polygon2D; val: PoolVector2Array) =
  ...
  argsToPassToGodot[][0] = val.godotPoolVector2Array
  var ptrCallRet: pointer
  polygon2DSetUvMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc `vertexColors=`(self: Polygon2D; val: PoolColorArray) =
  ...
  var ptrCallRet: pointer
  polygon2DSetVertexColorsMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc addBone(self: Polygon2D; path: NodePath; weights: PoolRealArray) =
  if isNil(polygon2DAddBoneMethodBind):
    polygon2DAddBoneMethodBind = getMethod(cstring"Polygon2D",
        cstring"add_bone")
  var

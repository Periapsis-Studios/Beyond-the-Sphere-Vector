...
proc getIntersections*(self: PolygonPathFinder; fromm: Vector2; to: Vector2): PoolVector2Array {.
    gcsafe, locks: 0.}
proc getPointPenalty*(self: PolygonPathFinder; idx: int64): float64 {.gcsafe,
    locks: 0.}
proc isPointInside*(self: PolygonPathFinder; point: Vector2): bool {.gcsafe,
    locks: 0.}
...
proc getBounds(self: PolygonPathFinder): Rect2 =
  if isNil(polygonPathFinderGetBoundsMethodBind):
    polygonPathFinderGetBoundsMethodBind = getMethod(
        cstring"PolygonPathFinder", cstring"get_bounds")
  var ptrCallRet: pointer

...
proc popup*(self: Popup; bounds: Rect2 = initRect2(0.0, 0.0, 0.0, 0.0)) {.
    gcsafe, locks: 0.}
proc popupCentered*(self: Popup; size: Vector2 = vec2(0.0, 0.0)) {.gcsafe,
    locks: 0.}
...
proc popupExclusive(self: Popup): bool =
  if isNil(popupIsExclusiveMethodBind):
    popupIsExclusiveMethodBind = getMethod(cstring"Popup",
        cstring"is_exclusive")
...
proc `popupExclusive=`(self: Popup; val: bool) =
  if isNil(popupSetExclusiveMethodBind):
    popupSetExclusiveMethodBind = getMethod(cstring"Popup",
        cstring"set_exclusive")
  var

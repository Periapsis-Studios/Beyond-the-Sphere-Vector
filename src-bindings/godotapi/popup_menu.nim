...
proc `hideOnCheckableItemSelection=`*(self: PopupMenu; val: bool) {.gcsafe,
    locks: 0.}
...
proc addIconCheckItem*(self: PopupMenu; texture: Texture; label: string;
                       id: int64 = -1'i64; accel: int64 = 0'i64) {.gcsafe,
    locks: 0.}
proc addIconCheckShortcut*(self: PopupMenu; texture: Texture;
...
proc addIconShortcut*(self: PopupMenu; texture: Texture; shortcut: ShortCut;
                      id: int64 = -1'i64; global: bool = false) {.gcsafe,
    locks: 0.}
proc addItem*(self: PopupMenu; label: string; id: int64 = -1'i64;
              accel: int64 = 0'i64) {.gcsafe, locks: 0.}
...
proc addRadioCheckItem*(self: PopupMenu; label: string; id: int64 = -1'i64;
                        accel: int64 = 0'i64) {.gcsafe, locks: 0.}
proc addRadioCheckShortcut*(self: PopupMenu; shortcut: ShortCut;
                            id: int64 = -1'i64; global: bool = false) {.gcsafe,
    locks: 0.}
proc addSeparator*(self: PopupMenu; label: string = ""; id: int64 = -1'i64) {.
    gcsafe, locks: 0.}
...
proc isItemShortcutDisabled*(self: PopupMenu; idx: int64): bool {.gcsafe,
    locks: 0.}
...
proc setItemAccelerator*(self: PopupMenu; idx: int64; accel: int64) {.gcsafe,
    locks: 0.}
proc setItemAsCheckable*(self: PopupMenu; idx: int64; enable: bool) {.gcsafe,
    locks: 0.}
proc setItemAsRadioCheckable*(self: PopupMenu; idx: int64; enable: bool) {.
    gcsafe, locks: 0.}
proc setItemAsSeparator*(self: PopupMenu; idx: int64; enable: bool) {.gcsafe,
    locks: 0.}
proc setItemChecked*(self: PopupMenu; idx: int64; checked: bool) {.gcsafe,
    locks: 0.}
proc setItemDisabled*(self: PopupMenu; idx: int64; disabled: bool) {.gcsafe,
    locks: 0.}
...
proc setItemMetadata*(self: PopupMenu; idx: int64; metadata: Variant) {.gcsafe,
    locks: 0.}
proc setItemMultistate*(self: PopupMenu; idx: int64; state: int64) {.gcsafe,
    locks: 0.}
...
proc setItemShortcutDisabled*(self: PopupMenu; idx: int64; disabled: bool) {.
    gcsafe, locks: 0.}
proc setItemSubmenu*(self: PopupMenu; idx: int64; submenu: string) {.gcsafe,
    locks: 0.}
proc setItemText*(self: PopupMenu; idx: int64; text: string) {.gcsafe, locks: 0.}
proc setItemTooltip*(self: PopupMenu; idx: int64; tooltip: string) {.gcsafe,
    locks: 0.}
...
proc hideOnStateItemSelection(self: PopupMenu): bool =
  if isNil(popupMenuIsHideOnStateItemSelectionMethodBind):
    popupMenuIsHideOnStateItemSelectionMethodBind = getMethod(
        cstring"PopupMenu", cstring"is_hide_on_state_item_selection")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc items(self: PopupMenu): Array =
  ...
  popupMenuUnderscoregetItemsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc addIconRadioCheckShortcut(self: PopupMenu; texture: Texture;
  ...
  if isNil(popupMenuAddIconRadioCheckShortcutMethodBind):
    popupMenuAddIconRadioCheckShortcutMethodBind = getMethod(
        cstring"PopupMenu", cstring"add_icon_radio_check_shortcut")
  var
    argsStatic: array[4, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not texture.isNil:
...
proc addIconShortcut(self: PopupMenu; texture: Texture; shortcut: ShortCut;
  ...
  argsToPassToGodot[][3] = unsafeAddr(global)
  var ptrCallRet: pointer
  popupMenuAddIconShortcutMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc addItem(self: PopupMenu; label: string; id: int64 = -1'i64;
  ...
  if isNil(popupMenuAddItemMethodBind):
    popupMenuAddItemMethodBind = getMethod(cstring"PopupMenu",
        cstring"add_item")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getItemMetadata(self: PopupMenu; idx: int64): Variant =
  ...
  ptrCallRet = addr(ptrCallVal)
  popupMenuGetItemMetadataMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getItemShortcut(self: PopupMenu; idx: int64): ShortCut =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  popupMenuGetItemShortcutMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc isItemCheckable(self: PopupMenu; idx: int64): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  popupMenuIsItemCheckableMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc isItemSeparator(self: PopupMenu; idx: int64): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  popupMenuIsItemSeparatorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setItemDisabled(self: PopupMenu; idx: int64; disabled: bool) =
  ...
  var ptrCallRet: pointer
  popupMenuSetItemDisabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setItemMetadata(self: PopupMenu; idx: int64; metadata: Variant) =
  ...
  var ptrCallRet: pointer
  popupMenuSetItemMetadataMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setItemShortcut(self: PopupMenu; idx: int64; shortcut: ShortCut;
  ...
  argsToPassToGodot[][2] = unsafeAddr(global)
  var ptrCallRet: pointer
  popupMenuSetItemShortcutMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

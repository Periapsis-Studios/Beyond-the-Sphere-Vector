...
proc size(self: PrismMesh): Vector3 =
  if isNil(prismMeshGetSizeMethodBind):
    prismMeshGetSizeMethodBind = getMethod(cstring"PrismMesh",
        cstring"get_size")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `size=`(self: PrismMesh; val: Vector3) =
  if isNil(prismMeshSetSizeMethodBind):
    prismMeshSetSizeMethodBind = getMethod(cstring"PrismMesh",
        cstring"set_size")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc subdivideHeight(self: PrismMesh): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  prismMeshGetSubdivideHeightMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

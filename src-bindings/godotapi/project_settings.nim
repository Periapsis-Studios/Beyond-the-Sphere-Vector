...
proc loadResourcePack*(pack: string; replaceFiles: bool = true;
                       offset: int64 = 0'i64): bool {.gcsafe, locks: 0.}
...
proc globalizePath(path: string): string =
  ...
  if isNil(projectSettingsGlobalizePathMethodBind):
    projectSettingsGlobalizePathMethodBind = getMethod(
        cstring"ProjectSettings", cstring"globalize_path")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

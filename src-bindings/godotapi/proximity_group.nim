...
method proximityGroupBroadcast*(self: ProximityGroup; methodd: string;
                                parameters: Variant) {.gcsafe, locks: 0, base.}
proc broadcast*(self: ProximityGroup; methodd: string; parameters: Variant) {.
    gcsafe, locks: 0.}
...
proc dispatchMode(self: ProximityGroup): int64 =
  if isNil(proximityGroupGetDispatchModeMethodBind):
    proximityGroupGetDispatchModeMethodBind = getMethod(
        cstring"ProximityGroup", cstring"get_dispatch_mode")
...
proc `dispatchMode=`(self: ProximityGroup; val: int64) =
  if isNil(proximityGroupSetDispatchModeMethodBind):
    proximityGroupSetDispatchModeMethodBind = getMethod(
        cstring"ProximityGroup", cstring"set_dispatch_mode")
  var
...
proc gridRadius(self: ProximityGroup): Vector3 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  proximityGroupGetGridRadiusMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
proc state*(self: RandomNumberGenerator): int64 {.gcsafe, locks: 0.}
proc `state=`*(self: RandomNumberGenerator; val: int64) {.gcsafe, locks: 0.}
...
proc randfRange*(self: RandomNumberGenerator; fromm: float64; to: float64): float64 {.
    gcsafe, locks: 0.}
proc randfn*(self: RandomNumberGenerator; mean: float64 = 0.0;
             deviation: float64 = 1.0): float64 {.gcsafe, locks: 0.}
...
proc seed(self: RandomNumberGenerator): int64 =
  ...
  randomNumberGeneratorGetSeedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var randomNumberGeneratorGetStateMethodBind {.threadvar.}: ptr GodotMethodBind
proc state(self: RandomNumberGenerator): int64 =
  if isNil(randomNumberGeneratorGetStateMethodBind):
    randomNumberGeneratorGetStateMethodBind = getMethod(
        cstring"RandomNumberGenerator", cstring"get_state")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  randomNumberGeneratorGetStateMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var randomNumberGeneratorSetStateMethodBind {.threadvar.}: ptr GodotMethodBind
proc `state=`(self: RandomNumberGenerator; val: int64) =
  if isNil(randomNumberGeneratorSetStateMethodBind):
    randomNumberGeneratorSetStateMethodBind = getMethod(
        cstring"RandomNumberGenerator", cstring"set_state")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  randomNumberGeneratorSetStateMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

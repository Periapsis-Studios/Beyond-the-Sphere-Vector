...
proc `expEdit=`(self: Range; val: bool) =
  if isNil(rangeSetExpRatioMethodBind):
    rangeSetExpRatioMethodBind = getMethod(cstring"Range",
        cstring"set_exp_ratio")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc `rounded=`(self: Range; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  rangeSetUseRoundedValuesMethodBind.ptrCall(self.godotObject,
...
proc `value=`(self: Range; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  rangeSetValueMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)

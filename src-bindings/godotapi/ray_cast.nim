...
proc debugShapeCustomColor*(self: RayCast): Color {.gcsafe, locks: 0.}
proc `debugShapeCustomColor=`*(self: RayCast; val: Color) {.gcsafe, locks: 0.}
proc debugShapeThickness*(self: RayCast): int64 {.gcsafe, locks: 0.}
proc `debugShapeThickness=`*(self: RayCast; val: int64) {.gcsafe, locks: 0.}
...
proc setCollisionMaskBit*(self: RayCast; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
...
proc castTo(self: RayCast): Vector3 =
  if isNil(rayCastGetCastToMethodBind):
    rayCastGetCastToMethodBind = getMethod(cstring"RayCast",
        cstring"get_cast_to")
...
proc `castTo=`(self: RayCast; val: Vector3) =
  if isNil(rayCastSetCastToMethodBind):
    rayCastSetCastToMethodBind = getMethod(cstring"RayCast",
        cstring"set_cast_to")
  var
...
var rayCastGetDebugShapeCustomColorMethodBind {.threadvar.}: ptr GodotMethodBind
proc debugShapeCustomColor(self: RayCast): Color =
  if isNil(rayCastGetDebugShapeCustomColorMethodBind):
    rayCastGetDebugShapeCustomColorMethodBind = getMethod(cstring"RayCast",
        cstring"get_debug_shape_custom_color")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rayCastGetDebugShapeCustomColorMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var rayCastSetDebugShapeCustomColorMethodBind {.threadvar.}: ptr GodotMethodBind
proc `debugShapeCustomColor=`(self: RayCast; val: Color) =
  if isNil(rayCastSetDebugShapeCustomColorMethodBind):
    rayCastSetDebugShapeCustomColorMethodBind = getMethod(cstring"RayCast",
        cstring"set_debug_shape_custom_color")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  rayCastSetDebugShapeCustomColorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var rayCastGetDebugShapeThicknessMethodBind {.threadvar.}: ptr GodotMethodBind
proc debugShapeThickness(self: RayCast): int64 =
  if isNil(rayCastGetDebugShapeThicknessMethodBind):
    rayCastGetDebugShapeThicknessMethodBind = getMethod(cstring"RayCast",
        cstring"get_debug_shape_thickness")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rayCastGetDebugShapeThicknessMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var rayCastSetDebugShapeThicknessMethodBind {.threadvar.}: ptr GodotMethodBind
proc `debugShapeThickness=`(self: RayCast; val: int64) =
  if isNil(rayCastSetDebugShapeThicknessMethodBind):
    rayCastSetDebugShapeThicknessMethodBind = getMethod(cstring"RayCast",
        cstring"set_debug_shape_thickness")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  rayCastSetDebugShapeThicknessMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc enabled(self: RayCast): bool =
  if isNil(rayCastIsEnabledMethodBind):
    rayCastIsEnabledMethodBind = getMethod(cstring"RayCast",
        cstring"is_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `enabled=`(self: RayCast; val: bool) =
  if isNil(rayCastSetEnabledMethodBind):
    rayCastSetEnabledMethodBind = getMethod(cstring"RayCast",
        cstring"set_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc excludeParent(self: RayCast): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rayCastGetExcludeParentBodyMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

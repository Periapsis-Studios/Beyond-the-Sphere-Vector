...
proc setCollisionMaskBit*(self: RayCast2D; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
...
proc collideWithAreas(self: RayCast2D): bool =
  if isNil(rayCast2DIsCollideWithAreasEnabledMethodBind):
    rayCast2DIsCollideWithAreasEnabledMethodBind = getMethod(
        cstring"RayCast2D", cstring"is_collide_with_areas_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc collideWithBodies(self: RayCast2D): bool =
  if isNil(rayCast2DIsCollideWithBodiesEnabledMethodBind):
    rayCast2DIsCollideWithBodiesEnabledMethodBind = getMethod(
        cstring"RayCast2D", cstring"is_collide_with_bodies_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc addExceptionRid(self: RayCast2D; rid: RID) =
  ...
  var ptrCallRet: pointer
  rayCast2DAddExceptionRidMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc forceRaycastUpdate(self: RayCast2D) =
  ...
  var ptrCallRet: pointer
  rayCast2DForceRaycastUpdateMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getCollisionNormal(self: RayCast2D): Vector2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rayCast2DGetCollisionNormalMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc removeException(self: RayCast2D; node: Object) =
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  rayCast2DRemoveExceptionMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

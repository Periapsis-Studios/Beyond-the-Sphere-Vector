...
proc `interiorAmbientColor=`*(self: ReflectionProbe; val: Color) {.gcsafe,
    locks: 0.}
...
proc `interiorAmbientEnergy=`*(self: ReflectionProbe; val: float64) {.gcsafe,
    locks: 0.}
...
proc intensity(self: ReflectionProbe): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  reflectionProbeGetIntensityMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `interiorEnable=`(self: ReflectionProbe; val: bool) =
  if isNil(reflectionProbeSetAsInteriorMethodBind):
    reflectionProbeSetAsInteriorMethodBind = getMethod(
        cstring"ReflectionProbe", cstring"set_as_interior")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc maxDistance(self: ReflectionProbe): float64 =
  if isNil(reflectionProbeGetMaxDistanceMethodBind):
    reflectionProbeGetMaxDistanceMethodBind = getMethod(
        cstring"ReflectionProbe", cstring"get_max_distance")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `maxDistance=`(self: ReflectionProbe; val: float64) =
  if isNil(reflectionProbeSetMaxDistanceMethodBind):
    reflectionProbeSetMaxDistanceMethodBind = getMethod(
        cstring"ReflectionProbe", cstring"set_max_distance")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc updateMode(self: ReflectionProbe): int64 =
  if isNil(reflectionProbeGetUpdateModeMethodBind):
    reflectionProbeGetUpdateModeMethodBind = getMethod(
        cstring"ReflectionProbe", cstring"get_update_mode")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  reflectionProbeGetUpdateModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `updateMode=`(self: ReflectionProbe; val: int64) =
  if isNil(reflectionProbeSetUpdateModeMethodBind):
    reflectionProbeSetUpdateModeMethodBind = getMethod(
        cstring"ReflectionProbe", cstring"set_update_mode")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

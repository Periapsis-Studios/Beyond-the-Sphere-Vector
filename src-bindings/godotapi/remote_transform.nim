...
proc `useGlobalCoordinates=`*(self: RemoteTransform; val: bool) {.gcsafe,
    locks: 0.}
...
proc remotePath(self: RemoteTransform): NodePath =
  if isNil(remoteTransformGetRemoteNodeMethodBind):
    remoteTransformGetRemoteNodeMethodBind = getMethod(
        cstring"RemoteTransform", cstring"get_remote_node")
  var ptrCallRet: pointer
  var ptrCallVal: GodotNodePath
  ptrCallRet = addr(ptrCallVal)
  remoteTransformGetRemoteNodeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `remotePath=`(self: RemoteTransform; val: NodePath) =
  if isNil(remoteTransformSetRemoteNodeMethodBind):
    remoteTransformSetRemoteNodeMethodBind = getMethod(
        cstring"RemoteTransform", cstring"set_remote_node")
  var
...
proc updateScale(self: RemoteTransform): bool =
  if isNil(remoteTransformGetUpdateScaleMethodBind):
    remoteTransformGetUpdateScaleMethodBind = getMethod(
        cstring"RemoteTransform", cstring"get_update_scale")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `updateScale=`(self: RemoteTransform; val: bool) =
  if isNil(remoteTransformSetUpdateScaleMethodBind):
    remoteTransformSetUpdateScaleMethodBind = getMethod(
        cstring"RemoteTransform", cstring"set_update_scale")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc useGlobalCoordinates(self: RemoteTransform): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  remoteTransformGetUseGlobalCoordinatesMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

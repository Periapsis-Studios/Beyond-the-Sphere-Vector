...
proc duplicate*(self: Resource; subresources: bool = false): Resource {.gcsafe,
    locks: 0.}
proc emitChanged*(self: Resource) {.gcsafe, locks: 0.}
...
method setupLocalToScene(self: Resource) =
  if isNil(resourceUnderscoresetupLocalToSceneMethodBind):
    resourceUnderscoresetupLocalToSceneMethodBind = getMethod(
        cstring"Resource", cstring"_setup_local_to_scene")
  var ptrCallRet: pointer
...
proc duplicate(self: Resource; subresources: bool = false): Resource =
  if isNil(resourceDuplicateMethodBind):
    resourceDuplicateMethodBind = getMethod(cstring"Resource",
        cstring"duplicate")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var resourceEmitChangedMethodBind {.threadvar.}: ptr GodotMethodBind
proc emitChanged(self: Resource) =
  if isNil(resourceEmitChangedMethodBind):
    resourceEmitChangedMethodBind = getMethod(cstring"Resource",
        cstring"emit_changed")
  var ptrCallRet: pointer
  resourceEmitChangedMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

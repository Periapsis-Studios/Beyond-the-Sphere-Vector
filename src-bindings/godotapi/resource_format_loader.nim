...
method getDependencies*(self: ResourceFormatLoader; path: string;
                        addTypes: string) {.gcsafe, locks: 0, base.}
...
method load*(self: ResourceFormatLoader; path: string; originalPath: string): Variant {.
    gcsafe, locks: 0, base.}
method renameDependencies*(self: ResourceFormatLoader; path: string;
                           renames: string): int64 {.gcsafe, locks: 0, base.}
...
method load(self: ResourceFormatLoader; path: string; originalPath: string): Variant =
  if isNil(resourceFormatLoaderLoadMethodBind):
    resourceFormatLoaderLoadMethodBind = getMethod(
        cstring"ResourceFormatLoader", cstring"load")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(path)
  ...
  ptrCallRet = addr(ptrCallVal)
  resourceFormatLoaderLoadMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  deinit(argToPassToGodot1)

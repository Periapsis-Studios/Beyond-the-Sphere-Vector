...
proc `overrideSelectedFontColor=`*(self: RichTextLabel; val: bool) {.gcsafe,
    locks: 0.}
...
method scrollChanged*(self: RichTextLabel; arg0: float64) {.gcsafe, locks: 0,
    base.}
...
proc appendBbcode*(self: RichTextLabel; bbcode: string): Error {.gcsafe,
    locks: 0.}
...
proc parseExpressionsForValues*(self: RichTextLabel;
                                expressions: PoolStringArray): Dictionary {.
    gcsafe, locks: 0.}
...
proc fitContentHeight(self: RichTextLabel): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  richTextLabelIsFitContentHeightEnabledMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc percentVisible(self: RichTextLabel): float64 =
  if isNil(richTextLabelGetPercentVisibleMethodBind):
    richTextLabelGetPercentVisibleMethodBind = getMethod(
        cstring"RichTextLabel", cstring"get_percent_visible")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `percentVisible=`(self: RichTextLabel; val: float64) =
  if isNil(richTextLabelSetPercentVisibleMethodBind):
    richTextLabelSetPercentVisibleMethodBind = getMethod(
        cstring"RichTextLabel", cstring"set_percent_visible")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc scrollActive(self: RichTextLabel): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  richTextLabelIsScrollActiveMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc scrollFollowing(self: RichTextLabel): bool =
  if isNil(richTextLabelIsScrollFollowingMethodBind):
    richTextLabelIsScrollFollowingMethodBind = getMethod(
        cstring"RichTextLabel", cstring"is_scroll_following")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc selectionEnabled(self: RichTextLabel): bool =
  if isNil(richTextLabelIsSelectionEnabledMethodBind):
    richTextLabelIsSelectionEnabledMethodBind = getMethod(
        cstring"RichTextLabel", cstring"is_selection_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
method guiInput(self: RichTextLabel; event: InputEvent) =
  if isNil(richTextLabelUnderscoreguiInputMethodBind):
    richTextLabelUnderscoreguiInputMethodBind = getMethod(
        cstring"RichTextLabel", cstring"_gui_input")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc parseBbcode(self: RichTextLabel; bbcode: string): Error =
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  richTextLabelParseBbcodeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  result = Error(ptrCallVal)
...
proc pushBoldItalics(self: RichTextLabel) =
  ...
  var ptrCallRet: pointer
  richTextLabelPushBoldItalicsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc pushStrikethrough(self: RichTextLabel) =
  if isNil(richTextLabelPushStrikethroughMethodBind):
    richTextLabelPushStrikethroughMethodBind = getMethod(
        cstring"RichTextLabel", cstring"push_strikethrough")
  var ptrCallRet: pointer

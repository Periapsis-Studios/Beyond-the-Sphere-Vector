...
proc physicsMaterialOverride*(self: RigidBody): PhysicsMaterial {.gcsafe,
    locks: 0.}
...
method directStateChanged*(self: RigidBody; arg0: Object) {.gcsafe, locks: 0,
    base.}
...
proc addForce*(self: RigidBody; force: Vector3; position: Vector3) {.gcsafe,
    locks: 0.}
...
proc getInverseInertiaTensor*(self: RigidBody): Basis {.gcsafe, locks: 0.}
...
proc angularVelocity(self: RigidBody): Vector3 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rigidBodyGetAngularVelocityMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `gravityScale=`(self: RigidBody; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  rigidBodySetGravityScaleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc mass(self: RigidBody): float64 =
  if isNil(rigidBodyGetMassMethodBind):
    rigidBodyGetMassMethodBind = getMethod(cstring"RigidBody",
        cstring"get_mass")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `mass=`(self: RigidBody; val: float64) =
  if isNil(rigidBodySetMassMethodBind):
    rigidBodySetMassMethodBind = getMethod(cstring"RigidBody",
        cstring"set_mass")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc mode(self: RigidBody): int64 =
  if isNil(rigidBodyGetModeMethodBind):
    rigidBodyGetModeMethodBind = getMethod(cstring"RigidBody",
        cstring"get_mode")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `mode=`(self: RigidBody; val: int64) =
  if isNil(rigidBodySetModeMethodBind):
    rigidBodySetModeMethodBind = getMethod(cstring"RigidBody",
        cstring"set_mode")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc physicsMaterialOverride(self: RigidBody): PhysicsMaterial =
  if isNil(rigidBodyGetPhysicsMaterialOverrideMethodBind):
    rigidBodyGetPhysicsMaterialOverrideMethodBind = getMethod(
        cstring"RigidBody", cstring"get_physics_material_override")
  var ptrCallRet: pointer
...
proc `physicsMaterialOverride=`(self: RigidBody; val: PhysicsMaterial) =
  if isNil(rigidBodySetPhysicsMaterialOverrideMethodBind):
    rigidBodySetPhysicsMaterialOverrideMethodBind = getMethod(
        cstring"RigidBody", cstring"set_physics_material_override")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method integrateForces(self: RigidBody; state: PhysicsDirectBodyState) =
  if isNil(rigidBodyUnderscoreintegrateForcesMethodBind):
    rigidBodyUnderscoreintegrateForcesMethodBind = getMethod(
        cstring"RigidBody", cstring"_integrate_forces")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc addCentralForce(self: RigidBody; force: Vector3) =
  ...
  var ptrCallRet: pointer
  rigidBodyAddCentralForceMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc addForce(self: RigidBody; force: Vector3; position: Vector3) =
  if isNil(rigidBodyAddForceMethodBind):
    rigidBodyAddForceMethodBind = getMethod(cstring"RigidBody",
        cstring"add_force")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getCollidingBodies(self: RigidBody): Array =
  ...
  ptrCallRet = addr(ptrCallVal)
  rigidBodyGetCollidingBodiesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = newArray(ptrCallVal)

var rigidBodyGetInverseInertiaTensorMethodBind {.threadvar.}: ptr GodotMethodBind
proc getInverseInertiaTensor(self: RigidBody): Basis =
  if isNil(rigidBodyGetInverseInertiaTensorMethodBind):
    rigidBodyGetInverseInertiaTensorMethodBind = getMethod(cstring"RigidBody",
        cstring"get_inverse_inertia_tensor")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rigidBodyGetInverseInertiaTensorMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc setAxisVelocity(self: RigidBody; axisVelocity: Vector3) =
  ...
  var ptrCallRet: pointer
  rigidBodySetAxisVelocityMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

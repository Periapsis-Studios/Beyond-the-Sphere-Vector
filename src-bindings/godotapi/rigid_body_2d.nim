...
proc physicsMaterialOverride*(self: RigidBody2D): PhysicsMaterial {.gcsafe,
    locks: 0.}
...
method directStateChanged*(self: RigidBody2D; arg0: Object) {.gcsafe, locks: 0,
    base.}
...
proc addForce*(self: RigidBody2D; offset: Vector2; force: Vector2) {.gcsafe,
    locks: 0.}
proc addTorque*(self: RigidBody2D; torque: float64) {.gcsafe, locks: 0.}
proc applyCentralImpulse*(self: RigidBody2D; impulse: Vector2) {.gcsafe,
    locks: 0.}
...
proc setAxisVelocity*(self: RigidBody2D; axisVelocity: Vector2) {.gcsafe,
    locks: 0.}
proc testMotion*(self: RigidBody2D; motion: Vector2;
                 infiniteInertia: bool = true; margin: float64 = 0.08;
...
proc appliedTorque(self: RigidBody2D): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rigidBody2DGetAppliedTorqueMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc contactsReported(self: RigidBody2D): int64 =
  if isNil(rigidBody2DGetMaxContactsReportedMethodBind):
    rigidBody2DGetMaxContactsReportedMethodBind = getMethod(
        cstring"RigidBody2D", cstring"get_max_contacts_reported")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `contactsReported=`(self: RigidBody2D; val: int64) =
  if isNil(rigidBody2DSetMaxContactsReportedMethodBind):
    rigidBody2DSetMaxContactsReportedMethodBind = getMethod(
        cstring"RigidBody2D", cstring"set_max_contacts_reported")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc `customIntegrator=`(self: RigidBody2D; val: bool) =
  if isNil(rigidBody2DSetUseCustomIntegratorMethodBind):
    rigidBody2DSetUseCustomIntegratorMethodBind = getMethod(
        cstring"RigidBody2D", cstring"set_use_custom_integrator")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc `linearDamp=`(self: RigidBody2D; val: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  rigidBody2DSetLinearDampMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc linearVelocity(self: RigidBody2D): Vector2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  rigidBody2DGetLinearVelocityMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method bodyExitTree(self: RigidBody2D; arg0: int64) =
  if isNil(rigidBody2DUnderscorebodyExitTreeMethodBind):
    rigidBody2DUnderscorebodyExitTreeMethodBind = getMethod(
        cstring"RigidBody2D", cstring"_body_exit_tree")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(arg0)

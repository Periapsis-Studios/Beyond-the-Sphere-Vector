...
proc `refuseNewNetworkConnections=`*(self: SceneTree; val: bool) {.gcsafe,
    locks: 0.}
...
method networkPeerConnected*(self: SceneTree; arg0: int64) {.gcsafe, locks: 0,
    base.}
method networkPeerDisconnected*(self: SceneTree; arg0: int64) {.gcsafe,
    locks: 0, base.}
...
proc callGroupFlags*(self: SceneTree; flags: int64; group: string;
                     methodd: string; variantArgs: varargs[Variant]): Variant {.
    gcsafe, locks: 0.}
...
proc changeSceneTo*(self: SceneTree; packedScene: PackedScene): Error {.gcsafe,
    locks: 0.}
proc createTimer*(self: SceneTree; timeSec: float64;
                  pauseModeProcess: bool = true): SceneTreeTimer {.gcsafe,
    locks: 0.}
...
proc setGroup*(self: SceneTree; group: string; property: string; value: Variant) {.
    gcsafe, locks: 0.}
proc setGroupFlags*(self: SceneTree; callFlags: int64; group: string;
                    property: string; value: Variant) {.gcsafe, locks: 0.}
proc setInputAsHandled*(self: SceneTree) {.gcsafe, locks: 0.}
proc setQuitOnGoBack*(self: SceneTree; enabled: bool) {.gcsafe, locks: 0.}
proc setScreenStretch*(self: SceneTree; mode: int64; aspect: int64;
                       minsize: Vector2; scale: float64 = 1.0) {.gcsafe,
    locks: 0.}
...
proc `currentScene=`(self: SceneTree; val: Node) =
  ...
  var ptrCallRet: pointer
  sceneTreeSetCurrentSceneMethodBind.ptrCall(self.godotObject,
...
proc debugCollisionsHint(self: SceneTree): bool =
  if isNil(sceneTreeIsDebuggingCollisionsHintMethodBind):
    sceneTreeIsDebuggingCollisionsHintMethodBind = getMethod(
        cstring"SceneTree", cstring"is_debugging_collisions_hint")
  var ptrCallRet: pointer
...
proc debugNavigationHint(self: SceneTree): bool =
  if isNil(sceneTreeIsDebuggingNavigationHintMethodBind):
    sceneTreeIsDebuggingNavigationHintMethodBind = getMethod(
        cstring"SceneTree", cstring"is_debugging_navigation_hint")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc editedSceneRoot(self: SceneTree): Node =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  sceneTreeGetEditedSceneRootMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc `multiplayerPoll=`(self: SceneTree; val: bool) =
  if isNil(sceneTreeSetMultiplayerPollEnabledMethodBind):
    sceneTreeSetMultiplayerPollEnabledMethodBind = getMethod(
        cstring"SceneTree", cstring"set_multiplayer_poll_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc paused(self: SceneTree): bool =
  if isNil(sceneTreeIsPausedMethodBind):
    sceneTreeIsPausedMethodBind = getMethod(cstring"SceneTree",
        cstring"is_paused")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `paused=`(self: SceneTree; val: bool) =
  if isNil(sceneTreeSetPauseMethodBind):
    sceneTreeSetPauseMethodBind = getMethod(cstring"SceneTree",
        cstring"set_pause")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc root(self: SceneTree): Viewport =
  if isNil(sceneTreeGetRootMethodBind):
    sceneTreeGetRootMethodBind = getMethod(cstring"SceneTree",
        cstring"get_root")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
method connectionFailed(self: SceneTree) =
  if isNil(sceneTreeUnderscoreconnectionFailedMethodBind):
    sceneTreeUnderscoreconnectionFailedMethodBind = getMethod(
        cstring"SceneTree", cstring"_connection_failed")
  var ptrCallRet: pointer
...
proc callGroupFlags(self: SceneTree; flags: int64; group: string;
  ...
  while idx < cint(3 + variantArgs.len):
    ...
    inc idx
  let callRet {.used.} = sceneTreeCallGroupFlagsMethodBind.call(
      self.godotObject, argsToPassToGodot, cint(3 + variantArgs.len), callError)
  godotFree(argsToPassToGodot)
  deinit(variant0)
  deinit(variant1)
  deinit(variantString1)
...
proc createTimer(self: SceneTree; timeSec: float64;
                 pauseModeProcess: bool = true): SceneTreeTimer =
...
proc getFrame(self: SceneTree): int64 =
  if isNil(sceneTreeGetFrameMethodBind):
    sceneTreeGetFrameMethodBind = getMethod(cstring"SceneTree",
        cstring"get_frame")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getNetworkUniqueId(self: SceneTree): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  sceneTreeGetNetworkUniqueIdMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getNodesInGroup(self: SceneTree; group: string): Array =
  ...
  ptrCallRet = addr(ptrCallVal)
  sceneTreeGetNodesInGroupMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc hasGroup(self: SceneTree; name: string): bool =
  if isNil(sceneTreeHasGroupMethodBind):
    sceneTreeHasGroupMethodBind = getMethod(cstring"SceneTree",
        cstring"has_group")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc quit(self: SceneTree; exitCode: int64 = -1'i64) =
  ...
  var ptrCallRet: pointer
  sceneTreeQuitMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc reloadCurrentScene(self: SceneTree): Error =
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  sceneTreeReloadCurrentSceneMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = Error(ptrCallVal)
...
proc setGroup(self: SceneTree; group: string; property: string; value: Variant) =
  if isNil(sceneTreeSetGroupMethodBind):
    sceneTreeSetGroupMethodBind = getMethod(cstring"SceneTree",
        cstring"set_group")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setQuitOnGoBack(self: SceneTree; enabled: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(enabled)
  var ptrCallRet: pointer
  sceneTreeSetQuitOnGoBackMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var sceneTreeSetScreenStretchMethodBind {.threadvar.}: ptr GodotMethodBind
proc setScreenStretch(self: SceneTree; mode: int64; aspect: int64;
                      minsize: Vector2; scale: float64 = 1.0) =
  ...
  argsToPassToGodot[][3] = unsafeAddr(scale)
  var ptrCallRet: pointer

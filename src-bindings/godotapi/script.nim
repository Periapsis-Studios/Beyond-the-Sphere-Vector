...
proc canInstance(self: Script): bool =
  if isNil(scriptCanInstanceMethodBind):
    scriptCanInstanceMethodBind = getMethod(cstring"Script",
...
proc getScriptPropertyList(self: Script): Array =
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  scriptGetScriptPropertyListMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = newArray(ptrCallVal)
...
proc instanceHas(self: Script; baseObject: Object): bool =
  if isNil(scriptInstanceHasMethodBind):
    scriptInstanceHasMethodBind = getMethod(cstring"Script",
        cstring"instance_has")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

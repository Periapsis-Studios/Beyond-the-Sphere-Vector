...
proc `scrollHorizontalEnabled=`*(self: ScrollContainer; val: bool) {.gcsafe,
    locks: 0.}
...
proc `scrollVerticalEnabled=`*(self: ScrollContainer; val: bool) {.gcsafe,
    locks: 0.}
method guiFocusChanged*(self: ScrollContainer; arg0: Control) {.gcsafe,
    locks: 0, base.}
method guiInput*(self: ScrollContainer; event: InputEvent) {.gcsafe, locks: 0.}
method scrollMoved*(self: ScrollContainer; arg0: float64) {.gcsafe, locks: 0,
    base.}
method updateScrollbarPosition*(self: ScrollContainer) {.gcsafe, locks: 0, base.}
proc ensureControlVisible*(self: ScrollContainer; control: Control) {.gcsafe,
    locks: 0.}
...
proc `followFocus=`(self: ScrollContainer; val: bool) =
  if isNil(scrollContainerSetFollowFocusMethodBind):
    scrollContainerSetFollowFocusMethodBind = getMethod(
        cstring"ScrollContainer", cstring"set_follow_focus")
  var
...
var scrollContainerUnderscoreguiFocusChangedMethodBind {.threadvar.}: ptr GodotMethodBind
method guiFocusChanged(self: ScrollContainer; arg0: Control) =
  if isNil(scrollContainerUnderscoreguiFocusChangedMethodBind):
    scrollContainerUnderscoreguiFocusChangedMethodBind = getMethod(
        cstring"ScrollContainer", cstring"_gui_focus_changed")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  var ptrCallRet: pointer
  scrollContainerUnderscoreguiFocusChangedMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var scrollContainerEnsureControlVisibleMethodBind {.threadvar.}: ptr GodotMethodBind
proc ensureControlVisible(self: ScrollContainer; control: Control) =
  if isNil(scrollContainerEnsureControlVisibleMethodBind):
    scrollContainerEnsureControlVisibleMethodBind = getMethod(
        cstring"ScrollContainer", cstring"ensure_control_visible")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  scrollContainerEnsureControlVisibleMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getHScrollbar(self: ScrollContainer): HScrollBar =
  if isNil(scrollContainerGetHScrollbarMethodBind):
    scrollContainerGetHScrollbarMethodBind = getMethod(
        cstring"ScrollContainer", cstring"get_h_scrollbar")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  scrollContainerGetHScrollbarMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getVScrollbar(self: ScrollContainer): VScrollBar =
  if isNil(scrollContainerGetVScrollbarMethodBind):
    scrollContainerGetVScrollbarMethodBind = getMethod(
        cstring"ScrollContainer", cstring"get_v_scrollbar")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  scrollContainerGetVScrollbarMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)

...
proc getDefaultTextureParam*(self: Shader; param: string): Texture {.gcsafe,
    locks: 0.}
...
proc `code=`(self: Shader; val: string) =
  ...
  var ptrCallRet: pointer
  shaderSetCodeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc hasParam(self: Shader; name: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  shaderHasParamMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  deinit(argToPassToGodot0)

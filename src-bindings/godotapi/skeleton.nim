...
proc bindChildNodeToBone*(self: Skeleton; boneIdx: int64; node: Node) {.gcsafe,
    locks: 0.}
...
proc getBoneCustomPose*(self: Skeleton; boneIdx: int64): Transform {.gcsafe,
    locks: 0.}
proc getBoneGlobalPose*(self: Skeleton; boneIdx: int64): Transform {.gcsafe,
    locks: 0.}
proc getBoneGlobalPoseNoOverride*(self: Skeleton; boneIdx: int64): Transform {.
    gcsafe, locks: 0.}
...
proc getBoundChildNodesToBone*(self: Skeleton; boneIdx: int64): Array {.gcsafe,
    locks: 0.}
proc isBoneRestDisabled*(self: Skeleton; boneIdx: int64): bool {.gcsafe,
    locks: 0.}
...
proc setBoneGlobalPoseOverride*(self: Skeleton; boneIdx: int64; pose: Transform;
    ...
    gcsafe, locks: 0.}
proc setBoneName*(self: Skeleton; boneIdx: int64; name: string) {.gcsafe,
    locks: 0.}
proc setBoneParent*(self: Skeleton; boneIdx: int64; parentIdx: int64) {.gcsafe,
    locks: 0.}
proc setBonePose*(self: Skeleton; boneIdx: int64; pose: Transform) {.gcsafe,
    locks: 0.}
proc setBoneRest*(self: Skeleton; boneIdx: int64; rest: Transform) {.gcsafe,
    locks: 0.}
...
proc clearBonesGlobalPoseOverride(self: Skeleton) =
  if isNil(skeletonClearBonesGlobalPoseOverrideMethodBind):
    skeletonClearBonesGlobalPoseOverrideMethodBind = getMethod(
        cstring"Skeleton", cstring"clear_bones_global_pose_override")
  var ptrCallRet: pointer
...
proc findBone(self: Skeleton; name: string): int64 =
  if isNil(skeletonFindBoneMethodBind):
    skeletonFindBoneMethodBind = getMethod(cstring"Skeleton",
        cstring"find_bone")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var skeletonGetBoneGlobalPoseNoOverrideMethodBind {.threadvar.}: ptr GodotMethodBind
proc getBoneGlobalPoseNoOverride(self: Skeleton; boneIdx: int64): Transform =
  if isNil(skeletonGetBoneGlobalPoseNoOverrideMethodBind):
    skeletonGetBoneGlobalPoseNoOverrideMethodBind = getMethod(
        cstring"Skeleton", cstring"get_bone_global_pose_no_override")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(boneIdx)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  skeletonGetBoneGlobalPoseNoOverrideMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc physicalBonesStartSimulation(self: Skeleton; bones: Array = newArray()) =
  if isNil(skeletonPhysicalBonesStartSimulationMethodBind):
    skeletonPhysicalBonesStartSimulationMethodBind = getMethod(
        cstring"Skeleton", cstring"physical_bones_start_simulation")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc physicalBonesStopSimulation(self: Skeleton) =
  if isNil(skeletonPhysicalBonesStopSimulationMethodBind):
    skeletonPhysicalBonesStopSimulationMethodBind = getMethod(
        cstring"Skeleton", cstring"physical_bones_stop_simulation")
  var ptrCallRet: pointer
...
var skeletonSetBoneNameMethodBind {.threadvar.}: ptr GodotMethodBind
proc setBoneName(self: Skeleton; boneIdx: int64; name: string) =
  if isNil(skeletonSetBoneNameMethodBind):
    skeletonSetBoneNameMethodBind = getMethod(cstring"Skeleton",
  ...
  var ptrCallRet: pointer
  skeletonSetBoneNameMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                        ptrCallRet)

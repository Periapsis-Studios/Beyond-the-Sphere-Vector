...
proc setBindPose*(self: Skin; bindIndex: int64; pose: Transform) {.gcsafe,
    locks: 0.}
...
proc getBindCount(self: Skin): int64 =
  if isNil(skinGetBindCountMethodBind):
    skinGetBindCountMethodBind = getMethod(cstring"Skin",
        cstring"get_bind_count")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc setBindCount(self: Skin; bindCount: int64) =
  if isNil(skinSetBindCountMethodBind):
    skinSetBindCountMethodBind = getMethod(cstring"Skin",
        cstring"set_bind_count")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

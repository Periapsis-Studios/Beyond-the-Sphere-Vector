...
proc editable(self: Slider): bool =
  if isNil(sliderIsEditableMethodBind):
    sliderIsEditableMethodBind = getMethod(cstring"Slider",
        cstring"is_editable")
...
proc `editable=`(self: Slider; val: bool) =
  if isNil(sliderSetEditableMethodBind):
    sliderSetEditableMethodBind = getMethod(cstring"Slider",
...
proc `tickCount=`(self: Slider; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  sliderSetTicksMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
method guiInput(self: Slider; event: InputEvent) =
  ...
  var ptrCallRet: pointer
  sliderUnderscoreguiInputMethodBind.ptrCall(self.godotObject,

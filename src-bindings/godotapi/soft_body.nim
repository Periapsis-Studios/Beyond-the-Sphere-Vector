...
proc physicsEnabled*(self: SoftBody): bool {.gcsafe, locks: 0.}
proc `physicsEnabled=`*(self: SoftBody; val: bool) {.gcsafe, locks: 0.}
proc poseMatchingCoefficient*(self: SoftBody): float64 {.gcsafe, locks: 0.}
proc `poseMatchingCoefficient=`*(self: SoftBody; val: float64) {.gcsafe,
    locks: 0.}
...
proc getPointTransform*(self: SoftBody; pointIndex: int64): Vector3 {.gcsafe,
    locks: 0.}
proc isPointPinned*(self: SoftBody; pointIndex: int64): bool {.gcsafe, locks: 0.}
proc removeCollisionExceptionWith*(self: SoftBody; body: Node) {.gcsafe,
    locks: 0.}
proc setCollisionLayerBit*(self: SoftBody; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
proc setCollisionMaskBit*(self: SoftBody; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
proc setPointPinned*(self: SoftBody; pointIndex: int64; pinned: bool;
                     attachmentPath: NodePath = "") {.gcsafe, locks: 0.}
...
proc `collisionMask=`(self: SoftBody; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  softBodySetCollisionMaskMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var softBodyIsPhysicsEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc physicsEnabled(self: SoftBody): bool =
  if isNil(softBodyIsPhysicsEnabledMethodBind):
    softBodyIsPhysicsEnabledMethodBind = getMethod(cstring"SoftBody",
        cstring"is_physics_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  softBodyIsPhysicsEnabledMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var softBodySetPhysicsEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc `physicsEnabled=`(self: SoftBody; val: bool) =
  if isNil(softBodySetPhysicsEnabledMethodBind):
    softBodySetPhysicsEnabledMethodBind = getMethod(cstring"SoftBody",
        cstring"set_physics_enabled")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  softBodySetPhysicsEnabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var softBodyGetPointTransformMethodBind {.threadvar.}: ptr GodotMethodBind
proc getPointTransform(self: SoftBody; pointIndex: int64): Vector3 =
  if isNil(softBodyGetPointTransformMethodBind):
    softBodyGetPointTransformMethodBind = getMethod(cstring"SoftBody",
        cstring"get_point_transform")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(pointIndex)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  softBodyGetPointTransformMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

var softBodyIsPointPinnedMethodBind {.threadvar.}: ptr GodotMethodBind
proc isPointPinned(self: SoftBody; pointIndex: int64): bool =
  if isNil(softBodyIsPointPinnedMethodBind):
    softBodyIsPointPinnedMethodBind = getMethod(cstring"SoftBody",
        cstring"is_point_pinned")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(pointIndex)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  softBodyIsPointPinnedMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
proc removeCollisionExceptionWith(self: SoftBody; body: Node) =
  if isNil(softBodyRemoveCollisionExceptionWithMethodBind):
    softBodyRemoveCollisionExceptionWithMethodBind = getMethod(
        cstring"SoftBody", cstring"remove_collision_exception_with")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not body.isNil:
...
var softBodySetPointPinnedMethodBind {.threadvar.}: ptr GodotMethodBind
proc setPointPinned(self: SoftBody; pointIndex: int64; pinned: bool;
                    attachmentPath: NodePath = "") =
  if isNil(softBodySetPointPinnedMethodBind):
    softBodySetPointPinnedMethodBind = getMethod(cstring"SoftBody",
        cstring"set_point_pinned")
  var
    argsStatic: array[3, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(pointIndex)
  argsToPassToGodot[][1] = unsafeAddr(pinned)
  argsToPassToGodot[][2] = attachmentPath.godotNodePath
  var ptrCallRet: pointer
  softBodySetPointPinnedMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)

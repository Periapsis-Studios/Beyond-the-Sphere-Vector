...
const
  NOTIFICATION_ENTER_GAMEPLAY* = 45'i64
  NOTIFICATION_ENTER_WORLD* = 41'i64
  NOTIFICATION_EXIT_GAMEPLAY* = 46'i64
...
proc globalRotate*(self: Spatial; axis: Vector3; angle: float64) {.gcsafe,
    locks: 0.}
...
proc isLocalTransformNotificationEnabled*(self: Spatial): bool {.gcsafe,
    locks: 0.}
...
proc lookAtFromPosition*(self: Spatial; position: Vector3; target: Vector3;
                         up: Vector3) {.gcsafe, locks: 0.}
proc orthonormalize*(self: Spatial) {.gcsafe, locks: 0.}
proc rotate*(self: Spatial; axis: Vector3; angle: float64) {.gcsafe, locks: 0.}
proc rotateObjectLocal*(self: Spatial; axis: Vector3; angle: float64) {.gcsafe,
    locks: 0.}
...
proc setIgnoreTransformNotification*(self: Spatial; enabled: bool) {.gcsafe,
    locks: 0.}
...
proc visible(self: Spatial): bool =
  if isNil(spatialIsVisibleMethodBind):
    spatialIsVisibleMethodBind = getMethod(cstring"Spatial",
        cstring"is_visible")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `visible=`(self: Spatial; val: bool) =
  if isNil(spatialSetVisibleMethodBind):
    spatialSetVisibleMethodBind = getMethod(cstring"Spatial",
        cstring"set_visible")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method updateGizmo(self: Spatial) =
  ...
  var ptrCallRet: pointer
  spatialUnderscoreupdateGizmoMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc forceUpdateTransform(self: Spatial) =
  ...
  var ptrCallRet: pointer
  spatialForceUpdateTransformMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc isTransformNotificationEnabled(self: Spatial): bool =
  if isNil(spatialIsTransformNotificationEnabledMethodBind):
    spatialIsTransformNotificationEnabledMethodBind = getMethod(
        cstring"Spatial", cstring"is_transform_notification_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc lookAt(self: Spatial; target: Vector3; up: Vector3) =
  ...
  var ptrCallRet: pointer
  spatialLookAtMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc rotate(self: Spatial; axis: Vector3; angle: float64) =
  ...
  argsToPassToGodot[][1] = unsafeAddr(angle)
  var ptrCallRet: pointer
  spatialRotateMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc rotateObjectLocal(self: Spatial; axis: Vector3; angle: float64) =
  ...
  argsToPassToGodot[][1] = unsafeAddr(angle)
  var ptrCallRet: pointer
  spatialRotateObjectLocalMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc rotateX(self: Spatial; angle: float64) =
  ...
  var ptrCallRet: pointer
  spatialRotateXMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc rotateY(self: Spatial; angle: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(angle)
  var ptrCallRet: pointer
  spatialRotateYMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc rotateZ(self: Spatial; angle: float64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(angle)
  var ptrCallRet: pointer
  spatialRotateZMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc setIgnoreTransformNotification(self: Spatial; enabled: bool) =
  if isNil(spatialSetIgnoreTransformNotificationMethodBind):
    spatialSetIgnoreTransformNotificationMethodBind = getMethod(
        cstring"Spatial", cstring"set_ignore_transform_notification")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc toLocal(self: Spatial; globalPoint: Vector3): Vector3 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  spatialToLocalMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

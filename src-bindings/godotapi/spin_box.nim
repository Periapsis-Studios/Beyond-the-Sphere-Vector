...
proc editable(self: SpinBox): bool =
  if isNil(spinBoxIsEditableMethodBind):
    spinBoxIsEditableMethodBind = getMethod(cstring"SpinBox",
        cstring"is_editable")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc prefix(self: SpinBox): string =
  if isNil(spinBoxGetPrefixMethodBind):
    spinBoxGetPrefixMethodBind = getMethod(cstring"SpinBox",
        cstring"get_prefix")
  var ptrCallRet: pointer
...
proc `prefix=`(self: SpinBox; val: string) =
  if isNil(spinBoxSetPrefixMethodBind):
    spinBoxSetPrefixMethodBind = getMethod(cstring"SpinBox",
        cstring"set_prefix")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc suffix(self: SpinBox): string =
  if isNil(spinBoxGetSuffixMethodBind):
    spinBoxGetSuffixMethodBind = getMethod(cstring"SpinBox",
        cstring"get_suffix")
  var ptrCallRet: pointer
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
...
proc `suffix=`(self: SpinBox; val: string) =
  if isNil(spinBoxSetSuffixMethodBind):
    spinBoxSetSuffixMethodBind = getMethod(cstring"SpinBox",
        cstring"set_suffix")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

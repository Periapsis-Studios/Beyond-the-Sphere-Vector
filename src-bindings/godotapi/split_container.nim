...
proc splitOffset(self: SplitContainer): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  splitContainerGetSplitOffsetMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc clampSplitOffset(self: SplitContainer) =
  if isNil(splitContainerClampSplitOffsetMethodBind):
    splitContainerClampSplitOffsetMethodBind = getMethod(
        cstring"SplitContainer", cstring"clamp_split_offset")
  var ptrCallRet: pointer

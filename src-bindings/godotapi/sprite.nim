...
proc centered(self: Sprite): bool =
  if isNil(spriteIsCenteredMethodBind):
    spriteIsCenteredMethodBind = getMethod(cstring"Sprite",
        cstring"is_centered")
...
proc `centered=`(self: Sprite; val: bool) =
  if isNil(spriteSetCenteredMethodBind):
    spriteSetCenteredMethodBind = getMethod(cstring"Sprite",
...
proc flipH(self: Sprite): bool =
  if isNil(spriteIsFlippedHMethodBind):
    spriteIsFlippedHMethodBind = getMethod(cstring"Sprite",
...
proc `flipH=`(self: Sprite; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  spriteSetFlipHMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc flipV(self: Sprite): bool =
  if isNil(spriteIsFlippedVMethodBind):
    spriteIsFlippedVMethodBind = getMethod(cstring"Sprite",
...
proc `flipV=`(self: Sprite; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  spriteSetFlipVMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc `frame=`(self: Sprite; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  spriteSetFrameMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc hframes(self: Sprite): int64 =
  if isNil(spriteGetHframesMethodBind):
    spriteGetHframesMethodBind = getMethod(cstring"Sprite",
        cstring"get_hframes")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `hframes=`(self: Sprite; val: int64) =
  if isNil(spriteSetHframesMethodBind):
    spriteSetHframesMethodBind = getMethod(cstring"Sprite",
        cstring"set_hframes")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc texture(self: Sprite): Texture =
  if isNil(spriteGetTextureMethodBind):
    spriteGetTextureMethodBind = getMethod(cstring"Sprite",
        cstring"get_texture")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc `texture=`(self: Sprite; val: Texture) =
  if isNil(spriteSetTextureMethodBind):
    spriteSetTextureMethodBind = getMethod(cstring"Sprite",
        cstring"set_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc vframes(self: Sprite): int64 =
  if isNil(spriteGetVframesMethodBind):
    spriteGetVframesMethodBind = getMethod(cstring"Sprite",
        cstring"get_vframes")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `vframes=`(self: Sprite; val: int64) =
  if isNil(spriteSetVframesMethodBind):
    spriteSetVframesMethodBind = getMethod(cstring"Sprite",
        cstring"set_vframes")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

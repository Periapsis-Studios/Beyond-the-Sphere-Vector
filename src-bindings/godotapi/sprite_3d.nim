...
proc frame(self: Sprite3D): int64 =
  if isNil(sprite3DGetFrameMethodBind):
    sprite3DGetFrameMethodBind = getMethod(cstring"Sprite3D",
        cstring"get_frame")
...
proc `frame=`(self: Sprite3D; val: int64) =
  if isNil(sprite3DSetFrameMethodBind):
    sprite3DSetFrameMethodBind = getMethod(cstring"Sprite3D",
        cstring"set_frame")
  var
...
proc regionEnabled(self: Sprite3D): bool =
  if isNil(sprite3DIsRegionMethodBind):
    sprite3DIsRegionMethodBind = getMethod(cstring"Sprite3D",
        cstring"is_region")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `regionEnabled=`(self: Sprite3D; val: bool) =
  if isNil(sprite3DSetRegionMethodBind):
    sprite3DSetRegionMethodBind = getMethod(cstring"Sprite3D",
        cstring"set_region")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

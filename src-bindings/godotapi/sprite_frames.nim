...
proc getAnimationLoop*(self: SpriteFrames; anim: string): bool {.gcsafe,
    locks: 0.}
proc getAnimationNames*(self: SpriteFrames): PoolStringArray {.gcsafe, locks: 0.}
proc getAnimationSpeed*(self: SpriteFrames; anim: string): float64 {.gcsafe,
    locks: 0.}
proc getFrame*(self: SpriteFrames; anim: string; idx: int64): Texture {.gcsafe,
    locks: 0.}
...
proc removeFrame*(self: SpriteFrames; anim: string; idx: int64) {.gcsafe,
    locks: 0.}
proc renameAnimation*(self: SpriteFrames; anim: string; newname: string) {.
    gcsafe, locks: 0.}
proc setAnimationLoop*(self: SpriteFrames; anim: string; loop: bool) {.gcsafe,
    locks: 0.}
...
proc frames(self: SpriteFrames): Array =
  if isNil(spriteFramesUnderscoregetFramesMethodBind):
    spriteFramesUnderscoregetFramesMethodBind = getMethod(
        cstring"SpriteFrames", cstring"_get_frames")
  var ptrCallRet: pointer
  var ptrCallVal: GodotArray
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
...
proc `frames=`(self: SpriteFrames; val: Array) =
  if isNil(spriteFramesUnderscoresetFramesMethodBind):
    spriteFramesUnderscoresetFramesMethodBind = getMethod(
        cstring"SpriteFrames", cstring"_set_frames")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = val.godotArray
...
proc addAnimation(self: SpriteFrames; anim: string) =
  ...
  var ptrCallRet: pointer
  spriteFramesAddAnimationMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc clear(self: SpriteFrames; anim: string) =
  if isNil(spriteFramesClearMethodBind):
    spriteFramesClearMethodBind = getMethod(cstring"SpriteFrames",
        cstring"clear")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc hasAnimation(self: SpriteFrames; anim: string): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  spriteFramesHasAnimationMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)

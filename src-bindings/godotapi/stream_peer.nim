...
proc getString*(self: StreamPeer; bytes: int64 = -1'i64): string {.gcsafe,
    locks: 0.}
...
proc getUtf8String*(self: StreamPeer; bytes: int64 = -1'i64): string {.gcsafe,
    locks: 0.}
proc getVar*(self: StreamPeer; allowObjects: bool = false): Variant {.gcsafe,
    locks: 0.}
...
proc putPartialData*(self: StreamPeer; data: PoolByteArray): Array {.gcsafe,
    locks: 0.}
...
proc bigEndian(self: StreamPeer): bool =
  ...
  streamPeerIsBigEndianEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getAvailableBytes(self: StreamPeer): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  streamPeerGetAvailableBytesMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getData(self: StreamPeer; bytes: int64): Array =
  if isNil(streamPeerGetDataMethodBind):
    streamPeerGetDataMethodBind = getMethod(cstring"StreamPeer",
        cstring"get_data")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getPartialData(self: StreamPeer; bytes: int64): Array =
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  streamPeerGetPartialDataMethodBind.ptrCall(self.godotObject,
...
proc getU16(self: StreamPeer): int64 =
  if isNil(streamPeerGetU16MethodBind):
    streamPeerGetU16MethodBind = getMethod(cstring"StreamPeer",
        cstring"get_u16")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getU32(self: StreamPeer): int64 =
  if isNil(streamPeerGetU32MethodBind):
    streamPeerGetU32MethodBind = getMethod(cstring"StreamPeer",
        cstring"get_u32")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getU64(self: StreamPeer): int64 =
  if isNil(streamPeerGetU64MethodBind):
    streamPeerGetU64MethodBind = getMethod(cstring"StreamPeer",
        cstring"get_u64")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getVar(self: StreamPeer; allowObjects: bool = false): Variant =
  if isNil(streamPeerGetVarMethodBind):
    streamPeerGetVarMethodBind = getMethod(cstring"StreamPeer",
        cstring"get_var")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc put8(self: StreamPeer; value: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(value)
  var ptrCallRet: pointer
  streamPeerPut8MethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc putData(self: StreamPeer; data: PoolByteArray): Error =
  if isNil(streamPeerPutDataMethodBind):
    streamPeerPutDataMethodBind = getMethod(cstring"StreamPeer",
        cstring"put_data")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc putPartialData(self: StreamPeer; data: PoolByteArray): Array =
  ...
  initGodotArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  streamPeerPutPartialDataMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  result = newArray(ptrCallVal)
...
proc putU16(self: StreamPeer; value: int64) =
  if isNil(streamPeerPutU16MethodBind):
    streamPeerPutU16MethodBind = getMethod(cstring"StreamPeer",
        cstring"put_u16")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc putU32(self: StreamPeer; value: int64) =
  if isNil(streamPeerPutU32MethodBind):
    streamPeerPutU32MethodBind = getMethod(cstring"StreamPeer",
        cstring"put_u32")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc putU64(self: StreamPeer; value: int64) =
  if isNil(streamPeerPutU64MethodBind):
    streamPeerPutU64MethodBind = getMethod(cstring"StreamPeer",
        cstring"put_u64")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc putVar(self: StreamPeer; value: Variant; fullObjects: bool = false) =
  if isNil(streamPeerPutVarMethodBind):
    streamPeerPutVarMethodBind = getMethod(cstring"StreamPeer",
        cstring"put_var")
  var

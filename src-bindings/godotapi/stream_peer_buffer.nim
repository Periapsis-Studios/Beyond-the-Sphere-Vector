...
proc `dataArray=`*(self: StreamPeerBuffer; val: PoolByteArray) {.gcsafe,
    locks: 0.}
...
proc dataArray(self: StreamPeerBuffer): PoolByteArray =
  if isNil(streamPeerBufferGetDataArrayMethodBind):
    streamPeerBufferGetDataArrayMethodBind = getMethod(
        cstring"StreamPeerBuffer", cstring"get_data_array")
  var ptrCallRet: pointer
  var ptrCallVal: GodotPoolByteArray
  initGodotPoolByteArray(ptrCallVal)
  ptrCallRet = addr(ptrCallVal)
  streamPeerBufferGetDataArrayMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `dataArray=`(self: StreamPeerBuffer; val: PoolByteArray) =
  if isNil(streamPeerBufferSetDataArrayMethodBind):
    streamPeerBufferSetDataArrayMethodBind = getMethod(
        cstring"StreamPeerBuffer", cstring"set_data_array")
  var
...
proc getPosition(self: StreamPeerBuffer): int64 =
  if isNil(streamPeerBufferGetPositionMethodBind):
    streamPeerBufferGetPositionMethodBind = getMethod(
        cstring"StreamPeerBuffer", cstring"get_position")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  streamPeerBufferGetPositionMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
proc acceptStream*(self: StreamPeerSSL; stream: StreamPeer;
                   privateKey: CryptoKey; certificate: X509Certificate;
...
proc poll(self: StreamPeerSSL) =
  if isNil(streamPeerSSLPollMethodBind):
    streamPeerSSLPollMethodBind = getMethod(cstring"StreamPeerSSL",
        cstring"poll")
  var ptrCallRet: pointer

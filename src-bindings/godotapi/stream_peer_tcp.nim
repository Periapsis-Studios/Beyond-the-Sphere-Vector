...
proc disconnectFromHost(self: StreamPeerTCP) =
  if isNil(streamPeerTCPDisconnectFromHostMethodBind):
    streamPeerTCPDisconnectFromHostMethodBind = getMethod(
        cstring"StreamPeerTCP", cstring"disconnect_from_host")
  var ptrCallRet: pointer
...
proc isConnectedToHost(self: StreamPeerTCP): bool =
  if isNil(streamPeerTCPIsConnectedToHostMethodBind):
    streamPeerTCPIsConnectedToHostMethodBind = getMethod(
        cstring"StreamPeerTCP", cstring"is_connected_to_host")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

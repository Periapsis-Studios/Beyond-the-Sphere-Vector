...
proc testMask*(self: StyleBox; point: Vector2; rect: Rect2): bool {.gcsafe,
    locks: 0.}
...
proc contentMarginBottom(self: StyleBox): float64 =
  ...
  styleBoxGetDefaultMarginMethodBind.ptrCall(self.godotObject,
...
proc `contentMarginBottom=`(self: StyleBox; val: float64) =
  ...
  var ptrCallRet: pointer
  styleBoxSetDefaultMarginMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getCurrentItemDrawn(self: StyleBox): CanvasItem =
  ...
  styleBoxGetCurrentItemDrawnMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getMargin(self: StyleBox; margin: int64): float64 =
  if isNil(styleBoxGetMarginMethodBind):
    styleBoxGetMarginMethodBind = getMethod(cstring"StyleBox",
        cstring"get_margin")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getOffset(self: StyleBox): Vector2 =
  if isNil(styleBoxGetOffsetMethodBind):
    styleBoxGetOffsetMethodBind = getMethod(cstring"StyleBox",
        cstring"get_offset")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc testMask(self: StyleBox; point: Vector2; rect: Rect2): bool =
  if isNil(styleBoxTestMaskMethodBind):
    styleBoxTestMaskMethodBind = getMethod(cstring"StyleBox",
        cstring"test_mask")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

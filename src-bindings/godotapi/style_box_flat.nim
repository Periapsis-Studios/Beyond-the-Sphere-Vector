...
proc antiAliasingSize*(self: StyleBoxFlat): float64 {.gcsafe, locks: 0.}
proc `antiAliasingSize=`*(self: StyleBoxFlat; val: float64) {.gcsafe, locks: 0.}
...
proc `cornerRadiusBottomLeft=`*(self: StyleBoxFlat; val: int64) {.gcsafe,
    locks: 0.}
proc cornerRadiusBottomRight*(self: StyleBoxFlat): int64 {.gcsafe, locks: 0.}
proc `cornerRadiusBottomRight=`*(self: StyleBoxFlat; val: int64) {.gcsafe,
    locks: 0.}
...
proc antiAliasingSize(self: StyleBoxFlat): float64 =
...
proc `antiAliasingSize=`(self: StyleBoxFlat; val: float64) =
...
proc cornerDetail(self: StyleBoxFlat): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  styleBoxFlatGetCornerDetailMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc drawCenter(self: StyleBoxFlat): bool =
  if isNil(styleBoxFlatIsDrawCenterEnabledMethodBind):
    styleBoxFlatIsDrawCenterEnabledMethodBind = getMethod(
        cstring"StyleBoxFlat", cstring"is_draw_center_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc shadowOffset(self: StyleBoxFlat): Vector2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  styleBoxFlatGetShadowOffsetMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

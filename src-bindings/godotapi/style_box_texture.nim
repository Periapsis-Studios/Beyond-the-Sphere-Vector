...
proc `axisStretchHorizontal=`*(self: StyleBoxTexture; val: int64) {.gcsafe,
    locks: 0.}
proc axisStretchVertical*(self: StyleBoxTexture): int64 {.gcsafe, locks: 0.}
proc `axisStretchVertical=`*(self: StyleBoxTexture; val: int64) {.gcsafe,
    locks: 0.}
...
proc `expandMarginBottom=`*(self: StyleBoxTexture; val: float64) {.gcsafe,
    locks: 0.}
proc expandMarginLeft*(self: StyleBoxTexture): float64 {.gcsafe, locks: 0.}
proc `expandMarginLeft=`*(self: StyleBoxTexture; val: float64) {.gcsafe,
    locks: 0.}
proc expandMarginRight*(self: StyleBoxTexture): float64 {.gcsafe, locks: 0.}
proc `expandMarginRight=`*(self: StyleBoxTexture; val: float64) {.gcsafe,
    locks: 0.}
...
proc setExpandMarginAll*(self: StyleBoxTexture; size: float64) {.gcsafe,
    locks: 0.}
...
proc `drawCenter=`(self: StyleBoxTexture; val: bool) =
  if isNil(styleBoxTextureSetDrawCenterMethodBind):
    styleBoxTextureSetDrawCenterMethodBind = getMethod(
        cstring"StyleBoxTexture", cstring"set_draw_center")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc marginBottom(self: StyleBoxTexture): float64 =
  if isNil(styleBoxTextureGetMarginSizeMethodBind):
    styleBoxTextureGetMarginSizeMethodBind = getMethod(
        cstring"StyleBoxTexture", cstring"get_margin_size")
  var
    godotBoundArg0 = 3'i64
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc `marginBottom=`(self: StyleBoxTexture; val: float64) =
  if isNil(styleBoxTextureSetMarginSizeMethodBind):
    styleBoxTextureSetMarginSizeMethodBind = getMethod(
        cstring"StyleBoxTexture", cstring"set_margin_size")
  var
    godotBoundArg0 = 3'i64
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc normalMap(self: StyleBoxTexture): Texture =
  ...
  styleBoxTextureGetNormalMapMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc regionRect(self: StyleBoxTexture): Rect2 =
  if isNil(styleBoxTextureGetRegionRectMethodBind):
    styleBoxTextureGetRegionRectMethodBind = getMethod(
        cstring"StyleBoxTexture", cstring"get_region_rect")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  styleBoxTextureGetRegionRectMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `regionRect=`(self: StyleBoxTexture; val: Rect2) =
  if isNil(styleBoxTextureSetRegionRectMethodBind):
    styleBoxTextureSetRegionRectMethodBind = getMethod(
        cstring"StyleBoxTexture", cstring"set_region_rect")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

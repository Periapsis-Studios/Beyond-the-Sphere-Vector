...
const
  ...
  ALIGN_LEFT* = 0'i64
  ALIGN_RIGHT* = 2'i64
proc allTabsInFront*(self: TabContainer): bool {.gcsafe, locks: 0.}
proc `allTabsInFront=`*(self: TabContainer; val: bool) {.gcsafe, locks: 0.}
...
proc `dragToRearrangeEnabled=`*(self: TabContainer; val: bool) {.gcsafe,
    locks: 0.}
...
proc `useHiddenTabsForMinSize=`*(self: TabContainer; val: bool) {.gcsafe,
    locks: 0.}
...
method repaint*(self: TabContainer) {.gcsafe, locks: 0, base.}
...
proc getTabControl*(self: TabContainer; tabIdx: int64): Control {.gcsafe,
    locks: 0.}
...
proc getTabHidden*(self: TabContainer; tabIdx: int64): bool {.gcsafe, locks: 0.}
proc getTabIcon*(self: TabContainer; tabIdx: int64): Texture {.gcsafe, locks: 0.}
proc getTabIdxAtPoint*(self: TabContainer; point: Vector2): int64 {.gcsafe,
    locks: 0.}
...
proc setTabDisabled*(self: TabContainer; tabIdx: int64; disabled: bool) {.
    gcsafe, locks: 0.}
proc setTabHidden*(self: TabContainer; tabIdx: int64; hidden: bool) {.gcsafe,
    locks: 0.}
proc setTabIcon*(self: TabContainer; tabIdx: int64; icon: Texture) {.gcsafe,
    locks: 0.}
proc setTabTitle*(self: TabContainer; tabIdx: int64; title: string) {.gcsafe,
    locks: 0.}
proc setTabsRearrangeGroup*(self: TabContainer; groupId: int64) {.gcsafe,
    locks: 0.}
var tabContainerIsAllTabsInFrontMethodBind {.threadvar.}: ptr GodotMethodBind
proc allTabsInFront(self: TabContainer): bool =
  if isNil(tabContainerIsAllTabsInFrontMethodBind):
    tabContainerIsAllTabsInFrontMethodBind = getMethod(cstring"TabContainer",
        cstring"is_all_tabs_in_front")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabContainerIsAllTabsInFrontMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var tabContainerSetAllTabsInFrontMethodBind {.threadvar.}: ptr GodotMethodBind
proc `allTabsInFront=`(self: TabContainer; val: bool) =
  if isNil(tabContainerSetAllTabsInFrontMethodBind):
    tabContainerSetAllTabsInFrontMethodBind = getMethod(cstring"TabContainer",
        cstring"set_all_tabs_in_front")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  tabContainerSetAllTabsInFrontMethodBind.ptrCall(self.godotObject,
...
proc useHiddenTabsForMinSize(self: TabContainer): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabContainerGetUseHiddenTabsForMinSizeMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
var tabContainerUnderscorerepaintMethodBind {.threadvar.}: ptr GodotMethodBind
method repaint(self: TabContainer) =
  if isNil(tabContainerUnderscorerepaintMethodBind):
    tabContainerUnderscorerepaintMethodBind = getMethod(cstring"TabContainer",
        cstring"_repaint")
  var ptrCallRet: pointer
  tabContainerUnderscorerepaintMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method updateCurrentTab(self: TabContainer) =
  ...
  var ptrCallRet: pointer
  tabContainerUnderscoreupdateCurrentTabMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc getCurrentTabControl(self: TabContainer): Control =
  if isNil(tabContainerGetCurrentTabControlMethodBind):
    tabContainerGetCurrentTabControlMethodBind = getMethod(
        cstring"TabContainer", cstring"get_current_tab_control")
  var ptrCallRet: pointer
...
var tabContainerGetTabHiddenMethodBind {.threadvar.}: ptr GodotMethodBind
proc getTabHidden(self: TabContainer; tabIdx: int64): bool =
  if isNil(tabContainerGetTabHiddenMethodBind):
    tabContainerGetTabHiddenMethodBind = getMethod(cstring"TabContainer",
        cstring"get_tab_hidden")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(tabIdx)
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabContainerGetTabHiddenMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var tabContainerGetTabIdxAtPointMethodBind {.threadvar.}: ptr GodotMethodBind
proc getTabIdxAtPoint(self: TabContainer; point: Vector2): int64 =
  if isNil(tabContainerGetTabIdxAtPointMethodBind):
    tabContainerGetTabIdxAtPointMethodBind = getMethod(cstring"TabContainer",
        cstring"get_tab_idx_at_point")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabContainerGetTabIdxAtPointMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
var tabContainerSetTabHiddenMethodBind {.threadvar.}: ptr GodotMethodBind
proc setTabHidden(self: TabContainer; tabIdx: int64; hidden: bool) =
  if isNil(tabContainerSetTabHiddenMethodBind):
    tabContainerSetTabHiddenMethodBind = getMethod(cstring"TabContainer",
        cstring"set_tab_hidden")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(tabIdx)
  ...
  var ptrCallRet: pointer
  tabContainerSetTabHiddenMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

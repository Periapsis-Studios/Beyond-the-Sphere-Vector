...
proc setTabDisabled*(self: Tabs; tabIdx: int64; disabled: bool) {.gcsafe,
    locks: 0.}
...
proc tabCloseDisplayPolicy(self: Tabs): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabsGetTabCloseDisplayPolicyMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
method onMouseExited(self: Tabs) =
  ...
  var ptrCallRet: pointer
  tabsUnderscoreonMouseExitedMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getOffsetButtonsVisible(self: Tabs): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabsGetOffsetButtonsVisibleMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var tabsGetPreviousTabMethodBind {.threadvar.}: ptr GodotMethodBind
proc getPreviousTab(self: Tabs): int64 =
  if isNil(tabsGetPreviousTabMethodBind):
    tabsGetPreviousTabMethodBind = getMethod(cstring"Tabs",
        cstring"get_previous_tab")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabsGetPreviousTabMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
proc getTabIcon(self: Tabs; tabIdx: int64): Texture =
  ...
  tabsGetTabIconMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc getTabOffset(self: Tabs): int64 =
  if isNil(tabsGetTabOffsetMethodBind):
    tabsGetTabOffsetMethodBind = getMethod(cstring"Tabs",
        cstring"get_tab_offset")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc getTabRect(self: Tabs; tabIdx: int64): Rect2 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tabsGetTabRectMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc removeTab(self: Tabs; tabIdx: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(tabIdx)
  var ptrCallRet: pointer
  tabsRemoveTabMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc setTabIcon(self: Tabs; tabIdx: int64; icon: Texture) =
  ...
  argsToPassToGodot[][1] = argToPassToGodot1
  var ptrCallRet: pointer
  tabsSetTabIconMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

...
proc flipH*(self: TextureButton): bool {.gcsafe, locks: 0.}
proc `flipH=`*(self: TextureButton; val: bool) {.gcsafe, locks: 0.}
proc flipV*(self: TextureButton): bool {.gcsafe, locks: 0.}
proc `flipV=`*(self: TextureButton; val: bool) {.gcsafe, locks: 0.}
...
var textureButtonIsFlippedHMethodBind {.threadvar.}: ptr GodotMethodBind
proc flipH(self: TextureButton): bool =
  if isNil(textureButtonIsFlippedHMethodBind):
    textureButtonIsFlippedHMethodBind = getMethod(cstring"TextureButton",
        cstring"is_flipped_h")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  textureButtonIsFlippedHMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var textureButtonSetFlipHMethodBind {.threadvar.}: ptr GodotMethodBind
proc `flipH=`(self: TextureButton; val: bool) =
  if isNil(textureButtonSetFlipHMethodBind):
    textureButtonSetFlipHMethodBind = getMethod(cstring"TextureButton",
        cstring"set_flip_h")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  textureButtonSetFlipHMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)

var textureButtonIsFlippedVMethodBind {.threadvar.}: ptr GodotMethodBind
proc flipV(self: TextureButton): bool =
  if isNil(textureButtonIsFlippedVMethodBind):
    textureButtonIsFlippedVMethodBind = getMethod(cstring"TextureButton",
        cstring"is_flipped_v")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  textureButtonIsFlippedVMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var textureButtonSetFlipVMethodBind {.threadvar.}: ptr GodotMethodBind
proc `flipV=`(self: TextureButton; val: bool) =
  if isNil(textureButtonSetFlipVMethodBind):
    textureButtonSetFlipVMethodBind = getMethod(cstring"TextureButton",
        cstring"set_flip_v")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  textureButtonSetFlipVMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
proc stretchMode(self: TextureButton): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  textureButtonGetStretchModeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc textureDisabled(self: TextureButton): Texture =
  if isNil(textureButtonGetDisabledTextureMethodBind):
    textureButtonGetDisabledTextureMethodBind = getMethod(
        cstring"TextureButton", cstring"get_disabled_texture")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc `textureDisabled=`(self: TextureButton; val: Texture) =
  if isNil(textureButtonSetDisabledTextureMethodBind):
    textureButtonSetDisabledTextureMethodBind = getMethod(
        cstring"TextureButton", cstring"set_disabled_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not val.isNil:
...
proc textureFocused(self: TextureButton): Texture =
  if isNil(textureButtonGetFocusedTextureMethodBind):
    textureButtonGetFocusedTextureMethodBind = getMethod(
        cstring"TextureButton", cstring"get_focused_texture")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc `textureFocused=`(self: TextureButton; val: Texture) =
  if isNil(textureButtonSetFocusedTextureMethodBind):
    textureButtonSetFocusedTextureMethodBind = getMethod(
        cstring"TextureButton", cstring"set_focused_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not val.isNil:
...
proc textureHover(self: TextureButton): Texture =
  ...
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  textureButtonGetHoverTextureMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc texturePressed(self: TextureButton): Texture =
  if isNil(textureButtonGetPressedTextureMethodBind):
    textureButtonGetPressedTextureMethodBind = getMethod(
        cstring"TextureButton", cstring"get_pressed_texture")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc `texturePressed=`(self: TextureButton; val: Texture) =
  if isNil(textureButtonSetPressedTextureMethodBind):
    textureButtonSetPressedTextureMethodBind = getMethod(
        cstring"TextureButton", cstring"set_pressed_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not val.isNil:

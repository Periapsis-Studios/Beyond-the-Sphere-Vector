...
proc `radialCenterOffset=`*(self: TextureProgress; val: Vector2) {.gcsafe,
    locks: 0.}
proc radialFillDegrees*(self: TextureProgress): float64 {.gcsafe, locks: 0.}
proc `radialFillDegrees=`*(self: TextureProgress; val: float64) {.gcsafe,
    locks: 0.}
proc radialInitialAngle*(self: TextureProgress): float64 {.gcsafe, locks: 0.}
proc `radialInitialAngle=`*(self: TextureProgress; val: float64) {.gcsafe,
    locks: 0.}
proc stretchMarginBottom*(self: TextureProgress): int64 {.gcsafe, locks: 0.}
proc `stretchMarginBottom=`*(self: TextureProgress; val: int64) {.gcsafe,
    locks: 0.}
...
proc `stretchMarginRight=`*(self: TextureProgress; val: int64) {.gcsafe,
    locks: 0.}
...
proc textureProgressOffset*(self: TextureProgress): Vector2 {.gcsafe, locks: 0.}
proc `textureProgressOffset=`*(self: TextureProgress; val: Vector2) {.gcsafe,
    locks: 0.}
...
proc radialFillDegrees(self: TextureProgress): float64 =
  if isNil(textureProgressGetFillDegreesMethodBind):
    textureProgressGetFillDegreesMethodBind = getMethod(
        cstring"TextureProgress", cstring"get_fill_degrees")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `radialFillDegrees=`(self: TextureProgress; val: float64) =
  if isNil(textureProgressSetFillDegreesMethodBind):
    textureProgressSetFillDegreesMethodBind = getMethod(
        cstring"TextureProgress", cstring"set_fill_degrees")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc textureOver(self: TextureProgress): Texture =
  if isNil(textureProgressGetOverTextureMethodBind):
    textureProgressGetOverTextureMethodBind = getMethod(
        cstring"TextureProgress", cstring"get_over_texture")
  var ptrCallRet: pointer
...
proc `textureOver=`(self: TextureProgress; val: Texture) =
  if isNil(textureProgressSetOverTextureMethodBind):
    textureProgressSetOverTextureMethodBind = getMethod(
        cstring"TextureProgress", cstring"set_over_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var textureProgressGetTextureProgressOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc textureProgressOffset(self: TextureProgress): Vector2 =
  if isNil(textureProgressGetTextureProgressOffsetMethodBind):
    textureProgressGetTextureProgressOffsetMethodBind = getMethod(
        cstring"TextureProgress", cstring"get_texture_progress_offset")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  textureProgressGetTextureProgressOffsetMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)

var textureProgressSetTextureProgressOffsetMethodBind {.threadvar.}: ptr GodotMethodBind
proc `textureProgressOffset=`(self: TextureProgress; val: Vector2) =
  if isNil(textureProgressSetTextureProgressOffsetMethodBind):
    textureProgressSetTextureProgressOffsetMethodBind = getMethod(
        cstring"TextureProgress", cstring"set_texture_progress_offset")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  textureProgressSetTextureProgressOffsetMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc tintUnder(self: TextureProgress): Color =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  textureProgressGetTintUnderMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

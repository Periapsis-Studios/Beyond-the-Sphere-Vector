...
proc isAlive*(self: Thread): bool {.gcsafe, locks: 0.}
...
var threadIsAliveMethodBind {.threadvar.}: ptr GodotMethodBind
proc isAlive(self: Thread): bool =
  if isNil(threadIsAliveMethodBind):
    threadIsAliveMethodBind = getMethod(cstring"_Thread", cstring"is_alive")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  threadIsAliveMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

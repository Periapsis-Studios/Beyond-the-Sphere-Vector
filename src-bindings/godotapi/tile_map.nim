...
proc showCollision*(self: TileMap): bool {.gcsafe, locks: 0.}
proc `showCollision=`*(self: TileMap; val: bool) {.gcsafe, locks: 0.}
...
method setCelld*(self: TileMap; position: Vector2; data: Dictionary) {.gcsafe,
    locks: 0, base.}
...
proc getCellAutotileCoord*(self: TileMap; x: int64; y: int64): Vector2 {.gcsafe,
    locks: 0.}
...
proc isCellTransposed*(self: TileMap; x: int64; y: int64): bool {.gcsafe,
    locks: 0.}
...
proc mapToWorld*(self: TileMap; mapPosition: Vector2;
                 ignoreHalfOfs: bool = false): Vector2 {.gcsafe, locks: 0.}
proc setCell*(self: TileMap; x: int64; y: int64; tile: int64;
              flipX: bool = false; flipY: bool = false; transpose: bool = false;
              autotileCoord: Vector2 = vec2(0.0, 0.0)) {.gcsafe, locks: 0.}
proc setCellv*(self: TileMap; position: Vector2; tile: int64;
               flipX: bool = false; flipY: bool = false;
               transpose: bool = false; autotileCoord: Vector2 = vec2(0.0, 0.0)) {.
    gcsafe, locks: 0.}
proc setCollisionLayerBit*(self: TileMap; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
proc setCollisionMaskBit*(self: TileMap; bit: int64; value: bool) {.gcsafe,
    locks: 0.}
...
proc worldToMap*(self: TileMap; worldPosition: Vector2): Vector2 {.gcsafe,
    locks: 0.}
...
proc cellClipUv(self: TileMap): bool =
  if isNil(tileMapGetClipUvMethodBind):
    tileMapGetClipUvMethodBind = getMethod(cstring"TileMap",
        cstring"get_clip_uv")
...
proc `cellClipUv=`(self: TileMap; val: bool) =
  if isNil(tileMapSetClipUvMethodBind):
    tileMapSetClipUvMethodBind = getMethod(cstring"TileMap",
        cstring"set_clip_uv")
  var
...
proc collisionFriction(self: TileMap): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tileMapGetCollisionFrictionMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `collisionLayer=`(self: TileMap; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  tileMapSetCollisionLayerMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc collisionUseParent(self: TileMap): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tileMapGetCollisionUseParentMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `mode=`(self: TileMap; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  tileMapSetModeMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc occluderLightMask(self: TileMap): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tileMapGetOccluderLightMaskMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
var tileMapIsShowCollisionEnabledMethodBind {.threadvar.}: ptr GodotMethodBind
proc showCollision(self: TileMap): bool =
  if isNil(tileMapIsShowCollisionEnabledMethodBind):
    tileMapIsShowCollisionEnabledMethodBind = getMethod(cstring"TileMap",
        cstring"is_show_collision_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tileMapIsShowCollisionEnabledMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var tileMapSetShowCollisionMethodBind {.threadvar.}: ptr GodotMethodBind
proc `showCollision=`(self: TileMap; val: bool) =
  if isNil(tileMapSetShowCollisionMethodBind):
    tileMapSetShowCollisionMethodBind = getMethod(cstring"TileMap",
        cstring"set_show_collision")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  tileMapSetShowCollisionMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
proc tileSet(self: TileMap): TileSet =
  if isNil(tileMapGetTilesetMethodBind):
    tileMapGetTilesetMethodBind = getMethod(cstring"TileMap",
        cstring"get_tileset")
  var ptrCallRet: pointer
...
proc `tileSet=`(self: TileMap; val: TileSet) =
  if isNil(tileMapSetTilesetMethodBind):
    tileMapSetTilesetMethodBind = getMethod(cstring"TileMap",
        cstring"set_tileset")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
method getTileData(self: TileMap): PoolIntArray =
  ...
  ptrCallRet = addr(ptrCallVal)
  tileMapUnderscoregetTileDataMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getCell(self: TileMap; x: int64; y: int64): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  tileMapGetCellMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc setCell(self: TileMap; x: int64; y: int64; tile: int64;
  ...
  var ptrCallRet: pointer
  tileMapSetCellMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc setCellv(self: TileMap; position: Vector2; tile: int64;
  ...
  argsToPassToGodot[][5] = unsafeAddr(autotileCoord)
  var ptrCallRet: pointer
...
proc updateBitmaskArea(self: TileMap; position: Vector2) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(position)
  var ptrCallRet: pointer
  tileMapUpdateBitmaskAreaMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc updateDirtyQuadrants(self: TileMap) =
  ...
  var ptrCallRet: pointer
  tileMapUpdateDirtyQuadrantsMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

...
proc autostart(self: Timer): bool =
  if isNil(timerHasAutostartMethodBind):
    timerHasAutostartMethodBind = getMethod(cstring"Timer",
        cstring"has_autostart")
...
proc `autostart=`(self: Timer; val: bool) =
  if isNil(timerSetAutostartMethodBind):
    timerSetAutostartMethodBind = getMethod(cstring"Timer",
        cstring"set_autostart")
  var
...
proc `paused=`(self: Timer; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  timerSetPausedMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc `processMode=`(self: Timer; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  timerSetTimerProcessModeMethodBind.ptrCall(self.godotObject,
...
proc timeLeft(self: Timer): float64 =
  if isNil(timerGetTimeLeftMethodBind):
    timerGetTimeLeftMethodBind = getMethod(cstring"Timer",
...
proc waitTime(self: Timer): float64 =
  if isNil(timerGetWaitTimeMethodBind):
    timerGetWaitTimeMethodBind = getMethod(cstring"Timer",
        cstring"get_wait_time")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `waitTime=`(self: Timer; val: float64) =
  if isNil(timerSetWaitTimeMethodBind):
    timerSetWaitTimeMethodBind = getMethod(cstring"Timer",
        cstring"set_wait_time")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)

...
proc action(self: TouchScreenButton): string =
  if isNil(touchScreenButtonGetActionMethodBind):
    touchScreenButtonGetActionMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"get_action")
...
proc `action=`(self: TouchScreenButton; val: string) =
  if isNil(touchScreenButtonSetActionMethodBind):
    touchScreenButtonSetActionMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"set_action")
  var
...
proc bitmask(self: TouchScreenButton): BitMap =
  if isNil(touchScreenButtonGetBitmaskMethodBind):
    touchScreenButtonGetBitmaskMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"get_bitmask")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  touchScreenButtonGetBitmaskMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc `bitmask=`(self: TouchScreenButton; val: BitMap) =
  if isNil(touchScreenButtonSetBitmaskMethodBind):
    touchScreenButtonSetBitmaskMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"set_bitmask")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc normal(self: TouchScreenButton): Texture =
  if isNil(touchScreenButtonGetTextureMethodBind):
    touchScreenButtonGetTextureMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"get_texture")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  touchScreenButtonGetTextureMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc `normal=`(self: TouchScreenButton; val: Texture) =
  if isNil(touchScreenButtonSetTextureMethodBind):
    touchScreenButtonSetTextureMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"set_texture")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not val.isNil:
...
proc isPressed(self: TouchScreenButton): bool =
  if isNil(touchScreenButtonIsPressedMethodBind):
    touchScreenButtonIsPressedMethodBind = getMethod(
        cstring"TouchScreenButton", cstring"is_pressed")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

...
method getMessage*(self: Translation; srcMessage: string): string {.gcsafe,
    locks: 0, base.}
...
proc getMessageImpl*(self: Translation; srcMessage: string): string {.gcsafe,
    locks: 0.}
...
proc messages(self: Translation): PoolStringArray =
  if isNil(translationUnderscoregetMessagesMethodBind):
    translationUnderscoregetMessagesMethodBind = getMethod(
        cstring"Translation", cstring"_get_messages")
  var ptrCallRet: pointer
...
proc `messages=`(self: Translation; val: PoolStringArray) =
  if isNil(translationUnderscoresetMessagesMethodBind):
    translationUnderscoresetMessagesMethodBind = getMethod(
        cstring"Translation", cstring"_set_messages")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var translationUnderscoregetMessageMethodBind {.threadvar.}: ptr GodotMethodBind
method getMessage(self: Translation; srcMessage: string): string =
  if isNil(translationUnderscoregetMessageMethodBind):
    translationUnderscoregetMessageMethodBind = getMethod(cstring"Translation",
        cstring"_get_message")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  translationUnderscoregetMessageMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc getMessageImpl(self: Translation; srcMessage: string): string =

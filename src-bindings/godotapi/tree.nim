...
proc createItem*(self: Tree; parent: Object = nil; idx: int64 = -1'i64): TreeItem {.
    gcsafe, locks: 0.}
proc editSelected*(self: Tree): bool {.gcsafe, locks: 0.}
proc ensureCursorIsVisible*(self: Tree) {.gcsafe, locks: 0.}
proc getColumnAtPosition*(self: Tree; position: Vector2): int64 {.gcsafe,
    locks: 0.}
...
proc getDropSectionAtPosition*(self: Tree; position: Vector2): int64 {.gcsafe,
    locks: 0.}
...
proc getItemAreaRect*(self: Tree; item: Object; column: int64 = -1'i64): Rect2 {.
    gcsafe, locks: 0.}
proc getItemAtPosition*(self: Tree; position: Vector2): TreeItem {.gcsafe,
    locks: 0.}
...
proc scrollToItem*(self: Tree; item: Object) {.gcsafe, locks: 0.}
proc setColumnExpand*(self: Tree; column: int64; expand: bool) {.gcsafe,
    locks: 0.}
proc setColumnMinWidth*(self: Tree; column: int64; minWidth: int64) {.gcsafe,
    locks: 0.}
proc setColumnTitle*(self: Tree; column: int64; title: string) {.gcsafe,
    locks: 0.}
...
proc `columns=`(self: Tree; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  treeSetColumnsMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc hideRoot(self: Tree): bool =
  if isNil(treeIsRootHiddenMethodBind):
    treeIsRootHiddenMethodBind = getMethod(cstring"Tree",
        cstring"is_root_hidden")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc createItem(self: Tree; parent: Object = nil; idx: int64 = -1'i64): TreeItem =
  ...
  treeCreateItemMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)

var treeEditSelectedMethodBind {.threadvar.}: ptr GodotMethodBind
proc editSelected(self: Tree): bool =
  if isNil(treeEditSelectedMethodBind):
    treeEditSelectedMethodBind = getMethod(cstring"Tree",
        cstring"edit_selected")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  treeEditSelectedMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)
...
var treeScrollToItemMethodBind {.threadvar.}: ptr GodotMethodBind
proc scrollToItem(self: Tree; item: Object) =
  if isNil(treeScrollToItemMethodBind):
    treeScrollToItemMethodBind = getMethod(cstring"Tree",
        cstring"scroll_to_item")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not item.isNil:
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  treeScrollToItemMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                     ptrCallRet)

...
proc addButton*(self: TreeItem; column: int64; button: Texture;
                buttonIdx: int64 = -1'i64; disabled: bool = false;
                tooltip: string = "") {.gcsafe, locks: 0.}
proc callRecursive*(self: TreeItem; methodd: string;
...
proc eraseButton*(self: TreeItem; column: int64; buttonIdx: int64) {.gcsafe,
    locks: 0.}
...
proc getNextVisible*(self: TreeItem; wrap: bool = false): TreeItem {.gcsafe,
    locks: 0.}
proc getParent*(self: TreeItem): TreeItem {.gcsafe, locks: 0.}
proc getPrev*(self: TreeItem): TreeItem {.gcsafe, locks: 0.}
proc getPrevVisible*(self: TreeItem; wrap: bool = false): TreeItem {.gcsafe,
    locks: 0.}
proc getRange*(self: TreeItem; column: int64): float64 {.gcsafe, locks: 0.}
proc getRangeConfig*(self: TreeItem; column: int64): Dictionary {.gcsafe,
    locks: 0.}
...
proc isCustomSetAsButton*(self: TreeItem; column: int64): bool {.gcsafe,
    locks: 0.}
...
proc setButton*(self: TreeItem; column: int64; buttonIdx: int64; button: Texture) {.
    gcsafe, locks: 0.}
proc setButtonDisabled*(self: TreeItem; column: int64; buttonIdx: int64;
                        disabled: bool) {.gcsafe, locks: 0.}
proc setCellMode*(self: TreeItem; column: int64; mode: int64) {.gcsafe, locks: 0.}
proc setChecked*(self: TreeItem; column: int64; checked: bool) {.gcsafe,
    locks: 0.}
proc setCustomAsButton*(self: TreeItem; column: int64; enable: bool) {.gcsafe,
    locks: 0.}
proc setCustomBgColor*(self: TreeItem; column: int64; color: Color;
                       justOutline: bool = false) {.gcsafe, locks: 0.}
proc setCustomColor*(self: TreeItem; column: int64; color: Color) {.gcsafe,
    locks: 0.}
proc setCustomDraw*(self: TreeItem; column: int64; objectt: Object;
                    callback: string) {.gcsafe, locks: 0.}
proc setEditable*(self: TreeItem; column: int64; enabled: bool) {.gcsafe,
    locks: 0.}
proc setExpandRight*(self: TreeItem; column: int64; enable: bool) {.gcsafe,
    locks: 0.}
proc setIcon*(self: TreeItem; column: int64; texture: Texture) {.gcsafe,
    locks: 0.}
proc setIconMaxWidth*(self: TreeItem; column: int64; width: int64) {.gcsafe,
    locks: 0.}
proc setIconModulate*(self: TreeItem; column: int64; modulate: Color) {.gcsafe,
    locks: 0.}
proc setIconRegion*(self: TreeItem; column: int64; region: Rect2) {.gcsafe,
    locks: 0.}
proc setMetadata*(self: TreeItem; column: int64; meta: Variant) {.gcsafe,
    locks: 0.}
...
proc setSelectable*(self: TreeItem; column: int64; selectable: bool) {.gcsafe,
    locks: 0.}
...
proc setTextAlign*(self: TreeItem; column: int64; textAlign: int64) {.gcsafe,
    locks: 0.}
proc setTooltip*(self: TreeItem; column: int64; tooltip: string) {.gcsafe,
    locks: 0.}
...
proc addButton(self: TreeItem; column: int64; button: Texture;
  ...
  if isNil(treeItemAddButtonMethodBind):
    treeItemAddButtonMethodBind = getMethod(cstring"TreeItem",
        cstring"add_button")
  var
...
proc clearCustomColor(self: TreeItem; column: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(column)
  var ptrCallRet: pointer
  treeItemClearCustomColorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getButton(self: TreeItem; column: int64; buttonIdx: int64): Texture =
  if isNil(treeItemGetButtonMethodBind):
    treeItemGetButtonMethodBind = getMethod(cstring"TreeItem",
        cstring"get_button")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getButtonTooltip(self: TreeItem; column: int64; buttonIdx: int64): string =
  ...
  ptrCallRet = addr(ptrCallVal)
  treeItemGetButtonTooltipMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc getCustomBgColor(self: TreeItem; column: int64): Color =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  treeItemGetCustomBgColorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc getParent(self: TreeItem): TreeItem =
  if isNil(treeItemGetParentMethodBind):
    treeItemGetParentMethodBind = getMethod(cstring"TreeItem",
        cstring"get_parent")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc getRange(self: TreeItem; column: int64): float64 =
  if isNil(treeItemGetRangeMethodBind):
    treeItemGetRangeMethodBind = getMethod(cstring"TreeItem",
        cstring"get_range")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc getSuffix(self: TreeItem; column: int64): string =
  if isNil(treeItemGetSuffixMethodBind):
    treeItemGetSuffixMethodBind = getMethod(cstring"TreeItem",
        cstring"get_suffix")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc isButtonDisabled(self: TreeItem; column: int64; buttonIdx: int64): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  treeItemIsButtonDisabledMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc isChecked(self: TreeItem; column: int64): bool =
  if isNil(treeItemIsCheckedMethodBind):
    treeItemIsCheckedMethodBind = getMethod(cstring"TreeItem",
        cstring"is_checked")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc select(self: TreeItem; column: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(column)
  var ptrCallRet: pointer
  treeItemSelectMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)

var treeItemSetButtonMethodBind {.threadvar.}: ptr GodotMethodBind
proc setButton(self: TreeItem; column: int64; buttonIdx: int64; button: Texture) =
  if isNil(treeItemSetButtonMethodBind):
    treeItemSetButtonMethodBind = getMethod(cstring"TreeItem",
        cstring"set_button")
  var
...
proc setCustomBgColor(self: TreeItem; column: int64; color: Color;
  ...
  var ptrCallRet: pointer
  treeItemSetCustomBgColorMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc setRange(self: TreeItem; column: int64; value: float64) =
  if isNil(treeItemSetRangeMethodBind):
    treeItemSetRangeMethodBind = getMethod(cstring"TreeItem",
        cstring"set_range")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setSuffix(self: TreeItem; column: int64; text: string) =
  if isNil(treeItemSetSuffixMethodBind):
    treeItemSetSuffixMethodBind = getMethod(cstring"TreeItem",
        cstring"set_suffix")
  var
    argsStatic: array[2, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

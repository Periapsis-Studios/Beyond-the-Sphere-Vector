...
proc addPortMapping*(self: UPNP; port: int64; portInternal: int64 = 0'i64;
                     desc: string = ""; proto: string = "UDP";
                     duration: int64 = 0'i64): int64 {.gcsafe, locks: 0.}
...
proc `discoverLocalPort=`(self: UPNP; val: int64) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  uPNPSetDiscoverLocalPortMethodBind.ptrCall(self.godotObject,
...
proc addDevice(self: UPNP; device: UPNPDevice) =
  ...
  var ptrCallRet: pointer
  uPNPAddDeviceMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)

var uPNPAddPortMappingMethodBind {.threadvar.}: ptr GodotMethodBind
proc addPortMapping(self: UPNP; port: int64; portInternal: int64 = 0'i64;
                    desc: string = ""; proto: string = "UDP";
                    duration: int64 = 0'i64): int64 =
...
proc clearDevices(self: UPNP) =
  if isNil(uPNPClearDevicesMethodBind):
    uPNPClearDevicesMethodBind = getMethod(cstring"UPNP",
        cstring"clear_devices")
  var ptrCallRet: pointer
...
proc getDevice(self: UPNP; index: int64): UPNPDevice =
  ...
  ptrCallRet = addr(ptrCallVal)
  uPNPGetDeviceMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc removeDevice(self: UPNP; index: int64) =
  if isNil(uPNPRemoveDeviceMethodBind):
    uPNPRemoveDeviceMethodBind = getMethod(cstring"UPNP",
        cstring"remove_device")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(index)
...
proc setDevice(self: UPNP; index: int64; device: UPNPDevice) =
  ...
  var ptrCallRet: pointer
  uPNPSetDeviceMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)

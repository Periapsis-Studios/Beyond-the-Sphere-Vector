...
proc descriptionUrl(self: UPNPDevice): string =
  ...
  uPNPDeviceGetDescriptionUrlMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc igdServiceType(self: UPNPDevice): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  uPNPDeviceGetIgdServiceTypeMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc `serviceType=`(self: UPNPDevice; val: string) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(argToPassToGodot0)
  var ptrCallRet: pointer
  uPNPDeviceSetServiceTypeMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
  deinit(argToPassToGodot0)
...
proc addPortMapping(self: UPNPDevice; port: int64; portInternal: int64 = 0'i64;
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  uPNPDeviceAddPortMappingMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)

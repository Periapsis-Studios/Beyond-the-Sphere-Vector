...
proc `suspensionStiffness=`*(self: VehicleWheel; val: float64) {.gcsafe,
    locks: 0.}
...
proc dampingRelaxation(self: VehicleWheel): float64 =
  if isNil(vehicleWheelGetDampingRelaxationMethodBind):
    vehicleWheelGetDampingRelaxationMethodBind = getMethod(
        cstring"VehicleWheel", cstring"get_damping_relaxation")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `dampingRelaxation=`(self: VehicleWheel; val: float64) =
  if isNil(vehicleWheelSetDampingRelaxationMethodBind):
    vehicleWheelSetDampingRelaxationMethodBind = getMethod(
        cstring"VehicleWheel", cstring"set_damping_relaxation")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc suspensionTravel(self: VehicleWheel): float64 =
  if isNil(vehicleWheelGetSuspensionTravelMethodBind):
    vehicleWheelGetSuspensionTravelMethodBind = getMethod(
        cstring"VehicleWheel", cstring"get_suspension_travel")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `suspensionTravel=`(self: VehicleWheel; val: float64) =
  if isNil(vehicleWheelSetSuspensionTravelMethodBind):
    vehicleWheelSetSuspensionTravelMethodBind = getMethod(
        cstring"VehicleWheel", cstring"set_suspension_travel")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
...
proc useAsSteering(self: VehicleWheel): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  vehicleWheelIsUsedAsSteeringMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc useAsTraction(self: VehicleWheel): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  vehicleWheelIsUsedAsTractionMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc wheelFrictionSlip(self: VehicleWheel): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  vehicleWheelGetFrictionSlipMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc wheelRollInfluence(self: VehicleWheel): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  vehicleWheelGetRollInfluenceMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

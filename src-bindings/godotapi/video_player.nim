...
proc `audioTrack=`(self: VideoPlayer; val: int64) =
  ...
  var ptrCallRet: pointer
  videoPlayerSetAudioTrackMethodBind.ptrCall(self.godotObject,
...
proc bufferingMsec(self: VideoPlayer): int64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  videoPlayerGetBufferingMsecMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc bus(self: VideoPlayer): string =
  if isNil(videoPlayerGetBusMethodBind):
    videoPlayerGetBusMethodBind = getMethod(cstring"VideoPlayer",
        cstring"get_bus")
  var ptrCallRet: pointer
...
proc `bus=`(self: VideoPlayer; val: string) =
  if isNil(videoPlayerSetBusMethodBind):
    videoPlayerSetBusMethodBind = getMethod(cstring"VideoPlayer",
        cstring"set_bus")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc streamPosition(self: VideoPlayer): float64 =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  videoPlayerGetStreamPositionMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

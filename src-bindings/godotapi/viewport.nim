...
proc debanding*(self: Viewport): bool {.gcsafe, locks: 0.}
proc `debanding=`*(self: Viewport; val: bool) {.gcsafe, locks: 0.}
...
proc fxaa*(self: Viewport): bool {.gcsafe, locks: 0.}
proc `fxaa=`*(self: Viewport; val: bool) {.gcsafe, locks: 0.}
proc globalCanvasTransform*(self: Viewport): Transform2D {.gcsafe, locks: 0.}
proc `globalCanvasTransform=`*(self: Viewport; val: Transform2D) {.gcsafe,
    locks: 0.}
...
proc sharpenIntensity*(self: Viewport): float64 {.gcsafe, locks: 0.}
proc `sharpenIntensity=`*(self: Viewport; val: float64) {.gcsafe, locks: 0.}
...
method processPicking*(self: Viewport; ignorePaused: bool) {.gcsafe, locks: 0,
    base.}
...
method vpUnhandledInput*(self: Viewport; arg0: InputEvent) {.gcsafe, locks: 0,
    base.}
...
var viewportGetUseDebandingMethodBind {.threadvar.}: ptr GodotMethodBind
proc debanding(self: Viewport): bool =
  if isNil(viewportGetUseDebandingMethodBind):
    viewportGetUseDebandingMethodBind = getMethod(cstring"Viewport",
        cstring"get_use_debanding")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  viewportGetUseDebandingMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var viewportSetUseDebandingMethodBind {.threadvar.}: ptr GodotMethodBind
proc `debanding=`(self: Viewport; val: bool) =
  if isNil(viewportSetUseDebandingMethodBind):
    viewportSetUseDebandingMethodBind = getMethod(cstring"Viewport",
        cstring"set_use_debanding")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  viewportSetUseDebandingMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
      ptrCallRet)
...
var viewportGetUseFxaaMethodBind {.threadvar.}: ptr GodotMethodBind
proc fxaa(self: Viewport): bool =
  if isNil(viewportGetUseFxaaMethodBind):
    viewportGetUseFxaaMethodBind = getMethod(cstring"Viewport",
        cstring"get_use_fxaa")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  viewportGetUseFxaaMethodBind.ptrCall(self.godotObject, nil, ptrCallRet)

var viewportSetUseFxaaMethodBind {.threadvar.}: ptr GodotMethodBind
proc `fxaa=`(self: Viewport; val: bool) =
  if isNil(viewportSetUseFxaaMethodBind):
    viewportSetUseFxaaMethodBind = getMethod(cstring"Viewport",
        cstring"set_use_fxaa")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  viewportSetUseFxaaMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                       ptrCallRet)
...
proc `hdr=`(self: Viewport; val: bool) =
  ...
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  viewportSetHdrMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                   ptrCallRet)
...
proc renderDirectToScreen(self: Viewport): bool =
  if isNil(viewportIsUsingRenderDirectToScreenMethodBind):
    viewportIsUsingRenderDirectToScreenMethodBind = getMethod(
        cstring"Viewport", cstring"is_using_render_direct_to_screen")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc renderTargetVFlip(self: Viewport): bool =
  if isNil(viewportGetVflipMethodBind):
    viewportGetVflipMethodBind = getMethod(cstring"Viewport",
        cstring"get_vflip")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `renderTargetVFlip=`(self: Viewport; val: bool) =
  if isNil(viewportSetVflipMethodBind):
    viewportSetVflipMethodBind = getMethod(cstring"Viewport",
        cstring"set_vflip")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc shadowAtlasQuad0(self: Viewport): int64 =
  if isNil(viewportGetShadowAtlasQuadrantSubdivMethodBind):
    viewportGetShadowAtlasQuadrantSubdivMethodBind = getMethod(
        cstring"Viewport", cstring"get_shadow_atlas_quadrant_subdiv")
  var
...
proc `shadowAtlasQuad0=`(self: Viewport; val: int64) =
  if isNil(viewportSetShadowAtlasQuadrantSubdivMethodBind):
    viewportSetShadowAtlasQuadrantSubdivMethodBind = getMethod(
        cstring"Viewport", cstring"set_shadow_atlas_quadrant_subdiv")
  var
    godotBoundArg0 = 0'i64
...
var viewportGetSharpenIntensityMethodBind {.threadvar.}: ptr GodotMethodBind
proc sharpenIntensity(self: Viewport): float64 =
  if isNil(viewportGetSharpenIntensityMethodBind):
    viewportGetSharpenIntensityMethodBind = getMethod(cstring"Viewport",
        cstring"get_sharpen_intensity")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  viewportGetSharpenIntensityMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var viewportSetSharpenIntensityMethodBind {.threadvar.}: ptr GodotMethodBind
proc `sharpenIntensity=`(self: Viewport; val: float64) =
  if isNil(viewportSetSharpenIntensityMethodBind):
    viewportSetSharpenIntensityMethodBind = getMethod(cstring"Viewport",
        cstring"set_sharpen_intensity")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  viewportSetSharpenIntensityMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc sizeOverrideStretch(self: Viewport): bool =
  if isNil(viewportIsSizeOverrideStretchEnabledMethodBind):
    viewportIsSizeOverrideStretchEnabledMethodBind = getMethod(
        cstring"Viewport", cstring"is_size_override_stretch_enabled")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc usage(self: Viewport): int64 =
  if isNil(viewportGetUsageMethodBind):
    viewportGetUsageMethodBind = getMethod(cstring"Viewport",
        cstring"get_usage")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc `usage=`(self: Viewport; val: int64) =
  if isNil(viewportSetUsageMethodBind):
    viewportSetUsageMethodBind = getMethod(cstring"Viewport",
        cstring"set_usage")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc world(self: Viewport): World =
  if isNil(viewportGetWorldMethodBind):
    viewportGetWorldMethodBind = getMethod(cstring"Viewport",
        cstring"get_world")
  var ptrCallRet: pointer
...
proc `world=`(self: Viewport; val: World) =
  if isNil(viewportSetWorldMethodBind):
    viewportSetWorldMethodBind = getMethod(cstring"Viewport",
        cstring"set_world")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
var viewportUnderscoreprocessPickingMethodBind {.threadvar.}: ptr GodotMethodBind
method processPicking(self: Viewport; ignorePaused: bool) =
  if isNil(viewportUnderscoreprocessPickingMethodBind):
    viewportUnderscoreprocessPickingMethodBind = getMethod(cstring"Viewport",
        cstring"_process_picking")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(ignorePaused)
  var ptrCallRet: pointer
  viewportUnderscoreprocessPickingMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc findWorld(self: Viewport): World =
  if isNil(viewportFindWorldMethodBind):
    viewportFindWorldMethodBind = getMethod(cstring"Viewport",
        cstring"find_world")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc getCamera(self: Viewport): Camera =
  if isNil(viewportGetCameraMethodBind):
    viewportGetCameraMethodBind = getMethod(cstring"Viewport",
        cstring"get_camera")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
...
proc input(self: Viewport; localEvent: InputEvent) =
  ...
  argsToPassToGodot[][0] = argToPassToGodot0
  var ptrCallRet: pointer
  viewportInputMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
...
proc warpMouse(self: Viewport; toPosition: Vector2) =
  if isNil(viewportWarpMouseMethodBind):
    viewportWarpMouseMethodBind = getMethod(cstring"Viewport",
        cstring"warp_mouse")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)

...
method unhandledInput*(self: ViewportContainer; event: InputEvent) {.gcsafe,
    locks: 0.}
...
proc `stretch=`(self: ViewportContainer; val: bool) =
  if isNil(viewportContainerSetStretchMethodBind):
    viewportContainerSetStretchMethodBind = getMethod(
        cstring"ViewportContainer", cstring"set_stretch")
  var

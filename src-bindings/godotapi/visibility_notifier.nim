...
proc aabb(self: VisibilityNotifier): AABB =
  if isNil(visibilityNotifierGetAabbMethodBind):
    visibilityNotifierGetAabbMethodBind = getMethod(
        cstring"VisibilityNotifier", cstring"get_aabb")
...
proc `aabb=`(self: VisibilityNotifier; val: AABB) =
  if isNil(visibilityNotifierSetAabbMethodBind):
    visibilityNotifierSetAabbMethodBind = getMethod(
        cstring"VisibilityNotifier", cstring"set_aabb")
  var
...
proc isOnScreen(self: VisibilityNotifier): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  visibilityNotifierIsOnScreenMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

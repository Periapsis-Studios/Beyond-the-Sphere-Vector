...
import
  godot, godottypes, godotinternal, cull_instance

export
  godottypes, cull_instance
...
proc getLayerMaskBit*(self: VisualInstance; layer: int64): bool {.gcsafe,
    locks: 0.}
...
proc getLayerMaskBit(self: VisualInstance; layer: int64): bool =
  if isNil(visualInstanceGetLayerMaskBitMethodBind):
    visualInstanceGetLayerMaskBitMethodBind = getMethod(
        cstring"VisualInstance", cstring"get_layer_mask_bit")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
...
proc setLayerMaskBit(self: VisualInstance; layer: int64; enabled: bool) =
  if isNil(visualInstanceSetLayerMaskBitMethodBind):
    visualInstanceSetLayerMaskBitMethodBind = getMethod(
        cstring"VisualInstance", cstring"set_layer_mask_bit")
  var

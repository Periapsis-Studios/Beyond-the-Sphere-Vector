...
proc getBufferedAmount*(self: WebRTCDataChannel): int64 {.gcsafe, locks: 0.}
...
var webRTCDataChannelGetBufferedAmountMethodBind {.threadvar.}: ptr GodotMethodBind
proc getBufferedAmount(self: WebRTCDataChannel): int64 =
  if isNil(webRTCDataChannelGetBufferedAmountMethodBind):
    webRTCDataChannelGetBufferedAmountMethodBind = getMethod(
        cstring"WebRTCDataChannel", cstring"get_buffered_amount")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  webRTCDataChannelGetBufferedAmountMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
...
proc getProtocol(self: WebRTCDataChannel): string =
  ...
  var ptrCallVal: GodotString
  ptrCallRet = addr(ptrCallVal)
  webRTCDataChannelGetProtocolMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = $ptrCallVal
  deinit(ptrCallVal)
...
proc isOrdered(self: WebRTCDataChannel): bool =
  if isNil(webRTCDataChannelIsOrderedMethodBind):
    webRTCDataChannelIsOrderedMethodBind = getMethod(
        cstring"WebRTCDataChannel", cstring"is_ordered")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)

...
proc addPeer*(self: WebRTCMultiplayer; peer: WebRTCPeerConnection;
              peerId: int64; unreliableLifetime: int64 = 1'i64): Error {.gcsafe,
    locks: 0.}
proc close*(self: WebRTCMultiplayer) {.gcsafe, locks: 0.}
proc getPeer*(self: WebRTCMultiplayer; peerId: int64): Dictionary {.gcsafe,
    locks: 0.}
...
proc addPeer(self: WebRTCMultiplayer; peer: WebRTCPeerConnection; peerId: int64;
  ...
  webRTCMultiplayerAddPeerMethodBind.ptrCall(self.godotObject,
...
proc getPeer(self: WebRTCMultiplayer; peerId: int64): Dictionary =
  ...
  ptrCallRet = addr(ptrCallVal)
  webRTCMultiplayerGetPeerMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc hasPeer(self: WebRTCMultiplayer; peerId: int64): bool =
  ...
  webRTCMultiplayerHasPeerMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc initialize(self: WebRTCMultiplayer; peerId: int64;
  ...
  if isNil(webRTCMultiplayerInitializeMethodBind):
    webRTCMultiplayerInitializeMethodBind = getMethod(
        cstring"WebRTCMultiplayer", cstring"initialize")
  var
...
proc removePeer(self: WebRTCMultiplayer; peerId: int64) =
  if isNil(webRTCMultiplayerRemovePeerMethodBind):
    webRTCMultiplayerRemovePeerMethodBind = getMethod(
        cstring"WebRTCMultiplayer", cstring"remove_peer")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(peerId)

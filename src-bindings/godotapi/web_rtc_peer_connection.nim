...
proc initialize*(self: WebRTCPeerConnection;
                 configuration: Dictionary = newDictionary()): Error {.gcsafe,
    locks: 0.}
...
proc setLocalDescription*(self: WebRTCPeerConnection; typee: string; sdp: string): Error {.
    gcsafe, locks: 0.}
proc setRemoteDescription*(self: WebRTCPeerConnection; typee: string;
                           sdp: string): Error {.gcsafe, locks: 0.}
...
proc getConnectionState(self: WebRTCPeerConnection): int64 =
  ...
  webRTCPeerConnectionGetConnectionStateMethodBind.ptrCall(self.godotObject,
      nil, ptrCallRet)
...
proc poll(self: WebRTCPeerConnection): Error =
  if isNil(webRTCPeerConnectionPollMethodBind):
    webRTCPeerConnectionPollMethodBind = getMethod(
        cstring"WebRTCPeerConnection", cstring"poll")
  var ptrCallRet: pointer
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)

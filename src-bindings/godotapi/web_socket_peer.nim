...
proc getCurrentOutboundBufferedAmount*(self: WebSocketPeer): int64 {.gcsafe,
    locks: 0.}
...
var webSocketPeerGetCurrentOutboundBufferedAmountMethodBind {.threadvar.}: ptr GodotMethodBind
proc getCurrentOutboundBufferedAmount(self: WebSocketPeer): int64 =
  if isNil(webSocketPeerGetCurrentOutboundBufferedAmountMethodBind):
    webSocketPeerGetCurrentOutboundBufferedAmountMethodBind = getMethod(
        cstring"WebSocketPeer", cstring"get_current_outbound_buffered_amount")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  webSocketPeerGetCurrentOutboundBufferedAmountMethodBind.ptrCall(
...
proc isConnectedToHost(self: WebSocketPeer): bool =
  if isNil(webSocketPeerIsConnectedToHostMethodBind):
    webSocketPeerIsConnectedToHostMethodBind = getMethod(
        cstring"WebSocketPeer", cstring"is_connected_to_host")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc wasStringPacket(self: WebSocketPeer): bool =
  ...
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  webSocketPeerWasStringPacketMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

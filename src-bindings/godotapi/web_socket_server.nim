...
proc handshakeTimeout*(self: WebSocketServer): float64 {.gcsafe, locks: 0.}
proc `handshakeTimeout=`*(self: WebSocketServer; val: float64) {.gcsafe,
    locks: 0.}
...
proc getPeerAddress*(self: WebSocketServer; id: int64): string {.gcsafe,
    locks: 0.}
...
proc listen*(self: WebSocketServer; port: int64;
             ...
             gdMpApi: bool = false): Error {.gcsafe, locks: 0.}
...
proc `bindIp=`(self: WebSocketServer; val: string) =
  ...
  var ptrCallRet: pointer
  webSocketServerSetBindIpMethodBind.ptrCall(self.godotObject,
...
var webSocketServerGetHandshakeTimeoutMethodBind {.threadvar.}: ptr GodotMethodBind
proc handshakeTimeout(self: WebSocketServer): float64 =
  if isNil(webSocketServerGetHandshakeTimeoutMethodBind):
    webSocketServerGetHandshakeTimeoutMethodBind = getMethod(
        cstring"WebSocketServer", cstring"get_handshake_timeout")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
  webSocketServerGetHandshakeTimeoutMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)

var webSocketServerSetHandshakeTimeoutMethodBind {.threadvar.}: ptr GodotMethodBind
proc `handshakeTimeout=`(self: WebSocketServer; val: float64) =
  if isNil(webSocketServerSetHandshakeTimeoutMethodBind):
    webSocketServerSetHandshakeTimeoutMethodBind = getMethod(
        cstring"WebSocketServer", cstring"set_handshake_timeout")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(val)
  var ptrCallRet: pointer
  webSocketServerSetHandshakeTimeoutMethodBind.ptrCall(self.godotObject,
      argsToPassToGodot, ptrCallRet)
...
proc privateKey(self: WebSocketServer): CryptoKey =
  if isNil(webSocketServerGetPrivateKeyMethodBind):
    webSocketServerGetPrivateKeyMethodBind = getMethod(
        cstring"WebSocketServer", cstring"get_private_key")
  var ptrCallRet: pointer
  var ptrCallVal: ptr GodotObject
  ptrCallRet = addr(ptrCallVal)
  webSocketServerGetPrivateKeyMethodBind.ptrCall(self.godotObject, nil,
      ptrCallRet)
  result = asNimGodotObject[type(result)](ptrCallVal, false, true)
...
proc `privateKey=`(self: WebSocketServer; val: CryptoKey) =
  if isNil(webSocketServerSetPrivateKeyMethodBind):
    webSocketServerSetPrivateKeyMethodBind = getMethod(
        cstring"WebSocketServer", cstring"set_private_key")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  let argToPassToGodot0 = if not val.isNil:
...
proc disconnectPeer(self: WebSocketServer; id: int64; code: int64 = 1000'i64;
  ...
  if isNil(webSocketServerDisconnectPeerMethodBind):
    webSocketServerDisconnectPeerMethodBind = getMethod(
        cstring"WebSocketServer", cstring"disconnect_peer")
  var
...
proc getPeerAddress(self: WebSocketServer; id: int64): string =
  if isNil(webSocketServerGetPeerAddressMethodBind):
    webSocketServerGetPeerAddressMethodBind = getMethod(
        cstring"WebSocketServer", cstring"get_peer_address")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  argsToPassToGodot[][0] = unsafeAddr(id)

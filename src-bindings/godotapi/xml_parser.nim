...
proc getNamedAttributeValue*(self: XMLParser; name: string): string {.gcsafe,
    locks: 0.}
...
proc openBuffer*(self: XMLParser; buffer: PoolByteArray): Error {.gcsafe,
    locks: 0.}
...
proc getNamedAttributeValueSafe(self: XMLParser; name: string): string =
  if isNil(xMLParserGetNamedAttributeValueSafeMethodBind):
    xMLParserGetNamedAttributeValueSafeMethodBind = getMethod(
        cstring"XMLParser", cstring"get_named_attribute_value_safe")
  var
    argsStatic: array[1, pointer]
    argsToPassToGodot = cast[ptr array[MAX_ARG_COUNT, pointer]](addr argsStatic)
  var argToPassToGodot0 = toGodotString(name)
...
proc isEmpty(self: XMLParser): bool =
  if isNil(xMLParserIsEmptyMethodBind):
    xMLParserIsEmptyMethodBind = getMethod(cstring"XMLParser",
        cstring"is_empty")
  var ptrCallRet: pointer
  ptrCallRet = addr(result)
...
proc open(self: XMLParser; file: string): Error =
  ...
  ptrCallRet = addr(ptrCallVal)
  xMLParserOpenMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  deinit(argToPassToGodot0)
...
proc seek(self: XMLParser; position: int64): Error =
  ...
  var ptrCallVal: int64
  ptrCallRet = addr(ptrCallVal)
  xMLParserSeekMethodBind.ptrCall(self.godotObject, argsToPassToGodot,
                                  ptrCallRet)
  result = Error(ptrCallVal)
